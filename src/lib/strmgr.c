...
int ustr_mgr_size(ustr_mgr_ptr smp) { return (hash_size(&(smp->utbl))); }
...
void free_temp_str_mgr(tstr_ptr tp) { free_rec(&tstr_rec_mgr, (pointer)tp); }
...
string gen_strtruncate(tstr_ptr tp, int len) {
  ASSERT(tp->initialized == STRMGR_MAGIC_NBR);
  if (len < 0) {
    len = 0;
  }
