...
void duplicate_hash_struct(hash_record_ptr old_hp, hash_record_ptr new_hp) {
...
unint str_hash(pointer key, unint size) {
  string s = (string)key;
  unint hash = 0;
  while (*s) {
    hash += *s;
...
unsigned int ptr_hash(pointer np, unsigned int n) {
  return (((unint)((lunint)np)) % n);
}

bool ptr_equ(pointer p1, pointer p2) { return (p1 == p2); }

unint int_hash(pointer p, unint n) { return (((long unsigned int)p) % n); }

bool int_equ(pointer p1, pointer p2) {
...
void scan_hash(hash_record_ptr hp, void (*scan_fn)(pointer key, pointer data)) {
  bucket_ptr *bpp;
...
int hash_size(hash_record_ptr hp) { return (hp->elements); }
