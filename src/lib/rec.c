...
void new_mgr(rec_mgr_ptr mp, unint size) { aligned_new_mgr(mp, size, 1); }
...
int ensure_pure_mgr(rec_mgr_ptr mp, char *file, int line) {
  if (!mp->pure) {
...
bool owned_by_mgr(rec_mgr_ptr mp, pointer item) {
  ...
  for (pointer cur_blk = mp->first_blk; cur_blk != NULL;
