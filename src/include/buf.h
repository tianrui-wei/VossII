...
#define M_LOCATE_BUF(bp, i)                                                    \
  (((bp)->buf_size >= (unint)(i))                                              \
       ? (bp)->data + (((unint)(i)) * (bp)->item_size)                         \
       : NULL)
#define FAST_LOC_BUF(bp, i) ((bp)->data + ((((unint)i)) * (bp)->item_size))

#define FOR_BUF(bp, t, item)                                                   \
  for ((item) = (t *)(bp)->data; (item) < ((t *)(bp)->data) + (bp)->buf_size;  \
       (item)++)

#define FUB_ROF(bp, t, item)                                                   \
  for ((item) = ((t *)(bp)->data) + (bp)->buf_size - 1;                        \
       (item) >= (t *)(bp)->data; (item)--)
