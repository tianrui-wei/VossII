...
#define FOR_REC(mp, type, Tp)                                                  \
  for (ensure_pure_mgr(mp, __FILE__, __LINE__),                                \
       (mp)->cur_blk = (mp)->first_blk;                                        \
       (mp)->cur_blk != (pointer)NULL;                                         \
       (mp)->cur_blk = (pointer)FWD_BLK_PTR((mp)->cur_blk))                    \
    for ((mp)->cur_rec = 0, Tp = (type)((mp)->cur_blk + (2 * POINTER_SIZE));   \
         ((mp)->cur_blk == (mp)->last_blk) ? (mp)->cur_rec < (mp)->allocated   \
                                           : (mp)->cur_rec < (mp)->capacity;   \
         (mp)->cur_rec++,                                                      \
        Tp = (type)(((mp)->cur_blk) + ((2 * POINTER_SIZE) +                    \
                                       (((mp)->cur_rec) * ((mp)->rec_size)))))
