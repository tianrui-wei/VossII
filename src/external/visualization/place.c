...
static list_ptr merge_max_bb_rec(int cur_u_y, int cur_l_y, list_ptr upper,
                                 list_ptr lower);
static list_ptr merge_min_bb_rec(int cur_u_y, int cur_l_y, list_ptr upper,
                                 list_ptr lower);
void shift_drivers_fn(sch_draw_ptr tree, hash_record_ptr drivers,
                      hash_record_ptr repeats);
void shift_driver_over(sch_draw_ptr driver, int dist);
sch_draw_ptr find_driver(sch_draw_ptr repeat, hash_record_ptr drivers);
...
static void make_room_for_verticals(space_tree_ptr spt, sch_draw_ptr tree);
static void rename_loop_repeat_nodes(sch_draw_ptr tree, hash_record_ptr done);
static void break_loops_rec(sch_draw_ptr tree, hash_record_ptr seen,
                            hash_record_ptr new_nodes, hash_record_ptr done);
...
void dbg_bbox(string msg, bbox_ptr bp) {
  fprintf(stderr, "\n%s\n  Upper: ", msg);
...
int Move_driver_leftmost(sch_draw_ptr tree) {
  if (IS_REPEAT_ND(tree)) {
    ...
    for (sch_draw_list_ptr lp = tree->fanins; lp != NULL; lp = lp->next) {
      if (Move_driver_leftmost(lp->tree) != 0) {
        return 1;
      }
    }
    return 0;
...
static void increase_bin_num(sch_draw_ptr tree) {
  tree->bin_num++;
  for (sch_draw_list_ptr lp = tree->fanins; lp != NULL; lp = lp->next) {
...
void Remove_drive_repeat_overlaps(sch_draw_ptr tree) {
  if (IS_REPEAT_ND(tree)) {
    ...
    if (driver->bin_num == tree->bin_num) {
      ...
      fanin->next = NULL;
      sch_draw_ptr wire = New_sch(NODE, min_sep, 4, driver->x, driver->y,
                                  tree->name, pfn, fanin);
      fanin->tree = wire;
      sch_draw_rec tmp = *driver;
      *driver = *wire;
...
static bbox_ptr find_bbox(sch_draw_ptr tree) {
  ...
  for (int i = 0; i < COUNT_BUF(&bbox_buffer); i++) {
    ...
    if (current_bbox->upper == NULL || current_bbox->lower == NULL) {
      ...
      current_bbox->upper = Merge_min_bb(current_bbox->upper, lower_bb->upper);
      current_bbox->lower = Merge_max_bb(current_bbox->lower, lower_bb->lower);
  ...
  current_bbox->upper =
      Move_bb(-tree->width, -center_height, current_bbox->upper);
  current_bbox->lower =
      Move_bb(-tree->width, -center_height, current_bbox->lower);
  // Add parent
...
static sch_draw_ptr absolute_placement(int cur_x, int cur_y,
                                       sch_draw_ptr tree) {
  tree->x += cur_x;
  tree->y += cur_y;
  for (sch_draw_list_ptr ch = tree->fanins; ch != NULL; ch = ch->next) {
...
sch_draw_ptr Place(sch_draw_ptr parent) {
  ...
  if (upper == NULL) {
    ...
    if (lower == NULL) {
      ...
      if (upper->x > lower->x) {
        ...
        tmp->next = merge_min_bb_rec(upper->y, cur_l_y, upper->next, lower);
        return tmp;
      } else if (upper->x < lower->x) {
        list_ptr tmp = (list_ptr)new_rec(&list_rec_mgr);
        tmp->x = lower->x;
        ...
        tmp->next = merge_min_bb_rec(cur_u_y, lower->y, upper, lower->next);
        return tmp;
      } else {
        list_ptr tmp = (list_ptr)new_rec(&list_rec_mgr);
        tmp->x = lower->x;
        ...
        tmp->next =
            merge_min_bb_rec(upper->y, lower->y, upper->next, lower->next);
        return tmp;
      }
    }
  }
  ...
  if (upper == NULL) {
    ...
    if (lower == NULL) {
      ...
      if (upper->x > lower->x) {
        ...
        tmp->next = merge_max_bb_rec(upper->y, cur_l_y, upper->next, lower);
        return tmp;
      } else if (upper->x < lower->x) {
        list_ptr tmp = (list_ptr)new_rec(&list_rec_mgr);
        tmp->x = lower->x;
        ...
        tmp->next = merge_max_bb_rec(cur_u_y, lower->y, upper, lower->next);
        return tmp;
      } else {
        list_ptr tmp = (list_ptr)new_rec(&list_rec_mgr);
        tmp->x = lower->x;
        ...
        tmp->next =
            merge_max_bb_rec(upper->y, lower->y, upper->next, lower->next);
        return tmp;
      }
    }
  }
...
static void replace_rightmost_repeat(pointer key, pointer data) {
...
static void add_vertical_to_wire(pointer key, pointer data) {
  string name = (string)key;
...
static int int_cmp(const void *pi, const void *pj) {
  int *i = (int *)pi;
  int *j = (int *)pj;
  return (*j - *i);
}
...
static int translate_x_location(space_tree_ptr spt, int x) {
...
sch_draw_ptr find_origin(char *name, sch_draw_ptr tree) {
  ...
  if (IS_REPEAT_ND(tree)) {
    ...
    if (driver != NULL) {
      ...
      if (fb_name != NULL) {
        ...
        tree->pfn_type = Pfn2pfn_type(tree->pfn);
        sch_draw_ptr new_driver = (sch_draw_ptr)new_rec(&sch_draw_rec_mgr);
        *new_driver = *driver;
        ...
        repeat_to_loop_src->pfn_type = Pfn2pfn_type(repeat_to_loop_src->pfn);
    ...
    if (find_hash(done, (pointer)tree->name)) {
      return;
    }
