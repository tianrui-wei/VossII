...
sch_draw_ptr New_sch(sch_type type, int w, int h, int x, int y, char *name,
                     string pfn, sch_draw_list_ptr fanins) {
...
void print_network_rec(int indent, sch_draw_ptr tree) {
  if (tree == NULL) {
    return;
  }
  char *name = tree->name;
  ...
  *p = '\0';
  fprintf(stderr, "%*s%02d %s Type: %s : (%d,%d), w:%d h:%d PFN: %s\n", indent,
          "", count, name, type, x, y, w, h, pfn_buf);
  count++;
...
void print_fn(pointer key, pointer data) {
  char *k = (char *)key;
...
void print_fn2(pointer key, pointer data) {
  char *k = (char *)key;
  sch_draw_ptr t = (sch_draw_ptr)data;
...
void Print_fanins(hash_record_ptr hash) { scan_hash(hash, print_fn); }

/* Print_pfn prints all the pfn in the given hash table. */
void Print_pfn(hash_record_ptr hash) { scan_hash(hash, print_fn2); }

/* print_drivers_fn provides a function for printing drivers in a hash table */
void print_drivers_fn(pointer key, pointer data) {
  char *name = (char *)key;
...
void print_dists_fn(pointer key, pointer data) {
  char *name = (char *)key;
  long dist = (long)data;
...
void Print_dists(hash_record_ptr dists) { scan_hash(dists, print_dists_fn); }

/* Copy a network */
sch_draw_ptr copy_draw_tree(sch_draw_ptr tree) {
  if (tree == NULL) {
    return NULL;
  }
  sch_draw_ptr ans = (sch_draw_ptr)new_rec(&sch_draw_rec_mgr);
