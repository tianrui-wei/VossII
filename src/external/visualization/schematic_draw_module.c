...
int create_sch(ClientData clientData, Tcl_Interp *interp, int objc,
               Tcl_Obj *const objv[]) {
  (void)clientData;
  if (objc != 1) {
...
int add_sch_object(ClientData clientData, Tcl_Interp *interp, int objc,
                   Tcl_Obj *const objv[]) {
  (void)clientData;
  ...
  if ((height = PTR2INT(find_hash(&pfn2height, pfn))) == 0) {
    ...
    if (sscanf(res, "%d %d %d %d", &lx, &ly, &ux, &uy) != 4) {
      Tcl_AppendResult(interp, "Failed to extract width/height from: ", res,
                       NULL);
      return TCL_ERROR;
    }
  ...
  if (Tcl_ListObjLength(interp, fanins, &nbr_fanins) != TCL_OK) {
    Tcl_AppendResult(interp, "Invalid fanin list: Cannot determine length",
                     NULL);
  ...
  for (int i = 0; i < nbr_fanins; i++) {
    ...
    if (Tcl_ListObjIndex(interp, fanins, i, &opp) != TCL_OK) {
      char idx[10];
      sprintf(idx, "%d", i);
      Tcl_AppendResult(interp, "Cannot access element ", idx, "in fanins",
                       NULL);
...
static char *strip_dummy_prefixes(char *vec) {
restart:
...
static bool get_coords(Tcl_Interp *interp, Tcl_Obj *locs, pair_list_ptr *resp,
                       int *cntp) {
  int nbr_inputs;
...
static bool draw_pfn(Tcl_Interp *interp, char *cmd, pair_list_ptr *iresp,
                     int *nbr_inputsp, pair_list_ptr *oresp,
                     int *nbr_outputsp) {
...
static bool draw_schematics(Tcl_Interp *interp, char *canvas,
                            sch_draw_ptr tree) {
  ...
  int nbr_inputs;
  int nbr_outputs;
  if (draw_pfn(interp, cmd_buf, &my_inps, &nbr_inputs, &my_outs,
               &nbr_outputs) != TRUE) {
    return FALSE;
  }
  if (tree->type == LEAF) {
  ...
  if (IS_OUTPUT(tree)) {
    ...
    for (sch_draw_list_ptr lp = tree->fanins; lp != NULL; lp = lp->next) {
      ...
      sprintf(cmd_buf, "draw_output1 [fl_tag2vec %s %s] %s %s %d %d", canvas,
              strip_dummy_prefixes(driver_name), canvas, driver_tag, tree->x,
              driver_y);
      if (Tcl_Eval(interp, cmd_buf) != TCL_OK) {
        return FALSE;
      }
      sprintf(cmd_buf, "out_connect %s %s %d %d %d %d %d %d %s", canvas,
              driver_tag, driver_x, driver_y, tree->x, driver_y, i, nbr_inputs,
              tag);
      if (Tcl_Eval(interp, cmd_buf) != TCL_OK) {
        return FALSE;
      }
    ...
    for (sch_draw_list_ptr lp = tree->fanins; lp != NULL; lp = lp->next) {
      ...
      int driver_x;
      int driver_y;
      if (IS_WIRE(lp->tree) && strcmp(lp->tree->pfn, "draw_wire 0") == 0 &&
          lp->tree->fanins != NULL) {
      ...
      if (!my_inps) {
        int cnt = 0;
        for (sch_draw_list_ptr lp2 = tree->fanins; lp2 != NULL;
             lp2 = lp2->next) {
          cnt++;
        }
        char tmp[30];
        sprintf(tmp, ". Got %d\n\n", cnt);
        Tcl_AppendResult(interp, "\n\nWrong number of inputs to ", tree->pfn,
                         tmp, NULL);
        return FALSE;
      }
      int driven_x = my_inps->x;
      ...
      if (driven_x > driver_x) {
        sprintf(cmd_buf, "connect %s %s %d %d %d %d %d %d %s", canvas,
                driver_tag, driver_x, driver_y, driven_x, driven_y, i,
                nbr_inputs, tag);
        if (Tcl_Eval(interp, cmd_buf) != TCL_OK) {
          return FALSE;
        }
...
int draw_network(ClientData clientData, Tcl_Interp *interp, int objc,
                 Tcl_Obj *const objv[]) {
  (void)clientData;
  // draw_network canvas tree
  if (objc != 3) {
    Tcl_WrongNumArgs(interp, objc, objv, "draw_network canvas network");
...
int Schematic_draw_module_Init(Tcl_Interp *interp) {
  Tcl_CreateObjCommand(interp, "create_sch", create_sch, (ClientData)NULL,
                       NULL);
  Tcl_CreateObjCommand(interp, "add_sch_object", add_sch_object,
                       (ClientData)NULL, NULL);
  Tcl_CreateObjCommand(interp, "draw_network", draw_network, (ClientData)NULL,
                       NULL);
