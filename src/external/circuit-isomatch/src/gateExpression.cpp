...
sign_t ExpressionConst::sign() const { return opcst_numconst(val); }

sign_t ExpressionLongConst::sign() const {
  uint32_t hashed = 0;
  for (size_t pos = 0; pos < val.size(); pos += 16) {
    uint32_t cVal = 0;
    for (int subpos = 0; pos + subpos < val.size() && subpos < 16; ++subpos) {
...
sign_t ExpressionVar::sign() const { return opcst_wireid(id); }
...
sign_t ExpressionUnOp::sign() const { return cstOf(op)(expr->sign()); }
...
bool ExpressionLongConst::innerEqual(const ExpressionBase &oth) const {
  const ExpressionLongConst &o = dynamic_cast<const ExpressionLongConst &>(oth);
  return val == o.val;
}
...
bool ExpressionUnOpCst::innerEqual(const ExpressionBase &oth) const {
  ...
  return op == o.op && val == o.val && expr->equals(*o.expr);
}
