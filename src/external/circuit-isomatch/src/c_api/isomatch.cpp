...
template <typename Circ> static Circ *circuitOfHandle(circuit_handle handle) {
...
static std::set<CircuitTree *>::iterator
freeCircuitPtr(std::set<CircuitTree *>::iterator circuit) {
  delete *circuit;
...
isom_rc isom_last_error() { return lastError; }
...
int build_comb_add_output(circuit_handle self, wire_handle wire,
  ...
  try {
    CircuitComb *circ = circuitOfHandle<CircuitComb>(self);
    circ->addOutput(exprOfHandle(expr), wireOfHandle(wire, circ->ancestor()));
    return ISOM_RC_OK;
  } catch (const IsomError &e) {
    return handleError(e);
  }
...
circuit_handle build_delay(circuit_handle parent, wire_handle input,
  ...
  try {
    CircuitGroup *par = circuitOfHandle<CircuitGroup>(parent);
    CircuitDelay *delay =
        new CircuitDelay(wireOfHandle(input, par), wireOfHandle(output, par));
...
int build_group_add_input(circuit_handle self, wire_handle actual,
  ...
  try {
    ...
    if (group->ancestor() != nullptr) {
      group->addInput(IOPin(wireOfHandle(formal, group->ancestor()),
                            wireOfHandle(actual, group), group));
    } else
...
int build_group_add_output(circuit_handle self, wire_handle actual,
  ...
  try {
    CircuitGroup *group = circuitOfHandle<CircuitGroup>(self);
    if (group->ancestor() != nullptr) {
      group->addOutput(IOPin(wireOfHandle(formal, group->ancestor()),
                             wireOfHandle(actual, group), group));
    } else
...
circuit_handle build_tristate(circuit_handle parent, wire_handle from,
  ...
  try {
    CircuitGroup *par = circuitOfHandle<CircuitGroup>(parent);
    CircuitTristate *out =
        new CircuitTristate(wireOfHandle(from, par), wireOfHandle(to, par),
...
expr_handle build_expr_binop(enum isom_expr_binop op, expr_handle left,
                             expr_handle right) {
  try {
    return new ExpressionBinOp(exprOfHandle(left), exprOfHandle(right),
...
expr_handle build_expr_unop(enum isom_expr_unop op, expr_handle expr) {
  try {
    return new ExpressionUnOp(exprOfHandle(expr),
...
expr_handle build_expr_unop_cst(enum isom_expr_unop_cst op, int param,
                                expr_handle expr) {
  try {
    return new ExpressionUnOpCst(exprOfHandle(expr), param,
...
match_results *subcircuit_find(circuit_handle needle, circuit_handle haystack) {
  try {
    std::vector<MatchResult> res =
        matchSubcircuit(circuitOfHandle<CircuitGroup>(needle),
...
void isom_clear_marks() { markSweepState.marks.clear(); }
