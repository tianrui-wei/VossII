...
IOPin::IOPin(WireId *formal, WireId *actual, CircuitGroup *group)
...
void IOPin::link() { _formal->connect(this, _actual); }
...
size_t CircuitGroup::inputCount() const { return grpInputs.size(); }

size_t CircuitGroup::outputCount() const { return grpOutputs.size(); }
...
void CircuitGroup::toDot(std::basic_ostream<char> &out, int indent) {
  ...
  if (ancestor() == NULL) {
    // Root group
    dotPrint::indent(out, indent) << "digraph " << thisCirc << " {\n";
  } else {
    dotPrint::indent(out, indent) << "subgraph cluster_" << thisCirc << " {\n";
  }
  indent += 2;
  ...
  for (auto wire : wireManager()->wires()) {
    dotPrint::indent(out, indent)
        << wire->uniqueName() << " [shape=plain, label=" << wire->uniqueName()
        << "]" << '\n';
  }
  ...
  for (auto inPin : grpInputs) {
    if (inPin->formal() != NULL) {
      dotPrint::indent(out, indent)
          ...
          << inPin->formal()->uniqueName() << " [arrowhead=none]" << '\n';
    }
  }
  for (auto outPin : grpOutputs) {
    if (outPin->formal() != NULL)
      dotPrint::indent(out, indent)
          ...
          << outPin->formal()->uniqueName() << " [arrowhead=none]" << '\n';
  }
...
sign_t CircuitGroup::innerSignature() const {
  ...
  return signatureConstants::opcst_leaftype(
      ((circType() << 16) | (grpInputs.size() << 8) | (grpOutputs.size())) +
      subsigs);
}
