...
public:
  ...
  struct PinConnection {
    PinConnection(IOPin *pin, WireId *other) : pin(pin), other(other) {}
    IOPin *pin;
    WireId *other;
  };
  ...
  public:
    ...
    CircIterator(const CircIterator &oth) { operator=(oth); }
    ...
    bool operator==(const CircIterator &oth) const {
      return parent == oth.parent && isCircIter == oth.isCircIter &&
             ((isCircIter && circIter == oth.circIter) ||
              (!isCircIter && pinIter == oth.pinIter));
    }
    bool operator!=(const CircIterator &oth) const { return !operator==(oth); }
...
private:
  void walkConnected(std::unordered_set<CircuitTree *> &curConnected,
                     std::unordered_set<WireId> &seenWires, WireId *curWire);
