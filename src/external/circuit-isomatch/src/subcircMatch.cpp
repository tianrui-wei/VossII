...
struct Vertice {
  ...
  enum Type { VertWire, VertCirc } type;
...
void findIn(vector<MatchResult> &results, CircuitGroup *needle,
            CircuitGroup *haystack);

// ========================================================================

sign_t localSign(CircuitTree *circ) { return circ->sign(0); }
...
public:
  ...
  bool fitFor(WireId *role) {
    try {
      ...
      for (auto circ = role->adjacent_begin(); circ != role->adjacent_end();
           ++circ) {
        bool isInput = true;
        int pinPos = 0;
        for (auto circWire = (*circ)->io_begin(); circWire != (*circ)->io_end();
             ++circWire, ++pinPos) {
...
private:
  struct ConnType {
    ConnType(sign_t inSig, bool in, int pin) : inSig(inSig), in(in), pin(pin) {}
    bool operator==(const ConnType &e) {
      return inSig == e.inSig && in == e.in && pin == e.pin;
    }
    bool operator<(const ConnType &e) const {
      return (inSig < e.inSig) || (inSig == e.inSig && in < e.in) ||
...
bool isMatchFit(CircuitTree *match, CircuitTree *needleMatch,
  ...
  auto wire = match->io_begin(), role = needleMatch->io_begin();

  for (; wire != match->io_end() && role != needleMatch->io_end();
       ++wire, ++role) {
  ...
  if (wire != match->io_end() || role != needleMatch->io_end()) {
    return false;
  }
...
void dumpPerm(const PermMatrix &permMatrix, const FullMapping &mapping) {
  ...
  FIND_DEBUG("\n");
#endif
  for (size_t needleId = 0; needleId < mapping.needle.vertices.size();
       ++needleId) {
    FIND_DEBUG(mapping.needle.vertices[needleId].type == Vertice::VertWire
                   ? "W "
                   : "C ");
...
bool isActualMatch(const PermMatrix &perm, const FullMapping &mapping) {
  ...
  for (size_t needlePos = 0; needlePos < mapping.needle.vertices.size();
       ++needlePos) {
...
void mapVertices(CircuitGroup *group, VerticeMapping &mapping) {
  ...
  sort(wires.begin(), wires.end(), [](WireId *&e1, WireId *&e2) {
...
void buildAdjacency(CircuitGroup *group, const VerticeMapping &mapping,
                    AdjacencyMatr &adjacency) {
  for (const auto &wire : group->wireManager()->wires()) {
    size_t wireId = mapping.wireId.at(wire);
    for (auto circ = wire->adjacent_begin(); circ != wire->adjacent_end();
         ++circ) {
...
WireId *mappedWire(WireId *of, const FullMapping &mapping,
...
MatchResult buildMatchResult(const CircuitGroup *fullNeedle,
                             const FullMapping &mapping, const PermMatrix &perm,
...
bool ullmannRefine(PermMatrix &matr, const FullMapping &mapping,
      ...
      for (size_t hayId = 0; hayId < mapping.haystack.vertices.size();
        ...
        if (needleVert.type == Vertice::VertCirc) {
          ...
          for (auto needleNeigh = needle->io_begin();
               needleNeigh != needle->io_end(); ++needleNeigh) {
            size_t neighId = mapping.needle.wireId.at(*needleNeigh);
          ...
          for (auto needleNeigh = needle->adjacent_begin();
               needleNeigh != needle->adjacent_end(); ++needleNeigh) {
            size_t neighId = mapping.needle.circId.at(*needleNeigh);
            if (!(matr[neighId] & hayAdj[hayId]).any()) {
              changed = true;
              matr[needleId][hayId].reset();
              break;
...
void ullmannFindDepth(size_t depth, DynBitset &freeHayVert,
                      vector<MatchResult> &results, PermMatrix &matr,
                      DynBitset &toUnmapHaystack, const FullMapping &mapping,
                      const AdjacencyMatr &hayAdj,
    ...
    if (ullmannRefine(matr, mapping, hayAdj)) {
      if (depth == mapping.needle.vertices.size() - 1) {
        if (isActualMatch(matr, mapping)) {
          results.push_back(
              buildMatchResult(fullNeedle, mapping, matr, toUnmapCur));
        }
      } else {
        freeHayVert[hayId].reset();
        FIND_DEBUG(">> Picking %lu at %lu\n", hayId, depth);
        ullmannFindDepth(depth + 1, freeHayVert, results, matr, toUnmapCur,
...
void ullmannFind(vector<MatchResult> &results, PermMatrix &matr,
                 const FullMapping &mapping, const AdjacencyMatr &hayAdj,
                 const CircuitGroup *fullNeedle,
...
void findIn(vector<MatchResult> &results, CircuitGroup *needle,
  ...
  for (const auto &needleMatch : singleMatches) {
    ...
    for (const auto &match : needleMatch.second) {
      int pin = 0;
      for (auto inp = match->inp_begin(); inp != match->inp_end(); ++inp) {
        wireFit[*inp].connected(cSig, true, pin);
        ++pin;
      }

      pin = 0;
      for (auto out = match->out_begin(); out != match->out_end(); ++out) {
  ...
  AdjacencyMatr hayAdj(mapping.haystack.vertices.size(),
  ...
  PermMatrix permMatrix(mapping.needle.vertices.size(),
                        DynBitset(mapping.haystack.vertices.size()));
  ...
  for (const auto &needleWire : needle->wireManager()->wires()) {
    if (needleWire->connectedCirc().size() == 0 &&
      ...
      LOG_WARNING("Dangling wire %s in needle", needleWire->name().c_str());
    }
  }
