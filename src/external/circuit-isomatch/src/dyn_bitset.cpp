...
DynBitset::Reference &
DynBitset::Reference::operator=(const DynBitset::Reference &e) {
  if (e)
    set();
  else
    reset();
...
DynBitset::DynBitset(size_t size) : size_(size) {
  data = new Word[nbWords()];
  reset();
}

DynBitset::DynBitset(const DynBitset &oth) : size_(oth.size_) {
  data = new Word[nbWords()];
  memcpy(data, oth.data, nbWords() * sizeof(Word));
  // We assume `oth`'s last bits are 0s, as it should be
}

DynBitset::DynBitset(size_t size, Word *words) : size_(size), data(words) {}

DynBitset::DynBitset(DynBitset &&oth) : size_(oth.size_), data(oth.data) {
  // This could seriously mess things up if called improperly
  oth.data = nullptr;
}

DynBitset::~DynBitset() { delete[] data; }
