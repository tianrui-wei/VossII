...
protected:
  ...
  public:
    ...
    InnerIoIter(const InnerIoIter &it) : ptr(it.ptr), circ(it.circ) {}
...
public:
  IoIter inp_begin() const {
    return IoIter(new InnerIoIter(this, gateInputs.begin()));
  }
  IoIter out_begin() const {
    return IoIter(new InnerIoIter(this, gateOutputs.begin()));
  }
  IoIter out_end() const {
    return IoIter(new InnerIoIter(this, gateOutputs.end()));
  }
  // ========= END I/O ITERATOR =========================================
  ...
  /** Gate's expressions */
  const std::vector<ExpressionBase *> &expressions() const { return gateExprs; }
