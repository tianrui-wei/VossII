...
void CircuitAssert::InnerIoIter::operator++() { ++ptr; }

CircuitAssert::CircuitAssert(const std::string &name, ExpressionBase *expr)
    : gateName(name), gateExpr(expr) {
  expr->addRef();
}

CircuitAssert::~CircuitAssert() { gateExpr->deleteSelf(); }
...
sign_t CircuitAssert::innerSignature() const {
  return signatureConstants::opcst_leaftype(
      ((circType() << 16) | (gateInputs.size() << 8)) + gateExpr->sign());
}
...
size_t CircuitAssert::inputCount() const { return gateInputs.size(); }

size_t CircuitAssert::outputCount() const { return 0; }
