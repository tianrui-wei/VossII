...
CircuitComb::InnerIoIter::InnerIoIter(const CircuitComb *circ,
                                      CircuitComb::InnerIoIter::LowIter lowIter)
...
sign_t CircuitComb::innerSignature() const {
  ...
  return signatureConstants::opcst_leaftype(
      ((circType() << 16) | (gateInputs.size() << 8) | (gateOutputs.size())) ^
      exprsSum);
}
...
size_t CircuitComb::inputCount() const { return gateInputs.size(); }

size_t CircuitComb::outputCount() const { return gateOutputs.size(); }
