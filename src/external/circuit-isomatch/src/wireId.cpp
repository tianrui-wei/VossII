...
bool WireId::operator!=(WireId &oth) { return !operator==(oth); }
bool WireId::operator!=(const WireId &oth) const { return !operator==(oth); }
...
void WireId::connect(CircuitTree *circ) { inner()->connected.push_back(circ); }
...
void WireId::merge(WireId *other) {
  ...
  kept->inner()->connected.insert(kept->inner()->connected.end(),
                                  merged->inner()->connected.begin(),
                                  merged->inner()->connected.end());
  kept->inner()->connectedPins.insert(kept->inner()->connectedPins.end(),
