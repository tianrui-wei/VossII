...
bool Permutation::next() {
  while (!next_permutation(nextChange->begin(), nextChange->end())) {
...
void splitOnSig(const vector<CircuitTree *> circuits, SigSplit &splitted,
                std::vector<sign_t> &signatures, int maxPermutations,
                int accuracy) {
  splitted.clear();
  signatures.clear();
...
bool equalWithPermutation(const SigSplit &leftSplit, const SigSplit &rightSplit,
  ...
  for (size_t pos = 0; pos < leftSplit.size(); ++pos) {
    ...
    for (size_t circId = 0; circId < leftSplit[pos].size(); ++circId) {
      if (!leftSplit[pos][circId]->equals(rightSplit[pos][curPerm[circId]])) {
  ...
  for (size_t pos = 0; pos < leftSplit.size(); ++pos) {
    ...
    for (size_t circId = 0; circId < leftSplit[pos].size(); ++circId) {
      ...
      CircuitTree::IoIter lWire = left->io_begin(), rWire = right->io_begin();
...
bool equal(CircuitGroup *left, CircuitGroup *right) {
  ...
  const int BASE_PRECISION = 2, MAX_PRECISION = 15, MAX_PERMUTATIONS = 4;

  EQ_DEBUG("\t> Entering %s <\n", left->name().c_str());

  groupEquality::SigSplit sigSplit[2];

  for (int precision = BASE_PRECISION; precision <= MAX_PRECISION;
       ++precision) {
    int maxPermutations = (precision == MAX_PRECISION) ? -1 : MAX_PERMUTATIONS;
    ...
    do {
      if (groupEquality::equalWithPermutation(leftSplit, rightSplit, perm)) {
