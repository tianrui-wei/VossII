...
sign_t CircuitTristate::innerSignature() const {
  return signatureConstants::opcst_leaftype((circType() << 16) + (1 << 8) + 1);
}
...
size_t CircuitTristate::inputCount() const { return 2; }

size_t CircuitTristate::outputCount() const { return 1; }
...
void CircuitTristate::toDot(std::basic_ostream<char> &out, int indent) {
  ...
  dotPrint::inWire(out, thisCirc, wireInput->uniqueName(), "headport=w");
  dotPrint::indent(out, indent);
  dotPrint::inWire(out, thisCirc, wireEnable->uniqueName(), "headport=n");
  dotPrint::indent(out, indent);
  dotPrint::outWire(out, thisCirc, wireOutput->uniqueName(), "headport=e");
}
