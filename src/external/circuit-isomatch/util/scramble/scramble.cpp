...
ExpressionLongConst *
scrambleExpressionLongConst(const ExpressionLongConst *expr);
...
ExpressionUnOpCst *scrambleExpressionUnOpCst(const ExpressionUnOpCst *expr);
...
CircuitDelay *scrambleDelay(CircuitDelay *orig, WireManager *manager,
                            NameMap &nameMap);
CircuitTristate *scrambleTristate(CircuitTristate *orig, WireManager *manager,
                                  NameMap &nameMap);
...
ExpressionBinOp *scrambleExpressionBinOp(const ExpressionBinOp *expr) {
  return new ExpressionBinOp(scrambleExpression(expr->left),
                             scrambleExpression(expr->right), expr->op);
}

ExpressionUnOp *scrambleExpressionUnOp(const ExpressionUnOp *expr) {
  return new ExpressionUnOp(scrambleExpression(expr->expr), expr->op);
}

ExpressionUnOpCst *scrambleExpressionUnOpCst(const ExpressionUnOpCst *expr) {
  return new ExpressionUnOpCst(scrambleExpression(expr->expr), expr->val,
                               expr->op);
}

ExpressionSlice *scrambleExpressionSlice(const ExpressionSlice *expr) {
  return new ExpressionSlice(scrambleExpression(expr->expr), expr->beg,
                             expr->end);
}

ExpressionMerge *scrambleExpressionMerge(const ExpressionMerge *expr) {
  return new ExpressionMerge(scrambleExpression(expr->left),
...
ExpressionBase *scrambleExpression(const ExpressionBase *expr) {
  switch (expr->type) {
  case expr::ExprVar:
    return scrambleExpressionVar(dynamic_cast<const ExpressionVar *>(expr));
  case expr::ExprConst:
    return scrambleExpressionConst(dynamic_cast<const ExpressionConst *>(expr));
  case expr::ExprLongConst:
    return scrambleExpressionLongConst(
        dynamic_cast<const ExpressionLongConst *>(expr));
  case expr::ExprBinOp:
    return scrambleExpressionBinOp(dynamic_cast<const ExpressionBinOp *>(expr));
  case expr::ExprUnOp:
    return scrambleExpressionUnOp(dynamic_cast<const ExpressionUnOp *>(expr));
  case expr::ExprUnOpCst:
    return scrambleExpressionUnOpCst(
        dynamic_cast<const ExpressionUnOpCst *>(expr));
  case expr::ExprSlice:
    return scrambleExpressionSlice(dynamic_cast<const ExpressionSlice *>(expr));
  case expr::ExprMerge:
    return scrambleExpressionMerge(dynamic_cast<const ExpressionMerge *>(expr));
  }
...
CircuitGroup *scrambleGroup(CircuitGroup *orig, WireManager *manager,
                            NameMap &nameMap) {
  CircuitGroup *outGrp =
      new CircuitGroup(randName() + string("_") + orig->name());
  ...
  for (const auto &inp : orig->getInputs()) {
    if (manager == nullptr || inp->formal() == nullptr)
      ...
      outGrp->addInput(IOPin(wire(inp->formal(), manager, nameMap),
                             wire(inp->actual(), innerManager, innerNameMap),
                             outGrp));
  }
  for (const auto &out : orig->getOutputs()) {
    if (manager == nullptr || out->formal() == nullptr)
      ...
      outGrp->addOutput(IOPin(wire(out->formal(), manager, nameMap),
                              wire(out->actual(), innerManager, innerNameMap),
                              outGrp));
  }
  ...
  // Recreate all the children in a randomized order
  {
    ...
    for (const auto &curId : walkPerm) {
      ...
      outGrp->addChild(recScramble(child, innerManager, innerNameMap));
    }
  }
...
CircuitComb *scrambleComb(CircuitComb *orig, WireManager *manager,
  ...
  for (size_t outPos = 0;
       outPos < orig->outputs().size() && outPos < orig->expressions().size();
       ++outPos) {
    outGate->addOutput(scrambleExpression(orig->expressions()[outPos]),
...
CircuitDelay *scrambleDelay(CircuitDelay *orig, WireManager *manager,
                            NameMap &nameMap) {
  return new CircuitDelay(wire(orig->input()->name(), manager, nameMap),
                          wire(orig->output()->name(), manager, nameMap));
}

CircuitTristate *scrambleTristate(CircuitTristate *orig, WireManager *manager,
                                  NameMap &nameMap) {
  return new CircuitTristate(wire(orig->input()->name(), manager, nameMap),
                             wire(orig->output()->name(), manager, nameMap),
...
CircuitAssert *scrambleAssert(CircuitAssert *orig, WireManager *manager,
                              NameMap &nameMap) {
  CircuitAssert *out =
      new CircuitAssert(randName() + string("_") + orig->name(),
...
CircuitTree *recScramble(CircuitTree *orig, WireManager *manager,
  ...
  case CircuitTree::CIRC_GROUP:
    return scrambleGroup(dynamic_cast<CircuitGroup *>(orig), manager, nameMap);
  case CircuitTree::CIRC_COMB:
    return scrambleComb(dynamic_cast<CircuitComb *>(orig), manager, nameMap);
  case CircuitTree::CIRC_DELAY:
    return scrambleDelay(dynamic_cast<CircuitDelay *>(orig), manager, nameMap);
  case CircuitTree::CIRC_TRI:
    return scrambleTristate(dynamic_cast<CircuitTristate *>(orig), manager,
                            nameMap);
  case CircuitTree::CIRC_ASSERT:
    return scrambleAssert(dynamic_cast<CircuitAssert *>(orig), manager,
                          nameMap);
  }
...
CircuitGroup *scrambleCircuit(CircuitGroup *original) {
  ...
  return dynamic_cast<CircuitGroup *>(
      recScramble(static_cast<CircuitTree *>(original), nullptr, nameMap));
}
