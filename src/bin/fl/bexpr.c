...
bexpr BE_One() { return (BE_TRUE); }

/* BE_Zero -- Return the bexpr bexpr for the constant 0 */
bexpr BE_Zero() { return (BE_FALSE); }
...
void BE_Mark(bexpr f) { mark(f); }

static void insert_node_in_signature_tbl(bexpr p) {
  bexpr cur;
...
bexpr BE_Not(bexpr f1) { return (BE_NOT(f1)); }

bexpr BE_Nor(bexpr f1, bexpr f2) { return (be_and(BE_NOT(f1), BE_NOT(f2))); }

bexpr BE_Nand(bexpr f1, bexpr f2) { return (BE_NOT(be_and(f1, f2))); }

bexpr BE_And(bexpr f1, bexpr f2) { return (be_and(f1, f2)); }
...
bool BE_Equal(bexpr f1, bexpr f2) { return (f1 == f2); }
...
void BEP(bexpr f) { BE_Print(err_fp, f); }
...
bool Save_bexprs(string filename, buffer *roots) {
  ...
  int *ip;
  FOR_BUF(&res_buf, int, ip) { fprintf(fp, "r %+d\n", *ip); }
  fclose(fp);
  free_buf(&res_buf);
...
int SHA256_bexpr(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha, bexpr f) {
  int res;
  if (BE_IS_FALSE(f))
    return -1;
  if (BE_IS_TRUE(f))
...
static void bT(g_ptr redex) { MAKE_REDEX_BEXPR(redex, BE_One()); }

static void bF(g_ptr redex) { MAKE_REDEX_BEXPR(redex, BE_Zero()); }

static void bvariable(g_ptr redex) {
...
static void bexpr2bdd(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr bf;
...
static void bsubstitute(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void bool2bexpr(g_ptr redex) {
  g_ptr fs;
...
static void bexpr2str(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void bdepends(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void bget_model(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
  ...
  if (COUNT_BUF(&assumptions) == 0) {
    ...
    SET_TYPE(redex, CONS_ND);
    SET_CONS_HD(redex, Make_CONS_ND(Make_STRING_leaf(wastrsave(&strings, "")),
  ...
  int res = Find_model((int *)START_BUF(&assumptions), COUNT_BUF(&assumptions),
                       time_limit);
  ...
  case 1: {
    ...
    g_ptr tail = redex;
    bexpr *bep;
    qsort(START_BUF(&bvar_sat_buf), COUNT_BUF(&bvar_sat_buf), sizeof(bexpr),
          be_subs_cmp);
...
static void bexpr_save(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void bexpr_load(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  buffer results;
...
static void bget_type(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
static void bget_arguments(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  bexpr be = GET_BEXPR(arg1);
...
static void bget_variable(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  bexpr be = GET_BEXPR(arg1);
...
static void bget_literal(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  bexpr be = GET_BEXPR(arg1);
  if (BE_IS_CONSTANT(be)) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("bget_sat_literal on a constant bexpr"));
    return;
  }
  bool neg = BE_IS_NEG(be);
  bexpr b = BE_POS(be);
...
static void gen_bNOT(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
static void gen_bOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void gen_bAND(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void bITE(g_ptr redex) {
  ...
  } else if (BE_IS_FALSE(current_bexpr_cond)) {
...
static void bEqual(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void bev2num(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
void Bexpr_Install_Functions() {
  ...
  Add_ExtAPI_Function("bvariable", "1", FALSE,
                      GLmake_arrow(GLmake_string(), GLmake_bexpr()), bvariable);

  Add_ExtAPI_Function("bNOT", "1", FALSE,
                      GLmake_arrow(GLmake_bexpr(), GLmake_bexpr()), bNOT);
  ...
  Add_ExtAPI_Function("prim_bexpr2bdd", "1", FALSE,
                      GLmake_arrow(GLmake_bexpr(), GLmake_bool()), bexpr2bdd);
  ...
  Add_ExtAPI_Function(
      "bsubstitute", "11", FALSE,
      GLmake_arrow(GLmake_list(GLmake_tuple(GLmake_string(), GLmake_bexpr())),
                   GLmake_arrow(tv1, tv1)),
      bsubstitute);

  Add_ExtAPI_Function(
      "bexpr2str", "11", FALSE,
      GLmake_arrow(GLmake_int(), GLmake_arrow(GLmake_bexpr(), GLmake_string())),
      bexpr2str);
  ...
  Add_ExtAPI_Function(
      "bget_model", "11", FALSE,
      GLmake_arrow(GLmake_list(GLmake_bexpr()),
  ...
  Add_ExtAPI_Function(
      "bexpr_load", "1", FALSE,
      GLmake_arrow(GLmake_string(), GLmake_list(GLmake_bexpr())), bexpr_load);

  Add_ExtAPI_Function("bget_type", "1", FALSE,
                      GLmake_arrow(GLmake_bexpr(), GLmake_string()), bget_type);

  Add_ExtAPI_Function(
      "bget_arguments", "1", FALSE,
      GLmake_arrow(GLmake_bexpr(),
  ...
  Add_ExtAPI_Function("bget_sat_literal", "1", FALSE,
                      GLmake_arrow(GLmake_bexpr(), GLmake_int()), bget_literal);

  Add_ExtAPI_Function(
      "bITE", "1--", FALSE,
      GLmake_arrow(GLmake_bexpr(), GLmake_arrow(tv1, GLmake_arrow(tv1, tv1))),
      bITE);

  Add_ExtAPI_Function("gen_bNOT", "1", FALSE, GLmake_arrow(tv1, tv1), gen_bNOT);

  Add_ExtAPI_Function("gen_bOR", "11", FALSE,
                      GLmake_arrow(tv1, GLmake_arrow(tv1, tv1)), gen_bOR);

  Add_ExtAPI_Function("gen_bAND", "11", FALSE,
                      GLmake_arrow(tv1, GLmake_arrow(tv1, tv1)), gen_bAND);
  ...
  Add_ExtAPI_Function(
      "bool2bexpr", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_bool()), GLmake_list(GLmake_bexpr())),
      bool2bexpr);
}
...
static unsigned int be_hash(pointer p, unsigned int n) {
  ...
  case BE_AND:
    return ((241 * ((ui)BE_GET_LEFT(be)) + 503 * ((ui)BE_GET_RIGHT(be))) % n);
  default:
...
static bool be_eq(pointer p1, pointer p2) {
  bexpr b1 = (bexpr)p1;
  bexpr b2 = (bexpr)p2;
  if (BE_GET_TYPE(b1) != BE_GET_TYPE(b2))
    return FALSE;
  if (BE_GET_TYPE(b1) == BE_ONE)
    return TRUE;
  if (BE_IS_VAR(b1)) {
    return (STREQ(BE_GET_VAR(b1), BE_GET_VAR(b2)));
  } else {
    if (BE_GET_LEFT(b1) != BE_GET_LEFT(b2))
      return FALSE;
    if (BE_GET_RIGHT(b1) != BE_GET_RIGHT(b2))
      return FALSE;
    return TRUE;
  }
}
  ...
  case BE_AND: {
    ...
    for (int i = 0; i < NBR_SIGNATURES; i++) {
      p->sig[i] = (ui)((long int)mk_prandom(&seed) +
                       3259 * ((long int)pl->sig[i]) + (long int)pr->sig[i]);
    }
...
static g_ptr g_be_or(g_ptr l, g_ptr r) {
  ...
  if (!IS_BEXPR(l)) {
    if (Is_equal(l, r, TRUE) != B_One()) {
      ...
      string m = wastrsave(&strings, "gen_bOR over non-matching structures");
