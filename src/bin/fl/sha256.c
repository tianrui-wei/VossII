...
static void put_bigendian(void *target, unsigned long long value,
                          size_t bytes) {
...
static unsigned long long get_bigendian(const void *target, size_t bytes) {
...
static unsigned long Sigma0(unsigned long x) {
  unsigned long mx = x & 0xFFFFFFFFUL;
  return ((((mx >> 2) | (mx << 30)) ^ ((mx >> 13) | (mx << 19)) ^
           ((mx >> 22) | (mx << 10))) &
          0xFFFFFFFFUL);
}

static unsigned long Sigma1(unsigned long x) {
  unsigned long mx = x & 0xFFFFFFFFUL;
  return ((((mx >> 6) | (mx << 26)) ^ ((mx >> 11) | (mx << 21)) ^
           ((mx >> 25) | (mx << 7))) &
          0xFFFFFFFFUL);
}
...
static unsigned long Gamma0(unsigned long x) {
  unsigned long mx = x & 0xFFFFFFFFUL;
  return ((((mx >> 7) | (mx << 25)) ^ ((mx >> 18) | (mx << 14)) ^ (mx >> 3)) &
          0xFFFFFFFFUL);
}

static unsigned long Gamma1(unsigned long x) {
  unsigned long mx = x & 0xFFFFFFFFUL;
  return ((((mx >> 17) | (mx << 15)) ^ ((mx >> 19) | (mx << 13)) ^ (mx >> 10)) &
          0xFFFFFFFFUL);
}

static void sha256_compress(SHA256_ptr ctx, const void *buf) {
...
static void SHA256_init(SHA256_ptr ctx) {
  ctx->Nl = 0;
  ctx->Nh = 0;
  ctx->num = 0;
...
static void SHA256_update(SHA256_ptr ctx, const void *src, unsigned int count) {
 ...
 */
static void SHA256_final(unsigned char *digest, SHA256_ptr ctx) {
  unsigned int i;
...
static void sha256_signature(g_ptr redex) {
...
void SHA256_Install_Functions() {
  ...
  Add_ExtAPI_Function("sha256_signature", "1", FALSE,
                      GLmake_arrow(tv, GLmake_string()), sha256_signature);
}
...
void End_SHA256(SHA256_ptr ctx) {
  ctx->in_use = FALSE;
...
static void do_printf(SHA256_ptr ctx, const string format, va_list arg) {
...
int SHA256_traverse_graph(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                          g_ptr node) {
  int res, lres, rres;
  string s;
  if (node == NULL)
  ...
  *g_cntp = res + 1;
  ...
  case LAMBDA_ND:
    rres = SHA256_traverse_graph(g_cntp, g_tblp, sha, GET_LAMBDA_BODY(node));
    SHA256_printf(sha, "%d=\\%s.%d\n", res, GET_LAMBDA_VAR(node), rres);
    return res;
  case APPLY_ND:
    lres = SHA256_traverse_graph(g_cntp, g_tblp, sha, GET_APPLY_LEFT(node));
    rres = SHA256_traverse_graph(g_cntp, g_tblp, sha, GET_APPLY_RIGHT(node));
    ...
    case PRIM_FN: {
      ...
      case P_EXTAPI_FN:
        SHA256_printf(sha, "%d=EFN(%s)\n", res,
                      Get_ExtAPI_Function_Name(GET_EXTAPI_FN(node)));
        return res;
      case P_SSCANF:
      ...
      case P_REF_VAR: {
        ...
        lres = SHA256_traverse_graph(g_cntp, g_tblp, sha, Get_RefVar(r));
        SHA256_printf(sha, "%d=REF %d\n", res, r);
        return res;
      }
      default:
        SHA256_printf(sha, "%d=PFN %s\n", res, Get_pfn_name(node, FALSE));
        return res;
      }
    }
    case EXT_OBJ: {
...
int SHA256_acyclic_type(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                        g_ptr node) {
  int res, lres, rres;
  string s;
  if (node == NULL)
  ...
  *g_cntp = res + 1;
  ...
  switch (GET_TYPE(node)) {
  case LAMBDA_ND:
    rres = SHA256_acyclic_type(g_cntp, g_tblp, sha, GET_LAMBDA_BODY(node));
    SHA256_printf(sha, "%d=\\%s.%d\n", res, GET_LAMBDA_VAR(node), rres);
    return res;
  case APPLY_ND:
    lres = SHA256_acyclic_type(g_cntp, g_tblp, sha, GET_APPLY_LEFT(node));
    rres = SHA256_acyclic_type(g_cntp, g_tblp, sha, GET_APPLY_RIGHT(node));
    SHA256_printf(sha, "%d=%d@%d\n", res, lres, rres);
    return res;
  case CONS_ND:
    lres = SHA256_acyclic_type(g_cntp, g_tblp, sha, GET_CONS_HD(node));
    rres = SHA256_acyclic_type(g_cntp, g_tblp, sha, GET_CONS_TL(node));
    SHA256_printf(sha, "%d=%d:%d\n", res, lres, rres);
    return res;
  case LEAF:
    switch (GET_LEAF_TYPE(node)) {
    ...
    case PRIM_FN: {
      ...
      case P_REF_VAR: {
        int r = GET_REF_VAR(node);
        lres = SHA256_acyclic_type(g_cntp, g_tblp, sha, Get_RefVar(r));
        SHA256_printf(sha, "%d=REF %d\n", res, r);
        return res;
      }
      default:
