...
static void table_create(g_ptr redex) {
  g_ptr g_n;
...
static void table_member(g_ptr redex) {
  g_ptr g_tbl, g_key;
...
static void table_get(g_ptr redex) {
  g_ptr g_tbl, g_key;
  EXTRACT_2_ARGS(redex, g_tbl, g_key);
  table_ptr tp = (table_ptr)GET_EXT_OBJ(g_tbl);
...
static void table_insert(g_ptr redex) {
...
static void table_insertn(g_ptr redex) {
  g_ptr g_tbl, kd_list;
...
static void set_table_insertn(g_ptr redex) {
  g_ptr g_tbl, k_list;
  ...
  while (!IS_NIL(k_list)) {
    ...
    if (find_hash(&(tp->tbl), g_key) != NULL) {
      MAKE_REDEX_FAILURE(redex, Fail_pr("set_tbl_insertn with duplicate key"));
      return;
    } else {
...
static void table_delete(g_ptr redex) {
  g_ptr g_tbl, g_key;
  EXTRACT_2_ARGS(redex, g_tbl, g_key);
  table_ptr tp = (table_ptr)GET_EXT_OBJ(g_tbl);
  if (find_hash(&(tp->tbl), g_key) == NULL) {
...
static void table_clear(g_ptr redex) {
  g_ptr g_tbl;
...
static void scan_fn(pointer pkey, pointer pdata) {
...
static void table_to_list(g_ptr redex) {
  g_ptr g_tbl;
  EXTRACT_1_ARG(redex, g_tbl);
  table_ptr tp = (table_ptr)GET_EXT_OBJ(g_tbl);
...
static void key_scan_fn(pointer pkey, pointer pdata) {
  g_ptr key = (g_ptr)pkey;
  g_ptr data = (g_ptr)pdata;
  INC_REFCNT(key);
  INC_REFCNT(data);
...
static void table_to_keys(g_ptr redex) {
  g_ptr g_tbl;
  EXTRACT_1_ARG(redex, g_tbl);
  table_ptr tp = (table_ptr)GET_EXT_OBJ(g_tbl);
  MAKE_REDEX_NIL(redex);
...
static void data_scan_fn(pointer pkey, pointer pdata) {
  g_ptr key = (g_ptr)pkey;
  g_ptr data = (g_ptr)pdata;
  INC_REFCNT(key);
  INC_REFCNT(data);
...
static void table_to_data(g_ptr redex) {
  g_ptr g_tbl;
  EXTRACT_1_ARG(redex, g_tbl);
  table_ptr tp = (table_ptr)GET_EXT_OBJ(g_tbl);
  MAKE_REDEX_NIL(redex);
...
static void list_to_table(g_ptr redex) {
  g_ptr list;
...
static void scan_mark_entries(pointer pkey, pointer pdata) {
  g_ptr key = (g_ptr)pkey;
  g_ptr data = (g_ptr)pdata;
  Mark(key);
  Mark(data);
...
static void table_mark_fn(pointer p) {
...
static void table_sweep_fn(void) {
  table_ptr tp;
...
static void table_save_fn(FILE *fp, pointer p) {
  write_table_ptr(fp, (table_ptr)p);
}

static pointer table_load_fn(FILE *fp) {
  table_ptr tp;
  read_table_ptr(fp, &tp);
  return ((pointer)tp);
}

static formula table_eq_fn(pointer p1, pointer p2, bool identical) {
  (void)identical;
...
static void do_sha256_comp(pointer pkey, pointer pdata) {
  g_ptr key = (g_ptr)pkey;
  g_ptr data = (g_ptr)pdata;
...
static int table_sha256_fn(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                           pointer a) {
...
void Table_Init() {
  ...
  //
  table_oidx = Add_ExtAPI_Object("table", table_mark_fn, table_sweep_fn,
                                 table_save_fn, table_load_fn, NULL,
                                 table_eq_fn, NULL, NULL, table_sha256_fn);
  table_free_list = NULL;
}
...
void Table_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      "tbl_member", "11", FALSE,
      GLmake_arrow(GLmake_tbl(tv1, tv2), GLmake_arrow(tv1, GLmake_bool())),
      table_member);

  Add_ExtAPI_Function(
      "tbl_get", "11", FALSE,
      GLmake_arrow(GLmake_tbl(tv1, tv2), GLmake_arrow(tv1, tv2)), table_get);

  Add_ExtAPI_Function(
      "tbl_insert", "111", FALSE,
      GLmake_arrow(GLmake_tbl(tv1, tv2),
                   GLmake_arrow(tv1, GLmake_arrow(tv2, GLmake_tbl(tv1, tv2)))),
      table_insert);

  Add_ExtAPI_Function(
      "tbl_insertn", "11", FALSE,
      GLmake_arrow(GLmake_tbl(tv1, tv2),
  ...
  Add_ExtAPI_Function(
      "set_tbl_insertn", "11", FALSE,
      GLmake_arrow(GLmake_tbl(tv1, tv1),
  ...
  Add_ExtAPI_Function(
      "tbl2list", "1", FALSE,
      GLmake_arrow(GLmake_tbl(tv1, tv2), GLmake_list(GLmake_tuple(tv1, tv2))),
      table_to_list);

  Add_ExtAPI_Function("tbl2keys", "1", FALSE,
                      GLmake_arrow(GLmake_tbl(tv1, tv2), GLmake_list(tv1)),
                      table_to_keys);

  Add_ExtAPI_Function("tbl2data", "1", FALSE,
                      GLmake_arrow(GLmake_tbl(tv1, tv2), GLmake_list(tv2)),
                      table_to_data);

  Add_ExtAPI_Function(
      "list2tbl", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_tuple(tv1, tv2)), GLmake_tbl(tv1, tv2)),
      list_to_table);
}
