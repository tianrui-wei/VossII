...
static int draw_graph_rec(FILE *fp, hash_record *hp, int depth, bool do_addr,
                          g_ptr node);
...
void oldGRl(g_ptr onode, int max_depth) {
  g_ptr nodes[2];
  g_ptr node = Get_node();
  *node = *onode;
  node->L = node->oldL;
...
static void do_draw_graph(g_ptr redex) {
...
void Draw_Graph_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      "draw_graph", "1-", FALSE,
      GLmake_arrow(GLmake_int(), GLmake_arrow(tv1, GLmake_void())),
      do_draw_graph);
}
...
static string make_string_safe(string s) {
  string res = strtemp("");
  while (*s) {
    if (*s == '"') {
      res = charappend('\\');
...
static int draw_graph_rec(FILE *fp, hash_record *hp, int depth, bool do_addr,
  ...
  case LAMBDA_ND: {
    ...
    fprintf(fp, "lbl%u [shape=plaintext, label = \"%s(%d)\"];\n", res,
            GET_LAMBDA_VAR(node), GET_LAMBDA_LINE_NBR(node));
    int l = draw_graph_rec(fp, hp, depth, do_addr, GET_LAMBDA_BODY(node));
  ...
  case APPLY_ND: {
    if (do_addr)
      ...
      fprintf(fp, "n%u [label = \"@%s\", ordering=out];\n", res, th);
    int l = draw_graph_rec(fp, hp, depth, do_addr, GET_APPLY_LEFT(node));
    fprintf(fp, "n%u -> n%u [color=red];\n", res, l);
    int r = draw_graph_rec(fp, hp, depth, do_addr, GET_APPLY_RIGHT(node));
  ...
  case CONS_ND: {
    if (do_addr)
      ...
      fprintf(fp, "n%u [label = \":%s\", ordering=out];\n", res, th);
    int l = draw_graph_rec(fp, hp, depth, do_addr, GET_CONS_HD(node));
    fprintf(fp, "n%u -> n%u [color=red];\n", res, l);
    int r = draw_graph_rec(fp, hp, depth, do_addr, GET_CONS_TL(node));
    fprintf(fp, "n%u -> n%u [color=blue];\n", res, r);
    return res;
  }
  case LEAF:
    ...
    case INT:
      ...
      if (do_addr)
        fprintf(fp, "n%u [shape=plaintext, label = \"%s%s,%p\"];\n", res, s, th,
                node);
      else
        fprintf(fp, "n%u [shape=plaintext, label = \"%s%s\"];\n", res, s, th);
      return res;
    case STRING:
    ...
    case EXT_OBJ: {
      char txt[1024];
      Sprintf(txt, "<<%s>>", Get_ExtAPI_Object_name(GET_EXT_OBJ_CLASS(node)));
      if (do_addr)
        fprintf(fp,
                "n%u [shape=plaintext, %s label "
                "= \"%s%s,%p\"];\n",
                res, "color=red,", txt, th, node);
      else
        fprintf(fp, "n%u [shape=plaintext, %s label = \"%s%s\"];\n", res,
                "color=red,", txt, th);
      return res;
    }
    ...
    case PRIM_FN: {
      ...
      case P_EXTAPI_FN:
        Sprintf(txt, "[%s]", Get_ExtAPI_Function_Name(GET_EXTAPI_FN(node)));
        hili = TRUE;
        break;
      case P_SSCANF:
      ...
      case P_REF_VAR: {
        if (do_addr)
          fprintf(fp, "n%u [label = \"ref%s,%p\"];\n", res, th, node);
        else
          fprintf(fp, "n%u [label = \"ref%s\"];\n", res, th);
        int ref_var = GET_REF_VAR(node);
        int l = draw_graph_rec(fp, hp, depth, do_addr, Get_RefVar(ref_var));
        fprintf(fp, "n%u -> n%u [color=black];\n", res, l);
        return res;
      }
      default:
      ...
      if (do_addr)
        fprintf(fp,
                "n%u [shape=plaintext, %s label "
                "= \"%s%s,%p\"];\n",
                res, (hili ? "color=orange," : ""), txt, th, node);
      else
        fprintf(fp, "n%u [shape=plaintext, %s label = \"%s%s\"];\n", res,
                (hili ? "color=orange," : ""), txt, th);
      return res;
    }
    ...
    case VAR: {
      ...
      if (do_addr) {
        ...
        fprintf(fp, "\"V\\\"%s\\\"(%d=%s)%s,%p\"];\n", s, version,
                get_version_name(node), th, node);
      } else {
        fprintf(fp, "n%u [shape=plaintext, label = ", res);
        fprintf(fp, "\"V\\\"%s\\\"(%d=%s)%s\"];\n", s, version,
                get_version_name(node), th);
      }
      return res;
    }
