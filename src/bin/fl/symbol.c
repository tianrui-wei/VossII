...
static char eprintf_help[] =
    ...
    "    d   the integer argument will be converted to a signed decimal "
    "number\n"
    "    x   the integer argument will be converted to a hexadecimal number\n"
    "    s   the string argument will be copied out\n"
    "    S   the list of strings argument will be copied out enclosed in [] "
    "and\n"
    "        each string separated by a comma.\n"
    "    B   the BDD (bool) argument will be converted to a string and copied "
    "to\n"
    "        the output\n";
...
static g_ptr add_non_lazy_context_rec(buffer *ctxt, symbol_tbl_ptr stbl,
                                      g_ptr node);
static bool is_non_lazy(symbol_tbl_ptr stbl, string name);
static bool check_match(string pat, string s);
static bool check_type_match(string arg_pat, string res_pat, typeExp_ptr type);
...
static impl_arg_ptr get_overloaded_calls_rec(impl_arg_ptr l,
                                             symbol_tbl_ptr stbl, g_ptr node);
...
static bool get_HFL_arg_name(g_ptr node, string *namep, g_ptr *next_nodep);
...
static g_ptr do_setjmp(buffer *contextp, symbol_tbl_ptr stbl, g_ptr *nodep) {
  g_ptr res = NULL;
...
g_ptr Add_non_lazy_context_and_find_Userdefs(g_ptr node, symbol_tbl_ptr stbl) {
  buffer context;
...
symbol_tbl_ptr Merge_fn_defs(symbol_tbl_ptr stbl1, symbol_tbl_ptr stbl2,
                             bool permissive) {
  if (stbl2 == NULL || stbl2->def_list == NULL) {
    return (stbl1);
  }
...
symbol_tbl_ptr Add_Destructors(string name, typeExp_ptr new_type,
                               symbol_tbl_ptr stbl, string file,
                               int start_line) {
  ...
  for (fnp = fnl; fnp != NULL; fnp = fnp->next) {
    ...
    tmp->next = newfnp;
    newfnp = tmp;
    insert_hash(&Constructor_Table, (pointer)(fnp->name), (pointer)(fnp->name));
  }
  ...
  if (Is_monomorphic(new_type)) {
    ...
    g_ptr save_expr =
        Make_1inp_Primitive(P_SAVE_GRAPH, Make_STRING_leaf(type_sig));
    typeExp_ptr save_type =
        GLmake_arrow(GLmake_string(), GLmake_arrow(new_type, GLmake_bool()));
    fn_ptr save_fun;
    ...
    save_fun->expr_init =
        cephalopode_mode ? Reflect_expr(save_fun->expr) : NULL;
    save_fun->expr_comb =
        cephalopode_mode ? Reflect_expr(save_fun->expr) : NULL;
    ...
    g_ptr load_expr =
        Make_1inp_Primitive(P_LOAD_GRAPH, Make_STRING_leaf(type_sig));
    ...
    load_fun->expr_init =
        cephalopode_mode ? Reflect_expr(load_fun->expr) : NULL;
    load_fun->expr_comb =
        cephalopode_mode ? Reflect_expr(load_fun->expr) : NULL;
...
symbol_tbl_ptr End_ADT(symbol_tbl_ptr stbl, var_list_ptr vlp) {
  ...
  if (empty_buf(&ADT_buf)) {
    FP(err_fp, "Syntax error: end_abstype without matching begin_abstype\n");
    while (vlp != NULL) {
      vp = vlp;
      vlp = vlp->next;
  ...
  while (vlp != NULL) {
    ...
    if (find_hash(stbl->tbl_ptr, vlp->name) == NULL) {
      FP(err_fp, "Error: end_abstype exports %s which is not defined\n",
         vlp->name);
      while (vlp != NULL) {
        vp = vlp;
        vlp = vlp->next;
...
static string do_special_help(string name) {
  string res = NULL;
  if (strcmp(name, "DIR") == 0)
    res = DIR_help;
  if (strcmp(name, "eprintf") == 0)
    res = eprintf_help;
...
string Get_Help(string fun) {
  ...
  if (fp->file_name != NULL && !built_in(fp->file_name)) {
    FP(FILE_fp, "File:  %s\nStart: %d\nEnd:   %d\n\n", fp->file_name,
       fp->start_line_nbr, fp->end_line_nbr);
  } else {
    FP(FILE_fp, "Built-in\n\n");
  }
  if (fp->overload) {
...
static formula default_eq_fn(pointer p1, pointer p2, bool identical) {
  (void)identical;
  if (p1 == p2) {
    return (B_One());
  } else {
...
int Add_ExtAPI_Object(string name, void (*mark_fn)(pointer p),
                      void (*sweep_fn)(), void (*save_fn)(FILE *fp, pointer p),
                      ...
                      pointer (*gmap2_fn)(gmap_info_ptr ip, pointer a,
                                          pointer b),
...
void Add_ExtAPI_Function(string name, string strictness, bool non_lazy,
...
#ifdef DEBUG
void PrintAllFuns(symbol_tbl_ptr stbl) {
...
static void get_args(g_ptr redex) {
...
static void get_arity(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  string name = GET_STRING(r);
  fn_ptr fp = Find_Function_Def(symb_tbl, name);
...
static void get_return_type(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  string name = GET_STRING(r);
  fn_ptr fp = Find_Function_Def(symb_tbl, name);
...
static void get_symbol_signature(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  string name = GET_STRING(r);
  fn_ptr fp = Find_Function_Def(symb_tbl, name);
...
static void is_defined(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void get_definition(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  if (!cephalopode_mode) {
  ...
  if (fp == NULL) {
    MAKE_REDEX_FAILURE(redex,
                       Fail_pr("Cannot find any function named '%s'", name));
  } else {
    if (fp->overload) {
      MAKE_REDEX_FAILURE(redex, Fail_pr("Function %s is overloaded", name));
    } else {
...
static void get_combinator_expression(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  if (!cephalopode_mode) {
    MAKE_REDEX_FAILURE(redex,
...
static void get_userdef_combinator_expression(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  if (!cephalopode_mode) {
    MAKE_REDEX_FAILURE(
        redex, "get_userdef_combinator_expression only available in -C mode");
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    return;
  }
  ...
  if (fp == NULL) {
    ...
    MAKE_REDEX_PAIR(redex, Make_STRING_leaf(fp->name), fp->expr_comb);
  }
  DEC_REF_CNT(l);
  DEC_REF_CNT(r);
}

static void get_fixity(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static int fn_cmp(const void *pi, const void *pj) {
  string *i = (string *)pi;
  string *j = (string *)pj;
  return (strcmp(*i, *j));
}

static void get_matching_functions(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_name_pat, g_file_pat, g_arg_type_pat, g_res_type_pat, g_max_cnt;
  EXTRACT_5_ARGS(redex, g_name_pat, g_file_pat, g_arg_type_pat, g_res_type_pat,
                 g_max_cnt);
...
void Symbols_Install_Functions() {
  ...
  Add_ExtAPI_Function("get_arity", "1", TRUE,
                      GLmake_arrow(GLmake_string(), GLmake_int()), get_arity);
  ...
  Add_ExtAPI_Function("get_definition", "1", TRUE,
                      GLmake_arrow(GLmake_string(), term), get_definition);
  Add_ExtAPI_Function("get_combinator_expression", "1", TRUE,
                      GLmake_arrow(GLmake_string(), term),
                      get_combinator_expression);
  Add_ExtAPI_Function(
      "get_userdef_combinator_expression", "1", TRUE,
      GLmake_arrow(GLmake_int(), GLmake_tuple(GLmake_string(), term)),
      get_userdef_combinator_expression);

  Add_ExtAPI_Function("is_defined", "1", TRUE,
                      GLmake_arrow(GLmake_string(), GLmake_bool()), is_defined);
  Add_ExtAPI_Function("get_fixity", "1", TRUE,
                      GLmake_arrow(GLmake_string(), GLmake_string()),
                      get_fixity);
  Add_ExtAPI_Function(
      "get_matching_functions", "11111", TRUE,
      GLmake_arrow(
          GLmake_string(),
          GLmake_arrow(
              GLmake_string(),
...
arg_names_ptr Get_argument_names(g_ptr onode) {
  buffer args;
  string *sp;
...
static symbol_tbl_ptr create_empty_symb_tbl() {
  symbol_tbl_ptr stbl = stbl = (symbol_tbl_ptr)new_rec(&symb_tbl_mgr);
  stbl->def_list = NULL;
...
static void add_binding(string name) {
...
static void remove_binding(string name) {
  int cur = PTR2INT(find_hash(&bound_var_tbl, (pointer)name));
  ASSERT(cur != 0);
  cur--;
  delete_hash(&bound_var_tbl, (pointer)name);
...
static bool is_bound(string name) {
  int cur = PTR2INT(find_hash(&bound_var_tbl, (pointer)name));
  return (cur > 1);
}
  ...
  case LEAF:
    if (!IS_VAR(node)) {
      ...
      if ((pfn != P_PRINTF) && (pfn != P_SPRINTF) && (pfn != P_EPRINTF) &&
          (pfn != P_FPRINTF) && (pfn != P_PRINT) && (pfn != P_MK_REF_VAR)) {
        return (node);
      }
      ...
      FOR_BUF(ctxt, string, sp) {
        ctx_nd = Make_2inp_Primitive(
            P_CONS, Make_1inp_Primitive(P_UNTYPE, Make_VAR_leaf(*sp)), ctx_nd);
      }
...
static bool get_named_arg(g_ptr node, string *namep, g_ptr *next_nodep) {
  if (!IS_APPLY(node))
    return FALSE;
  if (!IS_LEAF_VAR(GET_APPLY_RIGHT(node)))
    return FALSE;
...
static bool get_HFL_arg_name(g_ptr node, string *namep, g_ptr *next_nodep) {
  ...
  if (!IS_LEAF_VAR(GET_APPLY_RIGHT(node)))
    return FALSE;
  if (!STREQ(GET_VAR(GET_APPLY_RIGHT(node)), s_CELL))
    return FALSE;
  node = GET_APPLY_LEFT(node);
  if (!IS_APPLY(node))
    return FALSE;
  if (!IS_STRING(GET_APPLY_RIGHT(node)))
    return FALSE;
  *namep = GET_STRING(GET_APPLY_RIGHT(node));
  node = GET_APPLY_LEFT(node);
  if (!IS_LEAF_VAR(GET_APPLY_LEFT(node)))
    return FALSE;
  node = GET_APPLY_RIGHT(node);
  if (!IS_LAMBDA(node))
    return FALSE;
  if (!STREQ(GET_LAMBDA_VAR(node), *namep))
    return FALSE;
  node = GET_LAMBDA_BODY(node);
  if (!IS_LAMBDA(node))
    return FALSE;
  if (!STREQ(GET_LAMBDA_VAR(node), s_CELL))
    return FALSE;
  node = GET_LAMBDA_BODY(node);
  *next_nodep = node;
  return TRUE;
...
static g_ptr ignore_simple_P_PCATCH(g_ptr onode) {
  ...
  if (!IS_APPLY(node))
    return onode;
  if (!IS_APPLY(GET_APPLY_RIGHT(node)))
    return onode;
  if (!IS_P_ERROR(GET_APPLY_LEFT(GET_APPLY_RIGHT(node))))
    return onode;
  node = GET_APPLY_LEFT(node);
  if (!IS_APPLY(node))
    return onode;
...
static g_ptr ignore_P_CACHE(g_ptr onode) {
  ...
  if (!IS_APPLY(node))
    return onode;
  if (!IS_APPLY(GET_APPLY_LEFT(node)))
    return onode;
  if (!IS_P_CACHE(GET_APPLY_LEFT(GET_APPLY_LEFT(node))))
    return onode;
  node = GET_APPLY_RIGHT(node);
  return node;
}
