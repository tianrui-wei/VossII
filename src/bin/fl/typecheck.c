...
static int nbr_succ_overload_res(node_type_ptr cur, node_replace_ptr *replsp,
                                 bool closed, int level);
g_ptr make_implicit_call(impl_arg_ptr iargs, fn_ptr fun);
static node_type_ptr build_node_type_struct(hash_record *lvars_tbl, g_ptr node);
...
#endif
    if (nbr_solns == 0) {
      while (nrp != NULL && nrp->alts_found != 0) {
        nrp = nrp->next;
      }
      ASSERT(nrp != NULL);
      if (file_load)
        FP(err_fp, "===Type error around line %d in file %s\n",
           GET_LINE_NBR(nrp->old), cur_file_name);
      else
...
static bool check_typelists(typeList_ptr l1, typeList_ptr l2) {
repeat_len_check:
  if (l1 == NULL && l2 == NULL)
    return TRUE;
  if (l1 == NULL)
  ...
  if (type != NULL) {
    if (type->typelist != NULL) {
      ...
      if (!check_typelists(tvars, type->typelist)) {
        FP(err_fp,
           "-E-: Inconsistent number of type arguments "
           "for constructor %s "
...
typeExp_ptr Get_Real_Type(typeExp_ptr type) { return (get_real_type(type)); }
...
void Print_Type(typeExp_ptr type, odests fp, bool newline, bool reset) {
  if (reset) {
    reset_cnt(type);
    anon_cnt = 1;
  }
...
typeExp_ptr Fix_Types(typeExp_ptr *constructor_type,
  ...
  t1 = GLnew_tVar();
  old_type =
      GLmake_arrow(GLmake_arrow(old_type, t1), GLmake_arrow(concrete_type, t1));
  return (old_type);
}
...
typeExp_ptr GLmake_bool() { return (bool_op); }

typeExp_ptr GLmake_bexpr() { return (bexpr_op); }
...
typeExp_ptr GLmake_int() { return (int_op); }

typeExp_ptr GLmake_string() { return (string_op); }

typeExp_ptr GLmake_fsm() { return (obsolete_fsm_op); }

typeExp_ptr GLmake_void() { return (void_op); }

typeExp_ptr GLmake_fp() { return (fp_op); }
...
bool Is_Ref(typeExp_ptr type) { return (type->typeOp == ref_tp); }

bool Is_Tbl(typeExp_ptr type) { return (type->typeOp == tbl_tp); }

bool Is_List(typeExp_ptr type) { return (type->typeOp == list_tp); }

bool Is_Tuple(typeExp_ptr type) { return (type->typeOp == tuple_tp); }
...
bool Forward_Declare_Ok(typeExp_ptr declare_type, typeExp_ptr actual_type) {
...
bool Check_no_undef_types(typeExp_ptr type) {
  ...
  case list_tp:
    for (tlp = type->typelist; tlp != NULL; tlp = tlp->next) {
      if (!Check_no_undef_types(tlp->type))
        return (FALSE);
    }
    break;
  default:
...
static void record_old_value(typeExp_ptr t) {
  type_diff_rec tr;
  tr.type = t;
  tr.content = *t;
...
static bool unify(bool quiet, typeExp_ptr t1, typeExp_ptr t2) {
  ...
  if ((t1->typeOp != t2->typeOp) ||
      ((t1->typeOp == concrete_type_tp) && (!STREQ(t1->u.name, t2->u.name)))) {
    if (!quiet) {
  ...
  for (tlp1 = t1->typelist, tlp2 = t2->typelist; tlp1 != NULL;
...
static typeExp_ptr printf_type(ui pfn, string pat) {
  buffer args_buf;
  new_buf(&args_buf, 100, sizeof(typeExp_ptr));

  PROCESS_PRINTF_PAT(
      pat, get_types_lbl,                   /* lbl */
      {},                                   /* start_of_pat */
      {},                                   /* pat_0 */
      {},                                   /* pat_1_to_9 */
      {},                                   /* pat_minus */
  ...
  typeExp_ptr *tp;
  FUB_ROF(&args_buf, typeExp_ptr, tp) { res = make_arrow(*tp, res); }
  free_buf(&args_buf);
  if (pfn == P_FPRINTF)
...
static typeExp_ptr sscanf_type(string pat) {
  buffer args_buf;
  buffer res_buf;
  new_buf(&args_buf, 100, sizeof(typeExp_ptr));
...
static bool mismatch_fun_arg(typeExp_ptr *t1p, typeExp_ptr *t2p) {
  typeExp_ptr t1 = *t1p;
  typeExp_ptr t2 = *t2p;
  if (t1->typeOp != arrow_tp)
    return FALSE;
  if (t2->typeOp != arrow_tp)
    return FALSE;
...
static void report_failure(node_type_ptr cur, typeExp_ptr type,
                           typeExp_ptr expected_type) {
  g_ptr node = cur->node;
  if (GET_TYPE(node) == LEAF) {
    ...
    case VAR:
      if (strncmp("__DeStRuCtOr", GET_VAR(node), 12) == 0) {
        ...
        FP(err_fp, "The destructor function to the constructor `%s'",
           ((GET_VAR(node)) + 12));
...
static typeExp_ptr get_prim_fun_type(g_ptr node) {
  ...
  case P_S:
    ...
    tVar2 = new_tVar();
    type = (make_arrow(
        make_arrow(tVar1, make_arrow(tVar2, tVar0)),
        make_arrow(make_arrow(tVar1, tVar2), make_arrow(tVar1, tVar0))));
    break;
  case P_K:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(tVar1, tVar0)));
    break;
  case P_I:
    tVar0 = new_tVar();
  ...
  case P_C:
    ...
    tVar1 = new_tVar();
    tVar2 = new_tVar();
    type = (make_arrow(make_arrow(tVar1, make_arrow(tVar2, tVar0)),
                       make_arrow(tVar2, make_arrow(tVar1, tVar0))));
    break;
  case P_B:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
  ...
  case P_BPRIME:
    ...
    tVar3 = new_tVar();
    type =
        (make_arrow(make_arrow(tVar3, make_arrow(tVar1, tVar0)),
                    make_arrow(tVar3, make_arrow(make_arrow(tVar2, tVar1),
                                                 make_arrow(tVar2, tVar0)))));
    break;
  case P_SPRIME:
    tVar0 = new_tVar();
    ...
    tVar2 = new_tVar();
    tVar3 = new_tVar();
    type = (make_arrow(make_arrow(tVar3, make_arrow(tVar2, tVar0)),
  ...
  case P_CPRIME:
    ...
    tVar3 = new_tVar();
    type =
        (make_arrow(make_arrow(tVar3, make_arrow(tVar2, tVar0)),
                    make_arrow(make_arrow(tVar1, tVar3),
                               make_arrow(tVar2, make_arrow(tVar1, tVar0)))));
    break;
  case P_BSTAR:
  ...
  case P_Y:
    tVar0 = new_tVar();
    type = (make_arrow(make_arrow(tVar0, tVar0), tVar0));
    break;
  case P_FST:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
  ...
  case P_TAIL:
    tVar0 = new_tVar();
    type = (make_arrow(make_list(tVar0), make_list(tVar0)));
    break;
  case P_TUPLE:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(tVar1, make_tuple(tVar0, tVar1))));
    break;
  case P_CONS:
    tVar0 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(make_list(tVar0), make_list(tVar0))));
    break;
  case P_COND:
    tVar0 = new_tVar();
    type = (make_arrow(bool_op, make_arrow(tVar0, make_arrow(tVar0, tVar0))));
    break;
  case P_CAT:
    type = (make_arrow(string_op, make_arrow(string_op, string_op)));
    break;
  case P_EMPTY:
    tVar0 = new_tVar();
    ...
    break;
  case P_LOAD:
    type = (make_arrow(string_op, make_arrow(bool_op, bool_op)));
    break;
  case P_LOAD_PLUGIN:
  ...
  case P_PCATCH:
    tVar0 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(tVar0, tVar0)));
    break;
  case P_CATCH:
    tVar0 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(tVar0, tVar0)));
  ...
  case P_NSEQ:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
    type = (make_arrow(tVar1, make_arrow(tVar0, tVar0)));
    break;
  case P_IS_CONS:
    tVar0 = new_tVar();
  ...
  case P_QUANT_THEREIS:
    type = (make_arrow(bool_op, make_arrow(bool_op, bool_op)));
    break;
  case P_IDENTICAL:
    tVar0 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(tVar0, bool_op)));
    break;
  case P_EQUAL:
    tVar0 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(tVar0, bool_op)));
  ...
  case P_IMPLODE:
    ...
    break;
  case P_BOOL2STR:
    type = (make_arrow(int_op, make_arrow(bool_op, string_op)));
    break;
  case P_INT2STR:
    type = (make_arrow(int_op, string_op));
    break;
  case P_TIME:
    tVar0 = new_tVar();
    type = (make_arrow(tVar0, make_tuple(tVar0, string_op)));
    break;
  case P_BDD_SAVE:
    type = (make_arrow(string_op, make_arrow(make_list(bool_op), bool_op)));
    break;
  case P_BDD_LOAD:
  ...
  case P_RELPROD_FORALL:
    type = (make_arrow(bool_op,
                       make_arrow(bool_op, make_arrow(bool_op, bool_op))));
    break;
  case P_RVAR:
  ...
  case P_UPDATE_RVAR:
    tVar0 = new_tVar();
    type = (make_arrow(make_ref(tVar0), make_arrow(tVar0, void_op)));
    break;
  case P_REF_VAR:
    tVar0 = new_tVar();
    type = (make_ref(tVar0));
  ...
  case P_WRITE_TO_FILE:
    type = (make_arrow(string_op, make_arrow(string_op, void_op)));
    break;
  case P_PRINTF:
  case P_FPRINTF:
  case P_SPRINTF:
  ...
  case P_SSCANF:
    ...
    break;
  case P_FOPEN:
    type = make_arrow(string_op, make_arrow(string_op, fp_op));
    break;
  case P_FCLOSE:
  ...
  case P_GEN_CATCH:
    tVar0 = new_tVar();
    type = make_arrow(tVar0, make_arrow(make_arrow(string_op, tVar0), tVar0));
    break;
  case P_TCL_EVAL:
    type = make_arrow(make_list(string_op), string_op);
    break;
  ...
  case P_SAVE_GRAPH:
    ...
    type = (make_arrow(string_op,
                       make_arrow(string_op, make_arrow(tVar0, bool_op))));
    break;
  case P_LOAD_GRAPH:
    tVar0 = new_tVar();
    type = (make_arrow(string_op, make_arrow(string_op, tVar0)));
    break;
  case P_EXIT:
  ...
  case P_THEN:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
    type = (make_arrow(tVar0, make_arrow(make_arrow(tVar0, tVar1), tVar1)));
    break;
  case P_UNTYPE:
    tVar0 = new_tVar();
    tVar1 = new_tVar();
...
static void check_hint(g_ptr node, typeExp_ptr type) {
  ...
  if (!unify(FALSE, type, res)) {
    ...
    if (file_load)
      FP(err_fp, "===Type assertion around line %d in file %s failed\n", pos,
         cur_file_name);
    else
...
static int nbr_succ_overload_res(node_type_ptr cur, node_replace_ptr *replsp,
  ...
  if (level > MAX_OVERLOAD_RESOLUTION_DEPTH) {
    if (file_load)
      FP(err_fp, "===Type error around line %d in file %s\n",
         GET_LINE_NBR(cur->node), cur_file_name);
    else
  ...
  while (alts != NULL) {
    ...
    if (Type_eq(alt_type, cur_type) || unify(TRUE, alt_type, cur_type)) {
      g_ptr new_node = make_implicit_call(alts->fn->implicit_args, alts->fn);
      ...
      node_replace_ptr nrp;
      int rem_solns =
          nbr_succ_overload_res(to_be_resolved, &nrp, closed, level + 1);
      cnt += rem_solns;
      if (cnt > 1) {
...
static typeExp_ptr get_and_replace_final_type(typeExp_ptr *type,
  ...
  if ((*type)->typeOp == arrow_tp)
    return (
        get_and_replace_final_type(&((*type)->typelist->next->type), new_type));
  old_type = *type;
  *type = new_type;
  return (old_type);
}
...
unsigned int Type_hash(pointer tp, unsigned int n) {
  ...
  case arrow_tp:
    return ((937 * Type_hash((pointer)type->typelist->type, n) +
             Type_hash((pointer)type->typelist->next->type, n)) %
            n);
  case tuple_tp:
...
bool Type_eq(pointer t1, pointer t2) {
  ...
  case concrete_type_tp: {
    if (!STREQ(type1->u.name, type2->u.name))
      return (FALSE);
...
static void dpt(typeExp_ptr type) {
  switch (type->typeOp) {
  case Indirection_tp:
...
static void clean_aliases(typeExp_ptr type) {
  typeList_ptr tlp;
