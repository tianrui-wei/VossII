...
static string state_holding_names[] = {"draw_adff",
                                       "draw_adffe",
                                       "draw_ah_latch",
                                       "draw_al_latch",
                                       "draw_dff",
...
static vec_ptr split_vector_name(rec_mgr *vec_rec_mgrp, rec_mgr *range_rec_mgrp,
                                 string name);
...
static void extract_five_tuple(g_ptr nd, gbv *whenp, string *namep, gbv *val,
                               int *fromp, int *top);
static void extract_four_tuple(g_ptr nd, gbv *whenp, string *namep, int *fromp,
                               int *top);
static void extract_tripple(g_ptr nd, string *namep, int *fromp, int *top);
static int event_cmp(const void *p1, const void *p2);
static bool create_event_buffer(ste_ptr ste, buffer *ebufp, g_ptr wl, g_ptr ant,
                                g_ptr cons, g_ptr trl, bool abort_ASAP);
...
static bool compile_expr(hash_record *vtblp, string hier, ilist_ptr outs,
                         g_ptr we, bool pdel);
...
static void map_node(vec_ptr decl, vec_ptr ivec, ilist_ptr map, int cur);
...
static bool traverse_pexlif(hash_record *parent_tblp, g_ptr p, string hier,
                            bool top_level, int draw_level, int inst_cnt,
                            int max_depth);
...
static void build_limit_tbl(vstate_ptr vp, hash_record *limit_tblp, string sn,
                            sch_ptr sch);
...
static string mk_fresh_anon_name(g_ptr internals, g_ptr fa_inps, g_ptr fa_outs,
                                 int *cur_cntp);
static string mk_vector_name(string base, int size);
static string create_constant(int sz, int *ccnt, g_ptr ints, g_ptr fa_inps,
                              g_ptr fa_outs, string cnst, buffer *chbufp);
...
static string create_merge_component(int sz, int *ccnt, g_ptr *intsp,
                                     g_ptr fa_inps, g_ptr fa_outs, g_ptr acts,
                                     int len, buffer *chbufp);
static g_ptr clean_pexlif_ios(g_ptr node);
static int fsm_sha256_fn(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                         pointer a);
...
static fsm_ptr gen_pexlif2fsm(g_ptr p, g_ptr black_box_list, int max_depth);
...
void Fsm_Init() {
  ...
  //
  fsm_oidx = Add_ExtAPI_Object("fsm", mark_fsm_fn, sweep_fsm_fn, save_fsm_fn,
                               load_fsm_fn, fsm2str_fn, fsm_eq_fn, NULL, NULL,
                               fsm_sha256_fn);
  fsm_handle_tp = Get_Type("fsm", NULL, TP_INSERT_FULL_TYPE);

  ste_oidx = Add_ExtAPI_Object("ste", mark_ste_fn, sweep_ste_fn, save_ste_fn,
                               load_ste_fn, ste2str_fn, ste_eq_fn, ste_gmap_fn,
                               ste_gmap2_fn, NULL);
  ste_handle_tp = Get_Type("ste", NULL, TP_INSERT_FULL_TYPE);

  vstate_oidx = Add_ExtAPI_Object("vstate", mark_vstate_fn, sweep_vstate_fn,
                                  save_vstate_fn, load_vstate_fn, vstate2str_fn,
                                  NULL, // vstate_eq_fn,
                                  NULL, // vstate_gmap_fn,
...
static void newSTE(g_ptr redex) {
...
static void bSTE(g_ptr redex) { gSTE(redex, use_bexprs); }

static void toplevel_pexlif2fsm(g_ptr redex) {
...
static void pexlif2fsm(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr p = GET_APPLY_RIGHT(redex);
  fsm_ptr fsm = gen_pexlif2fsm(p, NULL, -1);
...
static void pexlif2fsm2(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr p, bb_list;
...
static void assertions(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm;
  EXTRACT_1_ARG(redex, g_fsm);
...
static void vectors(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm;
  EXTRACT_1_ARG(redex, g_fsm);
...
static void inputs(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm;
  EXTRACT_1_ARG(redex, g_fsm);
  ...
  g_ptr tail = redex;
  string *sp;
  FOR_BUF(top_inpsp, string, sp) { APPEND1(tail, Make_STRING_leaf(*sp)); }
  pop_fsm();
  DEC_REF_CNT(l);
  DEC_REF_CNT(r);
}

static void outputs(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm;
  EXTRACT_1_ARG(redex, g_fsm);
  ...
  g_ptr tail = redex;
  string *sp;
  FOR_BUF(top_outsp, string, sp) { APPEND1(tail, Make_STRING_leaf(*sp)); }
  pop_fsm();
  DEC_REF_CNT(l);
  DEC_REF_CNT(r);
}

static void visualization_nodes(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_level;
...
static void get_latch_type(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
...
static void is_latch(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void is_input(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void is_phase_delay(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void get_visualization_id(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void get_visualization_pinst_cnt(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node, g_level;
  EXTRACT_3_ARGS(redex, g_fsm, g_node, g_level);
...
static void get_visualization_pfn(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node, g_level;
  EXTRACT_3_ARGS(redex, g_fsm, g_node, g_level);
...
static void get_visualization_attributes(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node, g_level;
  EXTRACT_3_ARGS(redex, g_fsm, g_node, g_level);
...
static void get_visualization_fanins(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node, g_level;
  EXTRACT_3_ARGS(redex, g_fsm, g_node, g_level);
  ...
  for (vis_io_ptr vip = vp->fa_inps; vip != NULL; vip = vip->next) {
    g_ptr pair =
        Make_CONS_ND(Make_STRING_leaf(vip->f_vec), ilist2nds(vip->acts));
...
static void get_visualization_outputs(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node, g_level;
  EXTRACT_3_ARGS(redex, g_fsm, g_node, g_level);
...
static void excitation_function(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void fanin_dfs(g_ptr redex) {
  ...
  while (COUNT_BUF(&todo_buf) > 0) {
    ...
    if (find_hash(&done_tbl, INT2PTR(nd_idx)) == NULL) {
      ...
      if (keep_going) {
        ...
        if (composite >= 0) {
          ncomp_ptr cp = (ncomp_ptr)M_LOCATE_BUF(compositesp, composite);
          FOREACH_NODE(nd, cp->inps) { push_buf(&todo_buf, &nd); }
        }
      }
    }
  }
...
static void fanout(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void fanout_dfs(g_ptr redex) {
  g_ptr g_fsm, pred, nodes;
  EXTRACT_3_ARGS(redex, g_fsm, pred, nodes);
  fsm_ptr fsm = (fsm_ptr)GET_EXT_OBJ(g_fsm);
  push_fsm(fsm);
    ...
    if (find_hash(&done_tbl, INT2PTR(nd_idx)) == NULL) {
      ...
      if (keep_going) {
        ...
        for (idx_list_ptr ip = np->fanouts; ip != NULL; ip = ip->next) {
          ...
          FOREACH_NODE(nd, cp->outs) { push_buf(&todo_buf, &nd); }
        }
      }
    }
  }
...
static void node2vector(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void node2value_list(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
  ...
  if (vlist != NULL) {
    ...
    while (*s == '{') {
      ...
      *end = '\0';
      ...
      g_ptr pair = Make_CONS_ND(Make_INT_leaf(code), Make_STRING_leaf(name));
      APPEND1(tail, pair)
      s = end + 1;
    }
  }
...
static void emit_fsm(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_file;
...
static void basename(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, g_node;
  EXTRACT_2_ARGS(redex, g_fsm, g_node);
...
static void ste2fsm(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_ste;
...
static void get_weak_expressions(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_ste;
  EXTRACT_1_ARG(redex, g_ste);
...
static void get_abstract_depends(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_ste, obj;
...
static void get_trace(g_ptr redex) { gen_get_trace(redex, use_bdds); }

static void get_btrace(g_ptr redex) { gen_get_trace(redex, use_bexprs); }

static void get_trace_val(g_ptr redex) { gen_get_trace_val(redex, use_bdds); }

static void get_btrace_val(g_ptr redex) {
  gen_get_trace_val(redex, use_bexprs);
}

static void visualization_set_stop_nodes(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_vstate, vecs;
...
static void get_ste_type(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_ste;
  EXTRACT_1_ARG(redex, g_ste);
...
static void get_ste_result(g_ptr redex) { gen_get_ste_result(redex, use_bdds); }

static void bget_ste_result(g_ptr redex) {
  gen_get_ste_result(redex, use_bexprs);
}

static void get_ste_maxtime(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_ste;
  EXTRACT_1_ARG(redex, g_ste);
...
static void visualization_vecs2tags(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  ...
  while (!IS_NIL(vecs)) {
    ...
    if (il == NULL) {
      ...
      FOREACH_NODE(nd, il) { insert_hash(&idx_tbl, INT2PTR(nd), INT2PTR(1)); }
    }
...
static void visualization_anon2real(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void visualisation2tcl(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_vstate;
...
static void visualize_fanin(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_fsm, stop_vecs, ifc_vecs, g_levels, vecs, g_draw_level;
  EXTRACT_6_ARGS(redex, g_fsm, stop_vecs, ifc_vecs, g_levels, vecs,
                 g_draw_level);
...
static void visualize_expand_fanin(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void visualize_hide_fanin(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void visualize_undo(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_vstate;
  EXTRACT_1_ARG(redex, g_vstate);
...
static void visualize_get_shown_anons(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_vstate;
  EXTRACT_1_ARG(redex, g_vstate);
...
void Fsm_Install_Functions() {
  ...
  Add_ExtAPI_Function("dbg_clean_pexlif_ios", "1", FALSE,
                      GLmake_arrow(pexlif_tp, pexlif_tp), fl_clean_pexlif_ios);
  ...
  Add_ExtAPI_Function("pexlif2fsm", "1", FALSE,
                      GLmake_arrow(pexlif_tp, fsm_handle_tp), pexlif2fsm);

  Add_ExtAPI_Function(
      "pexlif2fsm2", "11", FALSE,
      GLmake_arrow(pexlif_tp,
                   GLmake_arrow(GLmake_list(GLmake_string()), fsm_handle_tp)),
      pexlif2fsm2);

  typeExp_ptr weak_tp = GLmake_list(GLmake_tuple(
      GLmake_bool(),
      GLmake_tuple(GLmake_string(), GLmake_tuple(GLmake_int(), GLmake_int()))));

  typeExp_ptr be_weak_tp = GLmake_list(GLmake_tuple(
      GLmake_bexpr(),
      GLmake_tuple(GLmake_string(), GLmake_tuple(GLmake_int(), GLmake_int()))));

  typeExp_ptr ant_tp = GLmake_list(GLmake_tuple(
      GLmake_bool(),
      GLmake_tuple(GLmake_string(),
                   GLmake_tuple(GLmake_bool(),
                                GLmake_tuple(GLmake_int(), GLmake_int())))));

  typeExp_ptr be_ant_tp = GLmake_list(GLmake_tuple(
      GLmake_bexpr(),
      GLmake_tuple(GLmake_string(),
                   GLmake_tuple(GLmake_bexpr(),
                                GLmake_tuple(GLmake_int(), GLmake_int())))));
          ...
          GLmake_arrow(
              ...
              GLmake_arrow(
                  ...
                  GLmake_arrow(
                      ant_tp,
                      GLmake_arrow(ant_tp,
                                   GLmake_arrow(tr_tp, ste_handle_tp)))))),
      newSTE);
      ...
      GLmake_arrow(
          ...
          GLmake_arrow(
              ...
              GLmake_arrow(
                  ...
                  GLmake_arrow(
                      be_ant_tp,
                      GLmake_arrow(be_ant_tp,
                                   GLmake_arrow(tr_tp, ste_handle_tp)))))),
      bSTE);

  Add_ExtAPI_Function("ste2fsm", "1", FALSE,
                      GLmake_arrow(ste_handle_tp, fsm_handle_tp), ste2fsm);

  Add_ExtAPI_Function("nodes", "1", FALSE,
                      GLmake_arrow(fsm_handle_tp, GLmake_list(GLmake_string())),
                      nodes);
  ...
  Add_ExtAPI_Function(
      "visualization_nodes", "11", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_int(), GLmake_list(GLmake_string()))),
      visualization_nodes);

  Add_ExtAPI_Function(
      "is_phase_delay", "11", FALSE,
      GLmake_arrow(fsm_handle_tp, GLmake_arrow(GLmake_string(), GLmake_bool())),
      is_phase_delay);
  ...
  Add_ExtAPI_Function(
      "get_latch_type", "11", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(), GLmake_string())),
      get_latch_type);

  Add_ExtAPI_Function(
      "get_visualization_id", "111", FALSE,
      GLmake_arrow(fsm_handle_tp,
  ...
  Add_ExtAPI_Function(
      "get_visualization_pinst_cnt", "111", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(),
                                GLmake_arrow(GLmake_int(), GLmake_int()))),
  ...
  Add_ExtAPI_Function(
      "get_visualization_pfn", "111", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(),
  ...
  Add_ExtAPI_Function(
      "get_visualization_attributes", "111", FALSE,
      GLmake_arrow(
          fsm_handle_tp,
          GLmake_arrow(GLmake_string(),
                       GLmake_arrow(GLmake_int(),
                                    GLmake_list(GLmake_tuple(
                                        GLmake_string(), GLmake_string()))))),
      get_visualization_attributes);

  Add_ExtAPI_Function(
      "get_visualization_fanins", "111", FALSE,
      GLmake_arrow(
          fsm_handle_tp,
          GLmake_arrow(
              GLmake_string(),
              GLmake_arrow(GLmake_int(), GLmake_list(GLmake_tuple(
                                             GLmake_string(),
                                             GLmake_list(GLmake_string())))))),
      get_visualization_fanins);

  Add_ExtAPI_Function(
      "get_visualization_outputs", "111", FALSE,
      GLmake_arrow(
          fsm_handle_tp,
          GLmake_arrow(
              GLmake_string(),
  ...
  Add_ExtAPI_Function(
      "fanin", "11", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(), GLmake_list(GLmake_string()))),
      fanin);

  Add_ExtAPI_Function(
      "excitation_function", "11", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(), GLmake_string())),
      excitation_function);
  ...
  Add_ExtAPI_Function(
      "fanin_dfs", "111", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_arrow(GLmake_string(), GLmake_bool()),
                                GLmake_arrow(GLmake_list(GLmake_string()),
                                             GLmake_list(GLmake_string())))),
      fanin_dfs);

  Add_ExtAPI_Function(
      "fanout_dfs", "111", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_arrow(GLmake_string(), GLmake_bool()),
                                GLmake_arrow(GLmake_list(GLmake_string()),
                                             GLmake_list(GLmake_string())))),
  ...
  Add_ExtAPI_Function(
      "node2vector", "11", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(), GLmake_string())),
      node2vector);

  Add_ExtAPI_Function(
      "node2value_list", "11", FALSE,
      GLmake_arrow(fsm_handle_tp,
                   GLmake_arrow(GLmake_string(),
                                GLmake_list(GLmake_tuple(GLmake_int(),
                                                         GLmake_string())))),
      node2value_list);

  Add_ExtAPI_Function(
      "dbg_emit_fsm", "11", FALSE,
      GLmake_arrow(fsm_handle_tp, GLmake_arrow(GLmake_string(), GLmake_void())),
      emit_fsm);
  ...
  Add_ExtAPI_Function(
      "get_ste_result", "11", FALSE,
      GLmake_arrow(ste_handle_tp, GLmake_arrow(GLmake_string(), GLmake_bool())),
      get_ste_result);

  Add_ExtAPI_Function("bget_ste_result", "11", FALSE,
                      GLmake_arrow(ste_handle_tp, GLmake_arrow(GLmake_string(),
                                                               GLmake_bexpr())),
                      bget_ste_result);
  ...
  Add_ExtAPI_Function(
      "get_weak_expressions", "1", FALSE,
      GLmake_arrow(ste_handle_tp,
                   GLmake_list(GLmake_tuple(GLmake_string(), GLmake_bool()))),
      get_weak_expressions);

  Add_ExtAPI_Function(
      "get_abstract_depends", "11", FALSE,
      GLmake_arrow(ste_handle_tp,
                   GLmake_arrow(GLnew_tVar(), GLmake_list(GLmake_string()))),
  ...
  Add_ExtAPI_Function(
      ...
      GLmake_arrow(
          ...
          GLmake_arrow(
              GLmake_string(),
              GLmake_list(GLmake_tuple(
                  GLmake_int(), GLmake_tuple(GLmake_bool(), GLmake_bool()))))),
      get_trace);
  ...
  Add_ExtAPI_Function(
      "visualization_anon2real", "11", FALSE,
      GLmake_arrow(vstate_handle_tp,
                   GLmake_arrow(GLmake_string(), GLmake_list(GLmake_string()))),
      visualization_anon2real);

  Add_ExtAPI_Function(
      "visualization_vecs2tags", "111", FALSE,
      GLmake_arrow(
          vstate_handle_tp,
          GLmake_arrow(
              GLmake_bool(),
              GLmake_arrow(GLmake_list(GLmake_string()),
                           GLmake_tuple(GLmake_list(GLmake_string()),
                                        GLmake_list(GLmake_string()))))),
      visualization_vecs2tags);

  Add_ExtAPI_Function(
      "visualisation2tcl", "1", FALSE,
      GLmake_arrow(vstate_handle_tp,
                   GLmake_tuple(GLmake_string(), GLmake_list(GLmake_string()))),
      visualisation2tcl);

  Add_ExtAPI_Function(
      "visualization_set_stop_nodes", "11", FALSE,
      GLmake_arrow(vstate_handle_tp,
                   GLmake_arrow(GLmake_list(GLmake_string()), GLmake_void())),
  ...
  Add_ExtAPI_Function(
      "visualize_get_shown_anons", "1", FALSE,
      GLmake_arrow(vstate_handle_tp, GLmake_list(GLmake_string())),
      visualize_get_shown_anons);

  Add_ExtAPI_Function(
      "visualize_expand_fanin", "1111", FALSE,
      GLmake_arrow(vstate_handle_tp,
                   GLmake_arrow(GLmake_string(),
                                GLmake_arrow(GLmake_int(),
                                             GLmake_arrow(GLmake_int(),
  ...
  Add_ExtAPI_Function(
      "visualize_hide_fanin", "111", FALSE,
      GLmake_arrow(vstate_handle_tp,
                   GLmake_arrow(GLmake_string(),
                                GLmake_arrow(GLmake_int(), vstate_handle_tp))),
...
static void push_undo_point(vstate_ptr vp) {
  ...
  ilist_ptr *ipp;
  FOR_BUF(&(vp->anon_buf), ilist_ptr, ipp) { push_buf(&(copy->anon_buf), ipp); }
  copy->next = vp->old_versions;
  vp->old_versions = copy;
}

static void pop_undo_point(vstate_ptr vp) {
  if (vp->old_versions == NULL)
    return;
...
static bool create_event_buffer(ste_ptr ste, buffer *ebufp, g_ptr wl, g_ptr ant,
  ...
  for (g_ptr cur = ant; !IS_NIL(cur); cur = GET_CONS_TL(cur)) {
    ...
    if (RCnotify_OK_A_failures && (nbr_errors_reported < RCmax_nbr_errors) &&
        c_NEQ(okA, c_ONE)) {
  ...
  for (g_ptr cur = cons; !IS_NIL(cur); cur = GET_CONS_TL(cur)) {
    ...
    if (RCnotify_OK_C_failures && (nbr_errors_reported < RCmax_nbr_errors) &&
        c_NEQ(okC, c_ONE)) {
...
static void record_trace(int idx, int time, gbv H, gbv L) {
  ...
  if (RCverbose_ste_run) {
    FP(sim_fp, "Node %s {%d} at time %d changes to: ", idx2name(idx), idx,
       time);
...
static void mark_ste_fn(pointer p) {
  ...
  formula *fp;
  FOR_BUF(weakening_bufp, formula, fp) { B_Mark(*fp); }
  event_ptr *epp;
...
static string op2str(ncomp_ptr cp) {
  ...
  if (op == op_X)
    ...
    return (strtemp("op_VAR"));
  else if (op == op_AND)
    return (strtemp("op_AND"));
  else if (op == op_LAT_LEQ)
    return (strtemp("op_LAT_LEQ"));
  else if (op == op_OR)
    return (strtemp("op_OR"));
  else if (op == op_NOT)
    return (strtemp("op_NOT"));
  else if (op == op_EQ)
    return (strtemp("op_EQ"));
  else if (op == op_GR)
    return (strtemp("op_GR"));
  else if (op == op_ADD)
    return (strtemp("op_ADD"));
  else if (op == op_SUB)
    return (strtemp("op_SUB"));
  else if (op == op_MUL)
    return (strtemp("op_MUL"));
  else if (op == op_ITE)
    return (strtemp("op_ITE"));
  else if (op == op_DIV)
    return (strtemp("op_DIV"));
  else if (op == op_MOD)
    return (strtemp("op_MOD"));
  else if (op == op_SHL)
    return (strtemp("op_SHL"));
  else if (op == op_SHR)
    return (strtemp("op_SHR"));
  else if (op == op_ASHR)
    return (strtemp("op_ASHR"));
  else if (op == op_SX) {
    ...
    return (strtemp("op_WIRE"));
  else if (op == op_MEM_READ) {
    Sprintf(buf, "op_MEM_READ %d %d %d", cp->arg.mem.addr_size,
            cp->arg.mem.data_size, cp->arg.mem.lines);
    return (strtemp(buf));
  } else if (op == op_MEM_WRITE) {
...
static void print_nodes(odests fp, fsm_ptr fsm) {
  ...
  FOR_BUF(&(fsm->nodes), nnode_rec, np) {
    int start = np->vec->map->from;
    string nname = get_real_name(np->vec, np->idx - start);
    FP(fp, "  %d %s composite:%d fanouts: [", np->idx, nname, np->composite);
    char sep = ' ';
    for (idx_list_ptr ip = np->fanouts; ip != NULL; ip = ip->next) {
...
static void print_composites(odests fp) {
  ...
  FOR_BUF(compositesp, ncomp_rec, cp) {
    FP(fp, " %3d %10s sz:%d rank:%d pdel:%d ", cnt++, op2str(cp), cp->size,
       cp->rank, cp->phase_delay);
    FP(fp, "inps: ");
    char *sep = strtemp("[");
...
static string fsm2str_fn(pointer p) {
  ...
  if (RCverbose_fsm_print) {
    ...
    FP(stdout_fp, "FSM for %s with %d nodes and %d composites\n", fsm->top_name,
       COUNT_BUF(&(fsm->nodes)) - 4, COUNT_BUF(&(fsm->composites)));
  }
  pop_fsm();
  return s_fsm;
}
...
static string compute_sha256_signature(fsm_ptr fsm) {
  ...
  FOR_BUF(&(fsm->nodes), nnode_rec, np) {
    ...
    SHA256_printf(sha, "  %d %s %s %d composite:%d fanouts: [", np->idx,
                  vp->hierarchy, vp->local_name, np->idx - start + 1,
                  np->composite);
    char sep = ' ';
    for (idx_list_ptr ip = np->fanouts; ip != NULL; ip = ip->next) {
  ...
  int cnt = 0;
  FOR_BUF(compositesp, ncomp_rec, cp) {
    SHA256_printf(sha, " %3d %s sz:%d rank:%d pdel:%d ", cnt++, op2str(cp),
                  cp->size, cp->rank, cp->phase_delay);
...
size_start:
  ...
  if (is_W_CONST(we, &sz, &ai)) {
    return sz;
  }
  if (is_W_NAMED_CONST(we, &s, &sz, &ai)) {
    return sz;
  }
  if (is_W_VAR(we, &sz, &s)) {
    return sz;
  }
  if (is_W_EXPLICIT_VAR(we, &sz, &s)) {
    return sz;
  }
  ...
  int d_sz, a_sz, lines;
  if (is_W_MEM_READ(we, &d_sz, &lines, &a_sz, &l, &r)) {
    return d_sz;
  }
...
static void add_fanout(nnode_ptr np, int comp_idx) {
  ...
  if (find_hash(&node_comp_pair_tbl, &nc) != NULL)
    return;
  idx_list_rec ir;
  ir.idx = comp_idx;
  ir.next = np->fanouts;
...
static ilist_ptr map_vector(hash_record *vtblp, string hier, string name,
  ...
  vec_list_ptr vl =
      Expand_vector(&vec_list_rec_mgr, vec_rec_mgrp, range_rec_mgrp, vp);
...
static bool inside(int i, int upper, int lower) {
  if (upper < lower && upper <= i && i <= lower)
    return TRUE;
  if (upper >= lower && upper >= i && i >= lower)
    return TRUE;
  return FALSE;
...
static ilist_ptr append_range(ilist_ptr l, int i_from, int i_to) {
  ...
  if ((cur_single || cur_up) && ((cur->to + 1) == i_from) &&
      (new_up || new_single)) {
    cur->to = i_to;
    return l;
  }
  if ((cur_single || cur_down) && ((cur->to - 1) == i_from) &&
      (new_down || new_single)) {
    cur->to = i_to;
    return l;
  }
...
static void sub_fn(int size, gbv *av, gbv *bv, gbv *resv) {
  ...
  FROM_LSB_TO_MSB(size, i) {
    ...
    gbv bH = *(bv + 2 * i + 1);
    gbv bL = *(bv + 2 * i);
    difH = c_OR(c_AND(c_OR(c_AND(aH, bL), c_AND(aL, bH)), cinL),
                c_AND(c_AND(c_OR(aL, bH), c_OR(aH, bL)), cinH));
    difL = c_AND(c_OR(c_AND(c_OR(aL, bH), c_OR(aH, bL)), cinH),
                 c_OR(c_OR(c_AND(aH, bL), c_AND(aL, bH)), cinL));
    cinH = c_OR(c_OR(c_AND(aH, bH), c_AND(aH, cinH)), c_AND(bH, cinH));
    cinL = c_AND(c_AND(c_OR(aL, bL), c_OR(aL, cinL)), c_OR(bL, cinL));
    *(resv + 2 * i) = difH;
    *(resv + 2 * i + 1) = difL;
  }
}
...
static void ITE_fn(int size, gbv cH, gbv cL, gbv *av, gbv *bv, gbv *resv) {
  FROM_MSB_TO_LSB(size, i) {
    ...
    gbv bH = *(bv + 2 * i);
    gbv bL = *(bv + 2 * i + 1);
    *(resv + 2 * i) = c_OR(c_OR(c_AND(aH, bH), c_AND(cH, aH)), c_AND(cL, bH));
    *(resv + 2 * i + 1) =
        c_AND(c_AND(c_OR(aL, bL), c_OR(cL, aL)), c_OR(cH, bL));
  }
}
...
static void op_ADD(ncomp_ptr op) {
  ...
  FROM_LSB_TO_MSB(op->size, i) {
    ...
    gbv bH = INP_H(i + op->size);
    gbv bL = INP_L(i + op->size);
    sumH = c_OR(c_AND(c_OR(c_AND(aH, bL), c_AND(aL, bH)), cinL),
                c_AND(c_AND(c_OR(aL, bH), c_OR(aH, bL)), cinH));
    sumL = c_AND(c_OR(c_AND(c_OR(aL, bH), c_OR(aH, bL)), cinH),
                 c_OR(c_OR(c_AND(aH, bL), c_AND(aL, bH)), cinL));
    cinH = c_OR(c_OR(c_AND(aH, bH), c_AND(aH, cinH)), c_AND(bH, cinH));
    cinL = c_AND(c_AND(c_OR(aL, bL), c_OR(aL, cinL)), c_OR(bL, cinL));
    OUT_H(i) = sumH;
...
static void op_ITE(ncomp_ptr op) {
  ...
  FROM_MSB_TO_LSB(op->size, i) {
    ...
    if (RCaccurate_ite_comp) {
      OUT_H(i) = c_OR(c_OR(c_AND(aH, bH), c_AND(cH, aH)), c_AND(cL, bH));
      OUT_L(i) = c_AND(c_AND(c_OR(aL, bL), c_OR(cL, aL)), c_OR(cH, bL));
    } else {
...
static void op_MEM_READ(ncomp_ptr op) {
  ...
  for (int line = 0; line < lines; line++) {
    ...
    FROM_LSB_TO_MSB(dsz, i) {
      ...
      gbv oldL = OUT_L(i);
      gbv resH =
          c_OR(c_OR(c_AND(newH, oldH), c_AND(selH, newH)), c_AND(selL, oldH));
      gbv resL =
          c_AND(c_AND(c_OR(newL, oldL), c_OR(selL, newL)), c_OR(selH, oldL));
      OUT_H(i) = resH;
      OUT_L(i) = resL;
    }
  }
...
static bool traverse_pexlif(hash_record *parent_tblp, g_ptr p, string hier,
  ...
  string name;
  bool leaf;
  if (!is_PINST(p, &name, &attrs, &leaf, &fa_inps, &fa_outs, &internals,
                &content)) {
    Fail_pr("Not a PINST where expected");
    return FALSE;
  }
  attrs =
      Make_CONS_ND(Make_CONS_ND(Make_STRING_leaf(wastrsave(&strings, "module")),
  ...
  vis_ptr vp = NULL;
  if (!top_level && (RCaccurate_hierachy_visualization ||
  ...
  if (top_level) {
    ...
    for (g_ptr l = fa_inps; !IS_NIL(l); l = GET_CONS_TL(l)) {
      ...
      if (RC_print_warnings_toplevel_fa_mismatch &&
        ...
        FP(warning_fp, "Actual != formal (%s) for top-level pexlif input. ",
           fname);
      ...
      ilist_ptr il =
          declare_vector(&vinfo_tbl, hier, fname, FALSE, NULL, value_list);
      FOREACH_NODE(nd, il) {
        nnode_ptr np = (nnode_ptr)M_LOCATE_BUF(nodesp, nd);
    ...
    for (g_ptr l = fa_outs; !IS_NIL(l); l = GET_CONS_TL(l)) {
      ...
      if (RC_print_warnings_toplevel_fa_mismatch &&
          (List_length(GET_SND(pair)) != 1 ||
           !STREQ(fname, GET_STRING(GET_CONS_HD(GET_SND(pair)))))) {
        FP(warning_fp, "Actual != formal (%s) for top-level pexlif output. ",
           fname);
        FP(warning_fp, "Actual ignored\n");
      }
    ...
    for (g_ptr l = fa_inps; !IS_NIL(l); l = GET_CONS_TL(l)) {
      ...
      while (!IS_NIL(acts)) {
        ...
        if (tmp == NULL) {
          ...
          tmp = declare_vector(&vinfo_tbl, hier, actual, FALSE, NULL, s_ES);
        }
    ...
    if ((match = strstr(vp->pfn, "draw_hfl ")) != NULL) {
      ...
      *(res + len) = 0;
      if ((strlen(res) + 20 + strlen(res + len + 1)) > 4000) {
        ...
        sprintf(buf, "%s_code %d %d %s", res, nbr_inputs, inst_cnt,
                res + len + 1);
      }
  ...
  if (is_P_HIER(content, &children)) {
    ...
    if (max_depth != 0 && find_hash(&bb_tbl, ihier_buf) == NULL) {
      ...
      for (g_ptr cl = children; !IS_NIL(cl); cl = GET_CONS_TL(cl)) {
        Sprintf(ep, "i%d/", inst++);
        if (!traverse_pexlif(&vinfo_tbl, GET_CONS_HD(cl), ihier_buf, FALSE,
                             draw_level, inst - 1, max_depth - 1)) {
          return FALSE;
        }
      }
    }
...
static int rank_order() {
  ...
  max_rank = 1;
  FOR_BUF(compositesp, ncomp_rec, cp) { assign_rank(1, cp); }
  return (max_rank);
}
...
static bool BDD_c_NEQ(gbv a, gbv b) { return (a.f != b.f); }

static bool c_EQ(gbv a, gbv b) { return (!c_NEQ(a, b)); }
...
static void switch_to_ints() { DIE("Not implemented yet!"); }
...
static int update_node(ste_ptr ste, int idx, gbv Hnew, gbv Lnew) {
  ...
  if (np->has_ant) {
    ...
    if (RCnotify_traj_failures && (nbr_errors_reported < RCmax_nbr_errors)) {
      gbv abort = c_NOT(c_OR(c_AND(aH, Hnew), c_AND(aL, Lnew)));
      if (c_NEQ(abort, c_ZERO)) {
        FP(warning_fp, "Warning: Antecedent failure at time %d on node %s\n",
  ...
  while (found) {
    ...
    if (!found) {
      for (g_ptr cur = fa_inps; !IS_NIL(cur); cur = GET_CONS_TL(cur)) {
        if (strncmp(GET_STRING(GET_FST(GET_CONS_HD(cur))), buf, len) == 0) {
          found = TRUE;
          break;
        }
      }
  ...
  if (index(cnst, 'x') != NULL || index(cnst, 'X') != NULL) {
    ...
    while (*s) {
      ...
      case 'X': {
        ...
        g_ptr var = mk_W_VAR(Make_INT_leaf(sz), Make_STRING_leaf(base));
    ...
    strappend(cnst);
    pfn = wastrsave(&strings, pfn);
    g_ptr res = mk_PINST(Make_STRING_leaf(pfn), Make_NIL(),
                         Make_BOOL_leaf(B_One()), Make_NIL(),
                         mk_list1(mk_pair(Make_STRING_leaf(out),
                                          mk_list1(Make_STRING_leaf(out)))),
                         Make_NIL(), mk_P_LEAF(leaf));
    push_buf(chbufp, &res);
    if (tmp != NULL)
      free(tmp);
    return out;
    ...
    g_ptr res = mk_PINST(
        Make_STRING_leaf(pfn), Make_NIL(), Make_BOOL_leaf(B_One()), Make_NIL(),
        mk_list1(
            mk_pair(Make_STRING_leaf(out), mk_list1(Make_STRING_leaf(out)))),
        Make_NIL(),
        mk_P_LEAF(mk_list1(
            mk_W_UPDATE_FN(mk_W_VAR(Make_INT_leaf(sz), Make_STRING_leaf(base)),
                           mk_W_CONST(Make_INT_leaf(sz), Make_AINT_leaf(v))))));
...
static bool is_binary_wexpr(g_ptr node, wl_op *opp, g_ptr *ap, g_ptr *bp) {
  EXTRACT(*bp);
  EXTRACT(*ap);
  if (!IS_LEAF(node) || !IS_STRING(node)) {
    return FALSE;
  }
...
static g_ptr mk_list1(g_ptr el) { return (Make_CONS_ND(el, Make_NIL())); }

static g_ptr mk_pair(g_ptr fst, g_ptr snd) { return (Make_CONS_ND(fst, snd)); }

static bool all_same(string msb, g_ptr al) {
  while (!IS_NIL(al)) {
    if (!STREQ(msb, GET_STRING(GET_CONS_HD(al)))) {
      return FALSE;
    }
    al = GET_CONS_TL(al);
  }
...
static string create_merge_component(int sz, int *ccnt, g_ptr *intsp,
  ...
  *intsp = Make_CONS_ND(Make_STRING_leaf(out), *intsp);
  ...
  if ((Get_Vector_Size(msb) == 1) && ((len == 1) || all_same(msb, acts))) {
    ...
    if (cnt > 1) {
      ...
      if (cnt == len) {
        ...
        g_ptr c_outs = mk_list1(
            mk_pair(Make_STRING_leaf(out), mk_list1(Make_STRING_leaf(out))));
        string i1 = wastrsave(&strings, "i1");
        g_ptr c_inps = mk_list1(
            mk_pair(Make_STRING_leaf(i1), mk_list1(Make_STRING_leaf(msb))));
        g_ptr rhs = mk_W_SX(Make_INT_leaf(sz),
                            mk_W_VAR(Make_INT_leaf(1), Make_STRING_leaf(i1)));
        g_ptr lhs = mk_W_VAR(Make_INT_leaf(sz), Make_STRING_leaf(base));
        g_ptr c_content = mk_P_LEAF(mk_list1(mk_W_UPDATE_FN(lhs, rhs)));
        g_ptr res =
            mk_PINST(Make_STRING_leaf(pfn), Make_NIL(), Make_BOOL_leaf(B_One()),
                     c_inps, c_outs, c_ints, c_content);
        push_buf(chbufp, &res);
        return (out);
      } else {
        // SX of vector
        ...
        g_ptr c_inps = mk_list1(mk_pair(Make_STRING_leaf(i1), base_inp));
        g_ptr rhs =
            mk_W_SX(Make_INT_leaf(sz), mk_W_VAR(Make_INT_leaf(sz - cnt + 1),
                                                Make_STRING_leaf(w1)));
        g_ptr lhs = mk_W_VAR(Make_INT_leaf(sz), Make_STRING_leaf(base));
        g_ptr c_content = mk_P_LEAF(mk_list1(mk_W_UPDATE_FN(lhs, rhs)));
        g_ptr res =
  ...
  g_ptr c_ints = Make_NIL();
  g_ptr c_outs =
      mk_list1(mk_pair(Make_STRING_leaf(out), mk_list1(Make_STRING_leaf(out))));
  int inp_cnt = 1;
  g_ptr cats = Make_NIL();
  ...
  while (!IS_NIL(acts)) {
    ...
    if (*act == '0') {
      csz = strlen(act) - 2;
      string t =
          create_constant(csz, ccnt, *intsp, fa_inps, fa_outs, act, chbufp);
...
static g_ptr clean_pexlif_ios(g_ptr node) {
  ...
  for (g_ptr cur = new_children; !IS_NIL(cur); cur = GET_CONS_TL(cur)) {
    ...
    for (g_ptr inp = cinps; !IS_NIL(inp); inp = GET_CONS_TL(inp)) {
      ...
      *new_pair = *pair;
      ...
      if (len > 1) {
        ...
        string t = create_merge_component(sz, &cur_cnt, &ints, inps, outs, acts,
                                          len, &ch_buf);
        ...
        if (*act == '0') {
          ...
          int sz = Get_Vector_Size(fname);
          string t =
              create_constant(sz, &cur_cnt, ints, inps, outs, act, &ch_buf);
    ...
    SET_CONS_HD(
        cur, mk_PINST(cnm, cattrs, cleaf, new_cinps, couts, cints, ccontent));
  }
  g_ptr *gpp;
  FOR_BUF(&ch_buf, g_ptr,
          gpp){APPEND1(new_children_tl, *gpp)} free_buf(&ch_buf);
...
static void allocate_ste_buffers(fsm_ptr fsm) {
  ...
  *(cur_buf + 3) = c_ZERO; // !1
  *(cur_buf + 4) = c_ONE;
  *(cur_buf + 5) = c_ONE; // !X (bottom)
  *(cur_buf + 6) = c_ZERO;
  *(cur_buf + 7) = c_ZERO; // !T (top)
}
        ...
        if ((type != use_bexprs) && RCnotify_check_failures &&
          ...
          if (c_NEQ(ok, c_ONE)) {
            ...
            if (print_failures) {
              ...
              gbv bad = c_AND(
                  c_OR(c_AND(chkH, c_NOT(curH)), c_AND(c_NOT(chkH), curH)),
                  c_OR(c_AND(chkL, c_NOT(curL)), c_AND(c_NOT(chkL), curL)));
              if (c_NEQ(bad, c_ZERO)) {
                ...
                gbv bad =
                    c_OR(c_AND(c_NOT(chkH), curH), c_AND(c_NOT(chkL), curL));
                c_Print(err_fp, bad, -1);
              }
              FP(err_fp, "\n");
            }
...
void dbg_dump_buf(string msg, gbv *buf) {
  FP(err_fp, "\n-----%s------\n ", msg);
...
void DBG_print_vis_io_ptr(vis_io_ptr vp) {
...
static vis_ptr get_vis_info_at_level(vis_list_ptr vlp, int level) {
  if (level < 0) {
    ...
    while (vlp && vlp->vp->draw_level != level) {
      vlp = vlp->next;
    }
    if (vlp == NULL)
      return NULL;
...
static void gen_get_trace(g_ptr redex, value_type type) {
  ...
  if (ste->type != type) {
    string msg = Fail_pr("Asking for %s values when STE run created %ss",
                         value_type2string(type), value_type2string(ste->type));
    MAKE_REDEX_FAILURE(redex, msg);
    return;
  }
  string node = GET_STRING(g_node);
  ...
  for (trace_event_ptr te = tp->events; te != NULL; te = te->next) {
    g_ptr event =
        Make_CONS_ND(Make_INT_leaf(te->time),
                     Make_CONS_ND(Make_GBV_leaf(te->H), Make_GBV_leaf(te->L)));
    APPEND1(tail, event);
  }
  pop_fsm();
  pop_ste();
...
static void gen_get_ste_result(g_ptr redex, value_type type) {
  ...
  if (strcmp(flag, "antOK") == 0) {
    ...
    MAKE_REDEX_FAILURE(redex,
                       Fail_pr("Unknown type (%s) in (b)get_ste_result", flag));
    return;
  }
