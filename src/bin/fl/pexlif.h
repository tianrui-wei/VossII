...
// destructors.
void destr_PINST(g_ptr node, g_ptr *namep, g_ptr *attrsp, g_ptr *leafp,
                 g_ptr *fa_inpsp, g_ptr *fa_outsp, g_ptr *internalsp,
                 g_ptr *contentp);
bool destr_MEM(g_ptr node, int *a_szp, int *linesp, int *d_szp);
// constructors.
g_ptr mk_PINST(g_ptr name, g_ptr attrs, g_ptr leaf, g_ptr fa_inps,
...
// 'is' tests.
bool is_PINST(g_ptr node, string *namep, g_ptr *attrsp, bool *leafp,
              g_ptr *fa_inpsp, g_ptr *fa_outsp, g_ptr *internalsp,
              g_ptr *contentp);
...
bool is_W_UPDATE_NAMED_SLICE(g_ptr node, g_ptr *basep, string *namep,
                             g_ptr *idxlistp, g_ptr *ep);
bool is_W_CAT(g_ptr node, g_ptr *listp);
bool is_W_MEM_READ(g_ptr node, int *a_szp, int *linesp, int *d_szp, g_ptr *memp,
                   g_ptr *addrp);
bool is_W_MEM_WRITE(g_ptr node, int *a_szp, int *linesp, int *d_szp,
...
#define FORMAL_OF_CONS(fa) GET_STRING(GET_FST(GET_CONS_HD(fa)))

#define ACTUAL_OF_CONS(fa) GET_STRING(GET_CONS_HD(fa))

#define INTERNAL_OF_CONS(is) GET_STRING(GET_CONS_HD(is))

#define FOREACH_FORMAL(vec, fa)                                                \
  for (g_ptr li = fa; !IS_NIL(li) && (vec = FORMAL_OF_CONS(li), TRUE);         \
       li = GET_CONS_TL(li))

#define FOREACH_ACTUAL(vec, fa)                                                \
  for (g_ptr li = fa; !IS_NIL(li); li = GET_CONS_TL(li))                       \
...
#define SCAN_HASH(hp, bpp, k, d)                                               \
  ...
  for (bucket_ptr chp = *bpp;                                                  \
       chp != NULL && (k = chp->key, TRUE) && (d = chp->data, TRUE);           \
       chp = chp->next)
