...
unint pint_hash(pointer p, unint n) { return (((long unsigned int)p) % n); }

bool pint_equ(pointer p1, pointer p2) {
...
bool write_pointer(FILE *fp, pointer p) {
  ...
  WR_DBG1("pointer");
  fprintf(fp, "%p\n", p);
  if (find_hash(&objs_saved, p) != NULL) {
    return FALSE;
  }
...
void write_pint(FILE *fp, void *pi) {
  int i = PTR2INT(pi);
  WR_DBG1("int");
  fprintf(fp, "%d\n", i);
}

void read_pint(FILE *fp, int *ip) {
  int i;
  RD_DBG1("int");
  if (fscanf(fp, "%d\n", &i) != 1)
    Rprintf("Corrupted int object");
...
void write_formula(FILE *fp, formula f) {
  WR_DBG1("formula");
  int i = Save_get_bool_idx(f);
  write_int(fp, i);
}

void read_formula(FILE *fp, formula *pf) {
  int idx;
  RD_DBG1("formula");
  read_int(fp, &idx);
...
void write_pformula(FILE *fp, void *pf) {
...
void read_pformula(FILE *fp, formula *pf) {
  int idx;
  RD_DBG1("pformula");
  read_int(fp, &idx);
  *pf = Load_get_bool_from_idx(idx);
...
void write_bexpr(FILE *fp, bexpr f) {
  WR_DBG1("bexpr");
  int i = Save_get_bexpr_idx(f);
  write_int(fp, i);
}

void read_bexpr(FILE *fp, bexpr *pf) {
  int idx;
  RD_DBG1("bexpr");
  read_int(fp, &idx);
...
void read_mgr(FILE *fp, rec_mgr_ptr mp, void (*read_rec)(FILE *fp, pointer p)) {
...
void read_buf(FILE *fp, buffer_ptr bp, void (*read_item)(FILE *fp, pointer p)) {
  ASSERT(unserialize_in_process);
  int cnt;
  RD_DBG0("{R buf");
  read_int(fp, &cnt);
...
void read_ustr_mgr(FILE *fp, ustr_mgr *usmp) {
  int cnt;
  RD_DBG1("ustr_mgr");
  read_int(fp, &cnt);
  for (int i = 0; i < cnt; i++) {
...
static unint g_ptr_hash(pointer p, unint n) { return (G_rec_hash(p, n)); }

static bool g_ptr_equ(pointer p1, pointer p2) { return (G_rec_equ(p1, p2)); }
