...
static int float_sha256_fn(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                           pointer a);
...
static float_ptr get_float_rec(double v) {
  if (free_list != NULL) {
...
static bool double2arbi(double d, arbi_T *resp) {
  int exp;
  switch (fpclassify(d)) {
  case FP_NAN:
    return FALSE;
...
static void mark_float_fn(pointer p) {
  float_ptr fp = (float_ptr)p;
  fp->flag = 1;
  return;
}

static void sweep_float_fn(void) {
  float_ptr fp;
  free_list = NULL;
...
static void save_float_fn(FILE *fp, pointer p) {
...
static pointer load_float_fn(FILE *fp) {
  union {
    double d;
    unsigned char bytes[sizeof(double)];
  } u;
...
static string float2str_fn(pointer p) {
  char buf[20];
  float_ptr fp = (float_ptr)p;
...
static formula float_eq_fn(pointer p1, pointer p2, bool identical) {
  (void)identical;
...
static void str2float(g_ptr redex) {
...
static void int2float(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
static void float2str(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void gen_double_binop(double (*binop)(double a1, double a2),
                             g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1, arg2;
...
static void gen_double_unop(double (*unop)(double a), g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg;
...
static void gen_double_relop(bool (*relop)(double a1, double a2), g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1, arg2;
  EXTRACT_2_ARGS(redex, arg1, arg2)
...
static void gen_convert_to_int(double (*cnvop)(double a), g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg;
  EXTRACT_1_ARG(redex, arg)
...
void Float_Init() {
  ...
  free_list = NULL;
  float_oidx = Add_ExtAPI_Object("float_handle", mark_float_fn, sweep_float_fn,
                                 save_float_fn, load_float_fn, float2str_fn,
                                 float_eq_fn, NULL, NULL, float_sha256_fn);
...
void Float_Install_Functions() {
  ...
  Add_ExtAPI_Function("int2float", "1", FALSE,
                      GLmake_arrow(GLmake_int(), float_handle_tp), int2float);
  ...
  Add_ExtAPI_Function(
      "fgt", "11", FALSE,
      GLmake_arrow(float_handle_tp,
                   GLmake_arrow(float_handle_tp, GLmake_bool())),
      float_gt);
  ...
  Add_ExtAPI_Function("floor", "1", FALSE,
                      GLmake_arrow(float_handle_tp, GLmake_int()), f_floor);

  Add_ExtAPI_Function("ceil", "1", FALSE,
                      GLmake_arrow(float_handle_tp, GLmake_int()), f_ceil);

  Add_ExtAPI_Function("round", "1", FALSE,
                      GLmake_arrow(float_handle_tp, GLmake_int()), f_round);

  Add_ExtAPI_Function("sqrt", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), fsqrt);

  Add_ExtAPI_Function("exp", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), fexp);

  Add_ExtAPI_Function("log", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), flog);

  Add_ExtAPI_Function("log10", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), flog10);

  Add_ExtAPI_Function("sin", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), fsin);

  Add_ExtAPI_Function("cos", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), fcos);

  Add_ExtAPI_Function("tan", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), ftan);

  Add_ExtAPI_Function("asin", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), fasin);

  Add_ExtAPI_Function("acos", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), facos);

  Add_ExtAPI_Function("atan", "1", FALSE,
                      GLmake_arrow(float_handle_tp, float_handle_tp), fatan);
}
