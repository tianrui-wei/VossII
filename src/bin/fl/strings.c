...
static void buffer_vectors_list(vec_list_ptr vs, buffer_ptr vec_buf,
                                bool range_canonical);
static void buffer_vectors_fl(g_ptr r, buffer_ptr vec_buf,
                              bool range_canonical);
...
static void extract_vectors_fl(g_ptr list, g_ptr res, bool non_contig_vecs,
                               bool range_canonical);
static int vec_size(vec_ptr vec);
static string show_non_contig_vector(tstr_ptr tstrings, vec_ptr vp);
static sname_list_ptr show_non_contig_vectors(rec_mgr *sname_list_mgrp,
                                              tstr_ptr tstrings,
                                              vec_list_ptr vlp);
static sname_list_ptr show_contig_vector(rec_mgr *sname_list_mgrp,
                                         tstr_ptr tstrings, vec_ptr vp);
static sname_list_ptr show_contig_vectors(rec_mgr *sname_list_mgrp,
                                          tstr_ptr tstrings, vec_list_ptr vp);
static sname_list_ptr show_merge_list(rec_mgr *sname_list_mgrp,
                                      tstr_ptr tstrings, merge_list_ptr mlp,
...
vec_ptr Split_vector_name(ustr_mgr *string_mgrp, rec_mgr *vector_mgrp,
...
sname_list_ptr Show_vector(rec_mgr *sname_list_mgrp, vec_ptr vec,
                           bool non_contig_vec) {
  tstr_ptr tstrings = new_temp_str_mgr();
...
sname_list_ptr Show_vectors(rec_mgr *sname_list_mgrp, vec_list_ptr vecs,
                            bool non_contig_vecs) {
  /* if(vecs == NULL) { */
...
bool range_equ(pointer k1, pointer k2) { return range_cmp(k1, k2) == 0; }
...
static void tcl2list(g_ptr redex) {
...
static void file_fullname(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  string tmp = realpath(GET_STRING(r), path_buf);
  if (tmp == NULL) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("Cannot find file %s\n", GET_STRING(r)));
    return;
  }
...
static void do_strlen(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void do_strcmp(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void str_is_prefix(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void str_is_suffix(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void str_is_substr(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void str_match(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void str_split(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void md_split_vector(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  begin_vector_ops();
  MAKE_REDEX_NIL(redex);
  ...
  while (vp != NULL) {
    ...
    if (vp->type == TXT) {
      ...
      cur =
          Make_CONS_ND(cur, Make_STRING_leaf(wastrsave(&strings, vp->u.name)));
    } else {
      // Range
      ...
      while (rp != NULL) {
        g_ptr pair =
            Make_PAIR_ND(Make_INT_leaf(rp->upper), Make_INT_leaf(rp->lower));
        APPEND1(rtail, pair);
        rp = rp->next;
      }
    }
...
static void md_expand_vectors(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  begin_vector_ops();
  tstr_ptr tstrings = new_temp_str_mgr();
...
static void md_expand_vector(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  begin_vector_ops();
  tstr_ptr tstrings = new_temp_str_mgr();
...
static void md_size(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void md_sizes(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  int sz = 0;
  g_ptr nl = r;
...
static void md_extract_vectors(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void extract_vectors(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void md_merge_vectors(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void merge_vectors(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void string_lastn(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void string_butlastn(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  string s = GET_STRING(GET_APPLY_RIGHT(l));
  int cnt = GET_INT(r);
...
static void string_strstr(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void lift_node_name_cmp(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_s1, g_s2;
...
static void string_substr(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_s, g_f, g_c;
  ...
  if (loc < 1) {
    string msg = Fail_pr(
        "Start position for str_substr before start of string (%d)", loc);
  ...
  if ((loc + cnt - 1) > len) {
    string msg = Fail_pr("loc (%d) plus cnt (%d) greater than length of string "
                         "(%d) in str_substr",
                         loc, cnt, len);
    MAKE_REDEX_FAILURE(redex, msg);
    DEC_REF_CNT(l);
...
static void string_trim(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_s;
...
static void string_gen_trim(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_l, g_r, g_s;
  ...
  if (lloc == NULL) {
    string msg =
        Fail_pr("trim: Cannot find prefix (%s) in string (%s)", pref, s);
    MAKE_REDEX_FAILURE(redex, msg);
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    return;
  ...
  if (last == NULL) {
    string msg =
        Fail_pr("trim: Cannot find suffix (%s) in string (%s)", suff, s);
    MAKE_REDEX_FAILURE(redex, msg);
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    return;
...
static void string_str_cluster(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
  ...
  if ((len % sz) != 0) {
    MAKE_REDEX_FAILURE(
        redex, Fail_pr("str_cluster: string length not a multiple of %d", sz));
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    return;
  }
...
static void get_vector_signature(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_s;
  EXTRACT_1_ARG(redex, g_s);
...
void Strings_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      "tcl2list", "1", FALSE,
      GLmake_arrow(GLmake_string(), GLmake_list(GLmake_string())), tcl2list);
  ...
  Add_ExtAPI_Function("strlen", "1", FALSE,
                      GLmake_arrow(GLmake_string(), GLmake_int()), do_strlen);
  ...
  Add_ExtAPI_Function(
      "str_is_prefix", "11", FALSE,
      GLmake_arrow(GLmake_string(),
                   GLmake_arrow(GLmake_string(), GLmake_bool())),
      str_is_prefix);

  Add_ExtAPI_Function(
      "str_is_suffix", "11", FALSE,
      GLmake_arrow(GLmake_string(),
                   GLmake_arrow(GLmake_string(), GLmake_bool())),
      str_is_suffix);

  Add_ExtAPI_Function(
      "str_is_substr", "11", FALSE,
      GLmake_arrow(GLmake_string(),
                   GLmake_arrow(GLmake_string(), GLmake_bool())),
      str_is_substr);
  ...
  Add_ExtAPI_Function("md_split_vector", "1", FALSE,
                      GLmake_arrow(GLmake_string(), GLmake_list(vec_info_tp)),
                      md_split_vector);

  Add_ExtAPI_Function(
      "md_expand_vector", "1", FALSE,
      GLmake_arrow(GLmake_string(), GLmake_list(GLmake_string())),
      md_expand_vector);

  Add_ExtAPI_Function(
      "md_expand_vectors", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_string()), GLmake_list(GLmake_string())),
      md_expand_vectors);

  Add_ExtAPI_Function("md_size", "1", FALSE,
                      GLmake_arrow(GLmake_string(), GLmake_int()), md_size);

  Add_ExtAPI_Function("md_sizes", "1", FALSE,
                      GLmake_arrow(GLmake_list(GLmake_string()), GLmake_int()),
                      md_sizes);

  Add_ExtAPI_Function(
      "md_extract_vectors", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_string()), GLmake_list(GLmake_string())),
      md_extract_vectors);

  Add_ExtAPI_Function(
      "extract_vectors", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_string()), GLmake_list(GLmake_string())),
      extract_vectors);

  Add_ExtAPI_Function(
      "md_merge_vectors", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_string()), GLmake_list(GLmake_string())),
      md_merge_vectors);

  Add_ExtAPI_Function(
      "merge_vectors", "1", FALSE,
      GLmake_arrow(GLmake_list(GLmake_string()), GLmake_list(GLmake_string())),
      merge_vectors);
...
static range_ptr merge_indices(range_ptr indices) {
  ...
  while (cur) {
    if ((tail->upper >= tail->lower) && (cur->upper >= cur->lower) &&
      ...
      cur = cur->next;
      tail->next = cur;
    } else if ((tail->upper <= tail->lower) && (cur->upper <= cur->lower) &&
               ((tail->lower + 1) == cur->upper)) {
      tail->lower = cur->lower;
      cur = cur->next;
      tail->next = cur;
...
static string get_base_name(vec_ptr vp) {
  string name = strtemp("");
  if (vp->type != TXT) {
    return (uStrsave(lstringsp, name));
  }
  ...
  while (alts > 1 && not_changed < alts) {
    ...
    if (mp->name_signature != mp->next->name_signature) {
      ...
      if (v1 == NULL) {
        ...
        v2 = v2->next;
        while (v1 &&
               (v1->type == TXT || same_range(v1->u.ranges, v2->u.ranges))) {
          v1 = v1->next;
          v2 = v2->next;
        }
        if (v1 != NULL) {
          ...
          if ((r1->upper >= r1->lower) && (r2->upper >= r2->lower) &&
            ...
            not_changed = 0;
            alts--;
          } else if ((r1->upper <= r1->lower) && (r2->upper <= r2->lower) &&
                     ((r1->lower + 1) == r2->upper)) {
...
static sname_list_ptr show_contig_vectors(rec_mgr *sname_list_mgrp,
                                          tstr_ptr tstrings, vec_list_ptr vlp) {
  sname_list_ptr res = NULL, *tail_ptr = &res;
  for (; vlp != NULL; vlp = vlp->next) {
...
static bool inside(int lrange, int rrange, int test) {
  if (lrange <= rrange) {
...
static bool colliding(vec_ptr v1, vec_ptr v2) {
  if (v1 == v2)
    return TRUE;
  if (v1 == NULL || v2 == NULL)
    return FALSE;
...
static range_ptr copy_range(rec_mgr *range_mgr_ptr, range_ptr old) {
  if (old == NULL)
    return NULL;
