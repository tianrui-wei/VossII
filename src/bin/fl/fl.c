...
string Get_DIR(string file_fullname) {
...
FILE *Tryopen(string name, string mode, string *fullnamep) {
  FILE *ret;
  string new_name;
...
FILE *file_mktemp(const char *base) {
...
bool Mk_output_file_in_tmp_dir(string prefix, FILE **fpp, string *filename) {
  FILE *fp;
  if (*prefix == '/') {
...
static string read_line(FILE *fp, bool once) {
  while (1) {
...
int fl_main(int argc, char *argv[]) {
  ...
  while (argc > 1 && argv[1][0] == '-') {
    ...
    if (strcmp(argv[1], "--dummy_graph_compare") == 0) {
      ...
      argc -= 2;
      argv += 2;
    } else if (strcmp(argv[1], "--start_comparing") == 0) {
      ...
      print_help();
      exit(0);
    } else if (strcmp(argv[1], "--read_input_from_file") == 0) {
      input_file_for_cmds = argv[2];
      argc -= 2;
      argv += 2;
    } else if (strcmp(argv[1], "--write_output_to_file") == 0) {
      ...
      argc--, argv++;
    } else if ((strcmp(argv[1], "-use_stdin") == 0) ||
      ...
      argc--, argv++;
    } else if ((strcmp(argv[1], "-unbuf_stdout") == 0) ||
               (strcmp(argv[1], "--unbuf_stdout") == 0)) {
      unbuf_stdout = TRUE;
      argc--, argv++;
    } else if ((strcmp(argv[1], "-use_stdout") == 0) ||
      ...
      gui_mode = FALSE;
      argc--, argv++;
    } else if ((strcmp(argv[1], "-hide-window") == 0) ||
               (strcmp(argv[1], "--hide-window") == 0)) {
      hide_window = TRUE;
      argc--, argv++;
      ...
      argc -= 2;
      argv += 2;
    } else if (strcmp(argv[1], "-use") == 0 || strcmp(argv[1], "--use") == 0) {
      use_window = argv[2];
      argc -= 2;
      argv += 2;
  ...
  if (rand_str != NULL && !str2int(rand_str, &rand_init))
    Eprintf("-r takes an integer to initialize the random number generator\n");
  srandom(rand_init);
  Init();
  ...
  if (gui_mode) {
    ...
    if (input_file_for_cmds != NULL) {
      ...
      if (input_file_for_cmds_pid == 0) {
        ...
        execl("/usr/bin/tail", "tail", "-f", input_file_for_cmds, "--pid", pbuf,
              NULL);
      }
      close(wr_fd);
    ...
    if (hide_window) {
      Send_to_tcl("hide_fl_window", NULL);
    }
    ...
    {
      string cmd =
          tprintf("(_load \"%s/preamble.fl\" F) fseq ();", binary_location);
...
void Set_default_break_handler() { signal(SIGINT, break_handler); }
...
bool Ustr_equ(pointer p1, pointer p2) { return (p1 == p2); }
...
static void setup_sockets() {
  ...
  if (use_window != NULL) {
    Sprintf(buf, "wish %s/front_end.tcl %d %d %d %s -use %s &", binary_location,
            getpid(), addr, port, Voss_tmp_dir, use_window);
  } else {
    Sprintf(buf, "wish %s/front_end.tcl %d %d %d %s &", binary_location,
            getpid(), addr, port, Voss_tmp_dir);
  }
  if (system(buf) != 0) {
...
bool Send_to_tcl(string cmd, string *resp) {
  ...
  while (1) {
    ...
    if (ok != -1) {
      if (resp != NULL) {
        *resp = *((string *)M_LOCATE_BUF(&tcl_eval_result_buf, tcl_eval_depth));
      }
      ...
      if (in_string) {
        ...
        *end = 0;
        ...
        if (verbose) {
          ...
          } else if (strncmp(nb, "set_line_number ", 16) == 0) {
            string pcmd = protect(nb);
            fprintf(to_tcl_fp, "WriteInfo {%s}\n", pcmd);
          } else {
...
static void print_help() {
  ...
  P("    -hide-window		    hide the main window while still "
    "preserving X functionality\n");
  ...
  P("    -v file			    save dynamic variable order in "
    "file\n");
