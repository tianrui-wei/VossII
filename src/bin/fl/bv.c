...
static bv_ptr get_bv_rec(g_ptr l) {
  bv_ptr res;
...
static void mark_bv_fn(pointer p) {
  bv_ptr fp = (bv_ptr)p;
  fp->flag = 1;
  Mark(fp->u.l);
  return;
...
static int get_bv_length(g_ptr l) {
  int len = 0;
  while (!IS_NIL(l)) {
    len++;
    l = GET_CONS_TL(l);
...
static void save_bv_fn(FILE *fp, pointer p) {
  bv_ptr bvp = (bv_ptr)p;
  g_ptr l = bvp->u.l;
...
static pointer load_bv_fn(FILE *fp) {
  int len;
...
static bool is_symbolic(formula f) { return (f != B_One() && f != B_Zero()); }

static string bv_list2str(g_ptr l, int depth) {
...
static string bv2str_fn(pointer p) {
  bv_ptr bp = (bv_ptr)p;
  g_ptr l = bp->u.l;
  return (bv_list2str(l, -1));
}

static formula bv_eq_fn(pointer p1, pointer p2, bool identical) {
...
static int bv_sha256_fn(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                        pointer a) {
  bv_ptr bp = (bv_ptr)a;
  int res = *g_cntp;
  *g_cntp = res + 1;
...
static pointer bv_gmap_fn(gmap_info_ptr ip, pointer a) {
  bv_ptr bp = (bv_ptr)a;
  g_ptr l = bp->u.l;
  if (ip->read_only) {
...
static pointer bv_gmap2_fn(gmap_info_ptr ip, pointer a1, pointer a2) {
...
static g_ptr add_rec(bool neg, g_ptr l1, g_ptr l2, formula *coutp) {
  if (IS_NIL(l1)) {
    ASSERT(IS_NIL(l2));
...
static formula is_zero_list(g_ptr l) {
  formula zero = B_One();
  while (!IS_NIL(l)) {
...
static g_ptr negate_bv_list(g_ptr l) {
...
static g_ptr decrement_bv_list(g_ptr l) {
...
static g_ptr ite_bv_list(formula cond, g_ptr l1, g_ptr l2) {
  sx2(&l1, &l2);
  g_ptr res = Make_NIL();
  PUSH_GLOBAL_GC(l1);
  PUSH_GLOBAL_GC(l2);
...
static g_ptr abs_bv_list(g_ptr l) {
  PUSH_GLOBAL_GC(l);
...
static g_ptr mult_bv_list(g_ptr xv, g_ptr yv) {
  if (IS_NIL(yv)) {
...
static formula less_rec(g_ptr l1, g_ptr l2) {
  if (IS_NIL(l1)) {
    ASSERT(IS_NIL(l2));
    return (B_Zero());
  }
...
static formula less_than_bv_list(g_ptr l1, g_ptr l2) {
  sx2(&l1, &l2);
...
static int constant_power_of_two(g_ptr bv) {
  if (!is_scalar_list(bv)) {
    return (-1);
  }
  bool found = FALSE;
...
static void fixed_ext_bvdiv(g_ptr av, g_ptr bv, g_ptr *Qp, g_ptr *Rp) {
  ...
  while (!IS_NIL(av)) {
    ...
    for (int i = sz - 1; i >= 0; i--) {
      ...
      geq = B_Or(B_And(ri, B_Not(bi)), B_And(B_Xnor(ri, bi), geq));
    }
...
static void bv_construct(g_ptr redex) {
...
static void bv_destruct(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void bv2str(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_size(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bv_ptr bp = (bv_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void bv_var(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static arbi_T bv_list2int(g_ptr l, arbi_T *ppow) {
  if (IS_NIL(l)) {
    *ppow = Arbi_FromInt(1);
...
static void bv2int(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bv_ptr bp = (bv_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void int2bv(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
static void bv_add(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_sub(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_mul(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
  ...
  g_ptr abs_res = Make_CONS_ND(Zero, mult_bv_list(abs_av, abs_bv));
...
static void signed_bvdiv(g_ptr dv, g_ptr nv, g_ptr *Q, g_ptr *R) {
...
static void bv_ashr(g_ptr redex) {
  g_ptr arg1;
  g_ptr arg2;
  EXTRACT_2_ARGS(redex, arg1, arg2);
  int cnt = GET_INT(arg2);
  if (cnt < 0) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("bv_ashr by negative amount (%d)", cnt));
    return;
  }
  if (cnt == 0) {
...
static void bv_div(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_mod(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_NOT(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bv_ptr bp = (bv_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void bv_AND(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_XOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_XNOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_less(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_leq(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_greater(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bv_geq(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
void Bv_Init() {
  ...
  PUSH_GLOBAL_GC(One);
  bv_free_list = NULL;
  bv_oidx = Add_ExtAPI_Object("bv", mark_bv_fn, sweep_bv_fn, save_bv_fn,
                              load_bv_fn, bv2str_fn, bv_eq_fn, bv_gmap_fn,
                              bv_gmap2_fn, bv_sha256_fn);
...
void Bv_Install_Functions() {
  ...
  Add_ExtAPI_Function("bv_size", "1", FALSE,
                      GLmake_arrow(bv_handle_tp, GLmake_int()), bv_size);

  Add_ExtAPI_Function("int2bv", "1", FALSE,
                      GLmake_arrow(GLmake_int(), bv_handle_tp), int2bv);

  Add_ExtAPI_Function("bv2int", "1", FALSE,
                      GLmake_arrow(bv_handle_tp, GLmake_int()), bv2int);

  Add_ExtAPI_Function("bv_variable", "1", FALSE,
                      GLmake_arrow(GLmake_string(), bv_handle_tp), bv_var);

  Add_ExtAPI_Function(
      "bv2str", "11", FALSE,
      GLmake_arrow(GLmake_int(), GLmake_arrow(bv_handle_tp, GLmake_string())),
      bv2str);

  Add_ExtAPI_Function(
      "bv_add", "11", FALSE,
      GLmake_arrow(bv_handle_tp, GLmake_arrow(bv_handle_tp, bv_handle_tp)),
      bv_add);
  ...
  Add_ExtAPI_Function(
      "bv_ashr", "11", FALSE,
      GLmake_arrow(bv_handle_tp, GLmake_arrow(GLmake_int(), bv_handle_tp)),
      bv_ashr);
  ...
  Add_ExtAPI_Function("bv_NOT", "1", FALSE,
                      GLmake_arrow(bv_handle_tp, bv_handle_tp), bv_NOT);

  Add_ExtAPI_Function("bv_ZX", "1", FALSE,
                      GLmake_arrow(bv_handle_tp, bv_handle_tp), bv_ZX);
  ...
  Add_ExtAPI_Function(
      "bv_less", "11", FALSE,
      GLmake_arrow(bv_handle_tp, GLmake_arrow(bv_handle_tp, GLmake_bool())),
      bv_less);
