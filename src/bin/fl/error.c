...
void FP_va(odests fp, const string format, va_list arg) {
  if (fp == FILE_fp) {
...
VOID ErrMsg(const string format, ...) {
  if (file_load) {
...
VOID Rprintf(const string format, ...) {
  if (file_load) {
    FP(err_fp,
...
VOID Info_printf(const string format, ...) {
  FP(warning_fp, "---- ");
  va_list arg;
  va_start(arg, format);
...
char *Fail_pr(const string format, ...) {
  va_list arg;
  va_start(arg, format);
  vsnprintf(fbuf, FBUF_MAX_SZ, format, arg);
  va_end(arg);
...
VOID Fail_append(const string format, ...) {
  va_list arg;
  va_start(arg, format);
  vsnprintf(fbuf, FBUF_MAX_SZ, format, arg);
  va_end(arg);
...
string tprintf(const string format, ...) {
  string ret;
  va_list arg;
  va_start(arg, format);
  vsnprintf(fbuf, FBUF_MAX_SZ, format, arg);
...
string tappend(const string format, ...) {
  string ret;
  va_list arg;
  va_start(arg, format);
  vsnprintf(fbuf, FBUF_MAX_SZ, format, arg);
...
/* A printf function into strtemp() */
string gen_tprintf(tstr_ptr tp, const string format, ...) {
  string ret;
  va_list arg;
  va_start(arg, format);
  vsnprintf(fbuf, FBUF_MAX_SZ, format, arg);
...
string gen_tappend(tstr_ptr tp, const string format, ...) {
  string ret;
  va_list arg;
  va_start(arg, format);
  vsnprintf(fbuf, FBUF_MAX_SZ, format, arg);
...
VOID Wprintf(const string format, ...) {
  if (file_load) {
...
/* Debug printf */
void DBG_printf(const string format, ...) {
  va_list arg;
  va_start(arg, format);
  FP_va(err_fp, format, arg);
  va_end(arg);
