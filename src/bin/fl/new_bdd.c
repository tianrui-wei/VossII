...
static unsigned long int primes[] = {
    13,        23,        59,         113,        241,      503,
    1019,      2039,      4091,       8179,       16369,    32749,
    65521,     131063,    262139,     524269,     1048571,  2097143,
    4194287,   8388593,   16777199,   33554393,   67108859, 134217689,
    268435399, 536870879, 1073741789, 2147483629, 0};
...
static bool sop_pr_rec(odests fp, formula node, bool look_for_zero, formula f);
...
static bool truth_cover_rec(hash_record *done_tblp, buffer *var_bufp, unint idx,
                            formula b, arbi_T *resp, string *emsgp);
...
bdd_ptr f_GET_BDDP(formula f) { return (GET_BDDP(f)); }

int f_ISNOT(formula f) { return (ISNOT(f)); }

formula f_POS(formula f) { return (POS(f)); }

int f_BDD_GET_VAR(bdd_ptr f) { return (BDD_GET_VAR(f)); }

formula f_GET_LSON(bdd_ptr f) { return (GET_LSON(f)); }

formula f_GET_RSON(bdd_ptr f) { return (GET_RSON(f)); }

void BP(formula f) { B_Print(err_fp, f, -1); }

#endif

/* B_One -- Return the Boolean formula for the constant 1 */
formula B_One() { BDD_RETURN(ONE); }

/* B_Zero -- Return the Boolean formula for the constant 0 */
formula B_Zero() { BDD_RETURN(ZERO); }

/* B_Var -- Return Boolean formula for variable name */
formula B_Var(string name) {
  formula ret;
...
void Get_top_cofactor(formula b, string *varp, formula *Hp, formula *Lp) {
...
static void mark_vars(pointer key, pointer data) {
  (void)key;
...
formula B_New_unique_var(formula f) {
  char buf[64];
  bdd_ptr bp;
  bp = GET_BDDP(f);
...
formula B_Nor(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
...
formula B_Nand(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
  ASSERT(GET_BDDP(f2)->in_use);
#endif
...
formula B_And(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
  ASSERT(GET_BDDP(f2)->in_use);
#endif
...
formula B_Or(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
  ASSERT(GET_BDDP(f2)->in_use);
#endif
...
formula B_Xor(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
  ASSERT(GET_BDDP(f2)->in_use);
#endif
...
formula B_Xnor(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
  ASSERT(GET_BDDP(f2)->in_use);
#endif
...
bool B_Equal(formula f1, formula f2) {
#ifdef REF_CNT_DEBUG
  ASSERT(GET_BDDP(f1)->in_use);
  ASSERT(GET_BDDP(f2)->in_use);
#endif
...
void HL_Print(odests fp, formula H, formula L) {
  formula o, x;
  if (B_Or(L, H) != ONE) {
...
unint Get_VarCnt() { return (nbr_VarTbl); }
...
bool Save_BDDs(string filename, buffer *roots) {
  ...
  Reset_BDD_Size();
  formula *bp;
  FOR_BUF(roots, formula, bp) { B_Size(*bp); }
  ...
  bdd_save_cnt = 1;
  FOR_BUF(roots, formula, bp) { b_save(fp, *bp); }
...
void Begin_RelProd() {
  ...
  relprod_cache =
      (relprod_cache_ptr)Calloc((relprod_cache_sz) * sizeof(relprod_cache_rec));
}

void End_RelProd() { Free((pointer)relprod_cache); }
...
static bool is_next_state_var(string name) {
  int len = strlen(name);
...
void PUSH_BDD_GC(formula f) { push_buf(&bdd_gc_buf, (pointer)&f); }
...
void Get_abstract_depends(g_ptr redex, hash_record *abs_tblp, g_ptr obj) {
  Reset_BDD_Size();
...
int SHA256_bdd(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha, formula f) {
  int res;
  if (f == ZERO)
    return -1;
  if (f == ONE)
...
static void bddSignature(g_ptr redex) {
...
static void bddT(g_ptr redex) { MAKE_REDEX_BOOL(redex, B_One()); }

static void bddF(g_ptr redex) { MAKE_REDEX_BOOL(redex, B_Zero()); }

static void variable(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void bddNOT(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void bddAND(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void bddXOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
...
static void bddXNOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void bv2num(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void draw_bdds(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
  if (!gui_mode) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("draw_bdds not available in -noX mode"));
    return;
  }
...
static void top_cofactor(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
static void depends(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  Reset_BDD_Size();
...
static void bdd_profile(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  Reset_BDD_Size();
  ...
  for (unint i = 0; i < vars; i++) {
    if (B_Width(i) > 0) {
      SET_CONS_HD(cur, Make_CONS_ND(Make_STRING_leaf(Get_Var_Name(i)),
...
static void bdd_size(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
  Reset_BDD_Size();
...
static void do_substitute(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(l);
  g_ptr arg2 = GET_APPLY_RIGHT(redex);
...
static void do_truth_cover(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static bool truth_cover_rec(hash_record *done_tblp, buffer *var_bufp, unint idx,
  ...
  if (idx == COUNT_BUF(var_bufp)) {
    ...
    *emsgp = Fail_pr("Variable %s not in truth_cover list but f depends on it",
                     vp->var_name);
    return FALSE;
  }
...
void BDD_Install_Functions() {
  ...
  Add_ExtAPI_Function("variable", "1", FALSE,
                      GLmake_arrow(GLmake_string(), GLmake_bool()), variable);

  Add_ExtAPI_Function("NOT", "1", FALSE,
                      GLmake_arrow(GLmake_bool(), GLmake_bool()), bddNOT);

  Add_ExtAPI_Function(
      "AND", "1-", FALSE,
      GLmake_arrow(GLmake_bool(), GLmake_arrow(GLmake_bool(), GLmake_bool())),
      bddAND);
  Insert_infix("AND", 4);
  ...
  Add_ExtAPI_Function(
      ...
      GLmake_arrow(GLmake_bool(),
                   GLmake_arrow(GLmake_list(GLmake_bool()), GLmake_void())),
      draw_bdds);
  ...
  Add_ExtAPI_Function(
      "top_cofactor", "1", FALSE,
      GLmake_arrow(GLmake_bool(),
                   GLmake_tuple(GLmake_string(),
                                GLmake_tuple(GLmake_bool(), GLmake_bool()))),
      top_cofactor);

  typeExp_ptr tv = GLnew_tVar();
  Add_ExtAPI_Function(
      "substitute", "11", FALSE,
      GLmake_arrow(GLmake_list(GLmake_tuple(GLmake_string(), GLmake_bool())),
                   GLmake_arrow(tv, tv)),
      do_substitute);

  tv = GLnew_tVar();
  Add_ExtAPI_Function("depends", "1", FALSE,
                      GLmake_arrow(tv, GLmake_list(GLmake_string())), depends);

  tv = GLnew_tVar();
  Add_ExtAPI_Function("profile", "1", FALSE,
                      GLmake_arrow(tv, GLmake_list(GLmake_tuple(GLmake_string(),
                                                                GLmake_int()))),
                      bdd_profile);

  tv = GLnew_tVar();
  Add_ExtAPI_Function("bdd_size", "1", FALSE, GLmake_arrow(tv, GLmake_int()),
                      bdd_size);
...
static void grow_uniq_tbl(var_ptr vp) {
...
void PR_uniq_tbl(var_ptr vp) {
  ...
  for (i = 0; i < vp->sz_uniq_tbl; i++) {
    ...
    if (cur != LNULL) {
      ...
      while (cur != LNULL) {
        bp = GET_BDDP(cur);
        FP(err_fp, " (%d = %d %s %d)", cur, GET_LSON(bp), vp->var_name,
           GET_RSON(bp));
        cur = GET_NEXT(bp);
      }
      FP(err_fp, "\n");
...
static formula quantify(formula f, formula vf, int type) {
  ...
  if (ORDER(cur) > ORDER(index)) {
    ...
    if (ORDER(cur) < ORDER(index)) {
      ...
      if (type == UNIVERSAL)
        ret =
            bdd_step(AND, quantify(lson, new, type), quantify(rson, new, type));
      else
        ret =
            bdd_step(OR, quantify(lson, new, type), quantify(rson, new, type));
    }
  }
  /* Insert into cache */
  cp = get_new_cache_rec();
...
static void clean_cache_rec_fn(pointer key, pointer data) {
...
static bool sop_pr_rec(odests fp, formula node, bool look_for_zero, formula f) {
  ...
  *cur_prod = B_Var(get_var_name(node));
  ...
  if (!sop_pr_rec(fp, POS(f2lson(node)), look_for_zero ^ ISNOT(f2lson(node)),
                  f))
    return (FALSE);
  cur_prod = old;
  *(cur_prod - 1) = NOT(*(cur_prod - 1));
  if (!sop_pr_rec(fp, POS(f2rson(node)), look_for_zero ^ ISNOT(f2rson(node)),
                  f))
    return (FALSE);
  cur_prod = old;
  return (TRUE);
        ...
        if (POS(lson) != ZERO && (f2var(lson) == new_var)) {
          if (POS(rson) != ZERO && (f2var(rson) == new_var)) {
            ...
            nlson = find_insert_bdd(vp, f2lson(lson),
                                    neg ? NOT(f2lson(rson)) : f2lson(rson));
            nrson = find_insert_bdd(vp, f2rson(lson),
                                    neg ? NOT(f2rson(rson)) : f2rson(rson));
          } else {
            /*					*/
            /*         x                  y     	*/
            /*        / \                / \    	*/
...
static relprod_cache_ptr find_in_relprod_cache(formula v, formula a,
                                               formula b) {
  unint idx;
...
static bool bdd_eq(pointer p1, pointer p2) { return (p1 == p2); }
...
static bool cache_equ(pointer p1, pointer p2) {
  ...
  return ((cp1->fn == cp2->fn) && (cp1->u.args.lson == cp2->u.args.lson) &&
...
static formula b_substitute(formula f, subst_ptr subs) {
  ...
  *old = tcache_rec;
  ...
  if (ISNOT(f)) {
    ret =
        B_Or(B_And(b_substitute(NOT(f2lson(f)), subs->next), subs->expr),
...
static int draw_bdd_rec(FILE *fp, hash_record *hp, formula f) {
  ...
  if ((res = PTR2INT(find_hash(hp, FORMULA2PTR(f)))) != 0) {
    return res;
  }
