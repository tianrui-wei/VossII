...
static bool is_Nil(g_ptr node) { return (IS_CONSTRUCTOR(Nil, node)); }

static bool is_NilAbove(g_ptr node, g_ptr *doc) {
  EXTRACT(*doc);
  return (IS_CONSTRUCTOR(NilAbove, node));
}

static bool is_TxtBeside(g_ptr node, int *len, string *txt, g_ptr *doc) {
  g_ptr g_txt;
  EXTRACT(*doc);
  EXTRACT(g_txt);
  if (!IS_CONSTRUCTOR(TxtBeside, node))
    return FALSE;
...
static bool is_Nest(g_ptr node, int *k, g_ptr *doc) {
  g_ptr g_k;
  EXTRACT(*doc);
  EXTRACT(g_k);
  if (!IS_CONSTRUCTOR(Nest, node))
    return FALSE;
  *k = GET_INT(g_k);
  return TRUE;
}

static bool is_Union(g_ptr node, g_ptr *doc1, g_ptr *doc2) {
  EXTRACT(*doc2);
  EXTRACT(*doc1);
  return (IS_CONSTRUCTOR(Union, node));
}

static bool is_Beside(g_ptr node, g_ptr *doc1, bool *b, g_ptr *doc2) {
  g_ptr g_b;
  EXTRACT(*doc2);
  EXTRACT(g_b);
  EXTRACT(*doc1);
  if (!IS_CONSTRUCTOR(Beside, node))
    return FALSE;
  *b = (GET_BOOL(g_b) == B_One()) ? TRUE : FALSE;
  return TRUE;
}

static bool is_Above(g_ptr node, g_ptr *doc1, bool *b, g_ptr *doc2) {
  g_ptr g_b;
  EXTRACT(*doc2);
  EXTRACT(g_b);
  EXTRACT(*doc1);
  if (!IS_CONSTRUCTOR(Above, node))
    return FALSE;
  *b = (GET_BOOL(g_b) == B_One()) ? TRUE : FALSE;
  return TRUE;
}

static bool is_Empty(g_ptr node, ...) { return (IS_CONSTRUCTOR(Empty, node)); }

#undef EXTRACT

static string get_doc_constructor(g_ptr node) {
  g_ptr d, p;
  int len, k;
  string txt;
  bool b;
  ...
  if (is_NilAbove(node, &d)) {
    return s_NilAbove;
  }
  if (is_TxtBeside(node, &len, &txt, &d)) {
    return s_TxtBeside;
  }
  if (is_Nest(node, &k, &d)) {
    return s_Nest;
  }
  if (is_Union(node, &d, &p)) {
    return s_Union;
  }
  if (is_Beside(node, &d, &b, &p)) {
    return s_Beside;
  }
  if (is_Above(node, &d, &b, &p)) {
    return s_Above;
  }
  if (is_Empty(node)) {
    return s_Empty;
...
static void do_pr(io_ptr ip, string s) {
...
static bool doc2print(int k, string end, g_ptr d, io_ptr ip) {
start:
  if (is_Nil(d)) {
    do_pr(ip, end);
    return TRUE;
...
static void do_layout(g_ptr redex) {
...
void Doc_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      "do_layout", "1111", FALSE,
      GLmake_arrow(
          GLmake_int(),
          GLmake_arrow(
              GLmake_string(),
              GLmake_arrow(doc_tp, GLmake_arrow(stream_tp, GLmake_void())))),
      do_layout);
}
