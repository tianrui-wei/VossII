...
static int get_callback_fun(string cmd) {
  string s = cmd;
  if (*s != 'F' && (*s && (*(s + 1) != 'l'))) {
    return (-2);
  }
...
void Tcl_callback_eval(string cmd, int rid, FILE *tcl_fp) {
  ...
  while (type->typeOp == arrow_tp) {
    string start = p;
    if (*start == 0) {
      Fail_pr("Argument %d missing for callback function %s", arg_cnt + 1,
              tcp->name);
      fprintf(tcl_fp, "%d 0%s\n", rid, protect(FailBuf));
    ...
    *p = tmp;
    ...
    if (!tcl_to_g_ptr(arg_s, arg_type, &arg)) {
      Fail_pr("Argument %d is of wrong type in tcl function %s", arg_cnt,
              tcp->name);
      fprintf(tcl_fp, "%d 0%s\n", rid, protect(FailBuf));
      free_temp_str_mgr(tstrings);
      return;
...
static bool get_end_of_item(string txt, string *endp) {
  string s = txt;
  int curl_cnt = 0;
  while (*s) {
    switch (*s) {
...
static bool tcl_to_g_ptr(string txt, typeExp_ptr type, g_ptr *resp) {
  ...
  case list_tp: {
    ...
    *resp = Make_NIL();
    ...
    while (*cur) {
      string end;
      if (!get_end_of_item(cur, &end)) {
        return FALSE;
      }
      char tmp = *end;
      *end = 0;
      g_ptr element;
      if (!tcl_to_g_ptr(cur, ltype, &element))
        return FALSE;
      *end = tmp;
      if (*end)
        end++;
...
static bool can_be_sent_to_tcl(g_ptr np, typeExp_ptr type) {
  ...
  case list_tp: {
    ...
    FOR_CONS(np, li, el) {
      if (!can_be_sent_to_tcl(el, etype)) {
        return FALSE;
      }
    }
    return TRUE;
...
static bool ok_tcl_callback_type_rec(typeExp_ptr type) {
  type = Get_Real_Type(type);
  switch (type->typeOp) {
  case string_tp:
  case int_tp:
...
static bool ok_tcl_callback_type(typeExp_ptr type) {
  ...
  while (type->typeOp == arrow_tp) {
    typeExp_ptr arg_type = type->typelist->type;
    if (!ok_tcl_callback_type_rec(arg_type)) {
      return FALSE;
    }
    type = Get_Real_Type(type->typelist->next->type);
  }
