...
g_cmp_restart:
  ...
  case LEAF: {
    ...
    case PRIM_FN:
      ...
      case P_EXTAPI_FN:
        return (((int)GET_EXTAPI_FN(n1)) - ((int)GET_EXTAPI_FN(n2)));
      case P_SSCANF:
      case P_PRINTF:
      case P_SPRINTF:
      case P_EPRINTF:
      case P_FPRINTF:
        return (strcmp(GET_PRINTF_STRING(n1), GET_PRINTF_STRING(n2)));
      case P_REF_VAR: {
    ...
    case EXT_OBJ: {
      ...
      return (PTR2INT(GET_EXT_OBJ(n1)) - PTR2INT(GET_EXT_OBJ(n2)));
    }
    case VAR:
    ...
    case USERDEF: {
      ...
      return (strcmp(Get_Fun_Signature(fn1), Get_Fun_Signature(fn2)));
    }
    default:
      DIE("Not possible");
    }
...
static int g_cmp(const void *pi, const void *pj) {
  g_ptr *i = (g_ptr *)pi;
  g_ptr *j = (g_ptr *)pj;
  INC_REFCNT(*i);
  INC_REFCNT(*j);
...
static void fl_qsort(g_ptr redex) {
...
static void rev_assoc(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr key = GET_APPLY_RIGHT(l);
  g_ptr alist = GET_APPLY_RIGHT(redex);
...
static void el(g_ptr redex) {
  ...
  // Not found
  MAKE_REDEX_FAILURE(
      redex, Fail_pr("el %d in list with only %d elements", idx, last_idx));
  DEC_REF_CNT(l);
  DEC_REF_CNT(r);
  return;
}

static void sitlist(g_ptr redex) {
  // sitlist fn l z
  g_ptr fn, list, init;
...
static void length(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr list = GET_APPLY_RIGHT(redex);
  int cnt = 0;
...
static void setify(g_ptr redex) {
...
static void butlast(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr list = r;
  if (IS_NIL(list)) {
...
static void butfirstn(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr gcnt;
  g_ptr list;
  ...
  for (int i = 0; i < cnt; i++) {
    if (IS_NIL(list)) {
      string msg =
          Fail_pr("butfirstn %d on list with only %d elements", cnt, len);
...
static void lastn(g_ptr redex) {
  ...
  if (cnt > len) {
    string msg = Fail_pr("lastn %d on list with only %d elements", cnt, len);
    MAKE_REDEX_FAILURE(redex, msg);
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    return;
...
static void butlastn(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr gcnt;
  g_ptr list;
  ...
  if (cnt > len) {
    string msg = Fail_pr("butlastn %d on list with only %d elements", cnt, len);
    MAKE_REDEX_FAILURE(redex, msg);
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    return;
...
static void firstn(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr gcnt;
  g_ptr list;
  ...
  for (int i = 0; i < cnt; i++) {
    if (IS_NIL(list)) {
      string msg = Fail_pr("firstn %d on list with only %d elements", cnt, len);
      MAKE_REDEX_FAILURE(redex, msg);
      DEC_REF_CNT(l);
      DEC_REF_CNT(r);
      POP_GLOBAL_GC(2);
...
static void find_first0(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  ...
  while (!IS_NIL(list)) {
    ...
    if (res == B_One()) {
      ...
      MAKE_REDEX_FAILURE(
          redex, "find_first0 predicate evaluated to symbolic condition");
      DEC_REF_CNT(l);
      DEC_REF_CNT(r);
      return;
    }
...
static void foreach_list_item(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void find_all(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr pred = GET_APPLY_RIGHT(l);
  g_ptr list = GET_APPLY_RIGHT(redex);
...
static void filter(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr pred = GET_APPLY_RIGHT(l);
  g_ptr list = GET_APPLY_RIGHT(redex);
  PUSH_GLOBAL_GC(pred);
...
static void partition(g_ptr redex) {
...
static void my_union(g_ptr redex) {
  g_ptr set1, set2;
...
static void intersect(g_ptr redex) {
  g_ptr set1, set2;
  EXTRACT_2_ARGS(redex, set1, set2);
  if (IS_NIL(set1)) {
    OVERWRITE(redex, set1);
...
static void subtract(g_ptr redex) {
  g_ptr set1, set2;
  EXTRACT_2_ARGS(redex, set1, set2);
  if (IS_NIL(set1)) {
    OVERWRITE(redex, set1);
...
static void append(g_ptr redex) {
  g_ptr list1, list2;
...
static void gather(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr list, indices;
...
static void separate(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr list, indices;
  EXTRACT_2_ARGS(redex, list, indices);
...
void List_ops_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      "qsort", "11", FALSE,
      GLmake_arrow(GLmake_arrow(tv1, GLmake_arrow(tv1, GLmake_int())),
                   GLmake_arrow(GLmake_list(tv1), GLmake_list(tv1))),
      fl_qsort);

  Add_ExtAPI_Function(
      "assoc", "11", FALSE,
      GLmake_arrow(tv1, GLmake_arrow(GLmake_list(GLmake_tuple(tv1, tv2)), tv2)),
      assoc);

  Add_ExtAPI_Function(
      "rev_assoc", "11", FALSE,
      GLmake_arrow(tv1, GLmake_arrow(GLmake_list(GLmake_tuple(tv2, tv1)), tv2)),
      rev_assoc);

  Add_ExtAPI_Function(
      "mem", "11", FALSE,
      GLmake_arrow(tv1, GLmake_arrow(GLmake_list(tv1), GLmake_bool())), mem);

  Add_ExtAPI_Function(
      "el", "10", FALSE,
      GLmake_arrow(GLmake_int(), GLmake_arrow(GLmake_list(tv1), tv1)), el);

  Add_ExtAPI_Function(
      "find_all", "11", FALSE,
      GLmake_arrow(GLmake_arrow(tv1, GLmake_bool()),
                   GLmake_arrow(GLmake_list(tv1), GLmake_list(GLmake_int()))),
      find_all);

  Add_ExtAPI_Function(
      "foreach", "10", FALSE,
      GLmake_arrow(GLmake_arrow(tv1, GLmake_void()),
                   GLmake_arrow(GLmake_list(tv1), GLmake_void())),
      foreach_list_item);

  Add_ExtAPI_Function(
      "sitlist", "-2-", FALSE,
      GLmake_arrow(GLmake_arrow(tv1, GLmake_arrow(tv2, tv2)),
                   GLmake_arrow(GLmake_list(tv1), GLmake_arrow(tv2, tv2))),
      sitlist);

  Add_ExtAPI_Function(
      "find_first0", "10", FALSE,
      GLmake_arrow(GLmake_arrow(tv1, GLmake_bool()),
  ...
  Add_ExtAPI_Function("rev", "2", FALSE,
                      GLmake_arrow(GLmake_list(tv1), GLmake_list(tv1)), rev);
  ...
  Add_ExtAPI_Function("length", "2", FALSE,
                      GLmake_arrow(GLmake_list(tv1), GLmake_int()), length);

  Add_ExtAPI_Function("setify", "1", FALSE,
                      GLmake_arrow(GLmake_list(tv1), GLmake_list(tv1)), setify);

  Add_ExtAPI_Function(
      "flat", "3", FALSE,
      GLmake_arrow(GLmake_list(GLmake_list(tv1)), GLmake_list(tv1)), flat);
  ...
  Add_ExtAPI_Function(
      ...
      GLmake_arrow(
          ...
          GLmake_arrow(GLmake_list(tv1),
                       GLmake_tuple(GLmake_list(tv1), GLmake_list(tv1)))),
      split);

  Add_ExtAPI_Function("partition", "11", FALSE,
                      GLmake_arrow(GLmake_arrow(tv1, tv2),
                                   GLmake_arrow(GLmake_list(tv1),
  ...
  Add_ExtAPI_Function("last", "2", FALSE, GLmake_arrow(GLmake_list(tv1), tv1),
                      last);

  Add_ExtAPI_Function(
      "gather", "21", FALSE,
      GLmake_arrow(GLmake_list(tv1),
                   GLmake_arrow(GLmake_list(GLmake_int()), GLmake_list(tv1))),
      gather);
  ...
  Add_ExtAPI_Function("gcmp", "11", FALSE,
                      GLmake_arrow(tv1, GLmake_arrow(tv1, GLmake_int())), gcmp);
}
