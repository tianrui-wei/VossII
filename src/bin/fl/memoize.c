...
void Init_Memo() { create_hash(&memo_tbl, 100, memo_hash, memo_eq); }

g_ptr Mk_unique(g_ptr nd) {}
...
static unsigned int memo_hash(pointer np, unsigned int n) {
  ...
  case CONS_ND:
    return (
        (((unsigned int)GET_CONS_HD(nd)) + ((unsigned int)GET_CONS_TL(nd))) %
        n);
  case APPLY_ND:
    return ((((unsigned int)GET_APPLY_LEFT(nd)) +
             ((unsigned int)GET_APPLY_RIGHT(nd))) %
            n);
  case LEAF:
