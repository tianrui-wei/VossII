...
void LP_Install_Functions() {
  Add_ExtAPI_Function(
      ...
      GLmake_arrow(GLmake_list(GLmake_tuple(
                       ...
                       GLmake_tuple(GLmake_string(), GLmake_int()))),
...
static void LPsol(g_ptr redex) {
  ...
  while (r != NULL && *r != NULL) {
    g_ptr tmp = Make_CONS_ND(Make_AINT_leaf((*r)->n), Make_AINT_leaf((*r)->d));
...
static void SetUpLP(int nrows, int nvars) {
  int i;
  rational_init();
...
static Tableau *FindBfs(Tableau *a) {
  ...
  for (i = 1; i <= aa->nrow; i++) {
    ...
    for (; j <= aa->ncol; j++) { /* slack variables */
      if (j == a->ncol + i)
        aa->rows[i].cols[j] = rat_copy(rat_one);
      else
        aa->rows[i].cols[j] = rat_copy(rat_zero);
    }
  }
...
static void standard3(Tableau *sa, Row **cv, int cn) {
  ...
  if (sa->ur) {
    ...
    for (i = 1; i <= sa->ur; i++)
      sa->unresolved[i].cols =
          (Rational *)Malloc((sa->uc + 1) * sizeof(Rational));
  }
  ...
  for (i = ii = 1; i <= a->nrow; i++) {
    ...
    for (; jj <= sa->ncol; jj++) {
      if (jj == slack) {
        switch (a->rows[i].rel) {
        case NE:
          cc[slack--] = rat_copy(rat_one);
          ii--;
          break;
        case EQ:
          cc[slack] = rat_copy(rat_zero);
          break;
        case GT:
        case GE:
          cc[slack--] = rat_copy(rat_MinusOne);
          break;
        case LT:
        case LE:
          cc[slack--] = rat_copy(rat_one);
          break;
        default:
          DIE("Internal error: standard4.2---bad relation");
        }
      } else {
...
static Rational rat_copy(Rational x) { return rat_return(x->n, x->d); }
...
static int rat_cmp(Rational x, Rational y) {
  arbi_T xn, yn;
  int retval;
...
static Rational rat_inverse(Rational x) { return (rat_return(x->d, x->n)); }

static Rational rat_div(Rational x, Rational y) {
  Rational r, inv;
...
static void DBG_write_int(odests f, int i) {
  char buf[32];
...
static void DBG_write_relation(odests f, int r) {
  switch (r) {
  case LT:
    DBG_write_string(f, "< ");
    break;
  ...
  case COST:
    ...
    break;
  default:
    DIE("Internal error: write_relation --- bad relation");
  }
}

static void DBG_write_dimension(odests f, Tableau *a) {
...
static void DBG_write_rows(odests f, Tableau *a) {
  int i;
...
void PRINT_Tableau(Tableau *a) { lp_WriteTableau(err_fp, a); }
