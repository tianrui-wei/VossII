...
/* Error logger to use when fl_init calls back into
 * register_types/register_funs. */
...
int plugin_already_loaded(fl_plugin plugin) {
...
void mark_plugin_as_loaded(fl_plugin plugin) {
...
static void insert_fixity(fl_plugin_fun f) {
  switch (f->fixity) {
  case NO_FIXITY:
    /* Nothing to do here. */
    break;
 ...
 */
static fl_plugin_api register_plugin(fl_plugin plugin) {
...
int register_types(int num_types, fl_plugin_types types) {
  ...
  for (int i = 0; i < num_types; i++) {
    types[i].class = Add_ExtAPI_Object(
        types[i].name, types[i].mark, types[i].sweep, types[i].save,
        types[i].load, types[i].obj2string, types[i].equals, types[i].gmap,
        types[i].gmap2, NULL // No SHA256 computation
    );
...
int register_funs(int num_funs, fl_plugin_funs funs) {
  ...
  for (int i = 0; i < num_funs; i++) {
    Add_ExtAPI_Function(funs[i].name, funs[i].strictness, funs[i].non_lazy,
                        funs[i].type, funs[i].fun_ptr);
...
/* Load the first plugin found with the given name.
   ...
   The output string is guaranteed to persist until the next call to
   load_plugin.
 */
int load_plugin(char *plugin_name, char **out) {
  ...
  *out = error;
  ...
  if (!fp) {
    snprintf(error, MAX_ERROR_SIZE, "Plugin file '%s' does not exist.",
             filename);
    return 0;
  }
  fclose(fp);
  ...
  if (!fl_init) {
    snprintf(error, MAX_ERROR_SIZE,
             "Plugin does not export 'fl_init'. Is this really an fl plugin?");
    return 0;
  }
