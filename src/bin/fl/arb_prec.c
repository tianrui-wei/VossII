...
int Arbi_cmp(arbi_T x, arbi_T y) {
  ...
  if (x[lx - 1] < y[lx - 1])
    return (arbi_LESS);
  else if (x[lx - 1] > y[lx - 1])
    return (arbi_GREAT);
  for (i = lx - 2; i >= 0; i--) {
    if ((unsigned long)(x[i]) < (unsigned long)(y[i]))
      return (arbi_LESS);
    else if ((unsigned long)(x[i]) > (unsigned long)(y[i]))
      return (arbi_GREAT);
  }
  return (arbi_EQ);
}
...
char *Arbi_ToString(arbi_T n, int base) /* FL doesn't garbage collect strings */
{
  char *ret;
...
arbi_T Arbi_FromInt(long int i) { return (arbi_collect(arbi__FromInt(i))); }

arbi_T Arbi_cpy(arbi_T x) { return (arbi_collect(arbi__cpy(x))); }

arbi_T Arbi_neg(arbi_T x) { return (arbi_collect(arbi__neg(x))); }

arbi_T Arbi_abs(arbi_T x) { return (arbi_collect(arbi__abs(x))); }

arbi_T Arbi_add(arbi_T x, arbi_T y) { return (arbi_collect(arbi__add(x, y))); }

arbi_T Arbi_sub(arbi_T x, arbi_T y) { return (arbi_collect(arbi__sub(x, y))); }

arbi_T Arbi_mlt(arbi_T x, arbi_T y) { return (arbi_collect(arbi__mlt(x, y))); }

arbi_T Arbi_div(arbi_T x, arbi_T y) { return (arbi_collect(arbi__div(x, y))); }

arbi_T Arbi_mod(arbi_T x, arbi_T y) { return (arbi_collect(arbi__mod(x, y))); }

arbi_T Arbi_gcd(arbi_T x, arbi_T y) { return (arbi_collect(arbi__gcd(x, y))); }

void Arbi_free(arbi_T n) { arbi_free((void *)n); }
...
arbi_T Arbi_bvNOT(arbi_T x) { return (arbi_collect(arbi__bvop(x, x, bv_not))); }

bool Arbi_IsNegative(arbi_T x) { return (cmp0(x) < 0); }
...
static void ffree(arbi_T x) { arbi_free((void *)x); }
...
static Word bv_xor(Word a, Word b) { return (a ^ b); }

static Word bv_or(Word a, Word b) { return (a | b); }

static Word bv_and(Word a, Word b) { return (a & b); }
...
static arbi_T arbi__bvop(arbi_T x, arbi_T y,
                         Word (*operation)(Word a, Word b)) {
  Length_T lx, ly, lz;
  int i, sign;
  arbi_T z;
...
static arbi_T arbi__mlt(arbi_T x, arbi_T y) {
  ...
  for (i = 0; i < lp; i++) {
    ...
    for (j = i - kmax; j <= jmax; j++) {
      ...
      yy = ay[k];
      Split(yy, yl, yh);
      pp = ((unsigned s_Word)xl) * ((unsigned s_Word)yl);
      Acc(p0, p1, pp);
      pp = ((unsigned s_Word)xl) * ((unsigned s_Word)yh);
      Acc(p1, p2, pp);
      pp = ((unsigned s_Word)xh) * ((unsigned s_Word)yl);
      Acc(p1, p2, pp);
      pp = ((unsigned s_Word)xh) * ((unsigned s_Word)yh);
      Acc(p2, p3, pp);
    }
    Carry(p0, p1);
    Carry(p1, p2);
...
static void divmod(arbi_T x, arbi_T y, arbi_T *qr) {
  ...
  if (yy == 0) {
    ...
    if (ys < 0)
      ...
      z = (yy << ys) |
          ((unsigned s_Word)(y[ly - 2]) >> (unsigned long)(WordSize - ys));
  }
  yh = z >> (HalfSize - 1);
  yl = z & (Hmask >> 1);
    ...
    if (cmpa(y, r) > 0) {
      ...
      if (rr == 0) { /* lr > 1, because Abs(r) > Abs(y) */
        ...
        if (rs < 0)
          ...
          z = (rr << rs) | (((unsigned s_Word)(r[lr - 2])) >>
...
static void *arbi_alloc(unsigned int nbytes) {
  ...
  if (BlockIndex < 0)
    ...
    space_left = BlockSize - (BlockPos - (blocks[BlockIndex]));
...
static char *WAMalloc(unint n) { return ((char *)Malloc(n)); }
