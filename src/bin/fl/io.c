...
static bool check_arg(g_ptr *rootp, g_ptr **spp, int *depthp, int n,
                      g_ptr redex, g_ptr arg);
static bool prs(int pfn, int i, g_ptr redex, char *s, bool ljust, bool zfill,
                int size);
static bool force_arg_and_check(g_ptr redex, g_ptr arg);
static bool print_list(int pfn, g_ptr l, int i, g_ptr redex, bool ljust,
                       bool zfill, int size);
...
FILE *Get_fp(io_ptr ip) { return (ip->fp); }
...
string Get_StreamName(io_ptr ip) { return (ip->name); }
...
bool Fopen(g_ptr *rootp, g_ptr **spp, int *depthp) {
  ...
  if (!check_arg(rootp, spp, depthp, 2, redex, arg1))
    return (TRUE);
  g_ptr arg2 = force(GET_APPLY_RIGHT(*(*spp + 1)), FALSE);
  if (!check_arg(rootp, spp, depthp, 2, redex, arg2))
    return (TRUE);
...
bool Fflush(g_ptr *rootp, g_ptr **spp, int *depthp) {
  ...
  g_ptr redex = **spp;
  g_ptr arg1 = force(GET_APPLY_RIGHT(**spp), FALSE);
  if (!check_arg(rootp, spp, depthp, 1, redex, arg1))
    return (TRUE);
...
bool Fclose(g_ptr *rootp, g_ptr **spp, int *depthp) {
  ...
  if (strcmp(ip->name, "stdin") == 0) {
    ...
    *rootp = redex;
    return (TRUE);
  } else if (strcmp(ip->name, "stdout") == 0) {
    g_ptr l = GET_APPLY_LEFT(redex);
    g_ptr r = GET_APPLY_RIGHT(redex);
    ...
    *rootp = redex;
    return (TRUE);
  } else if (strcmp(ip->name, "stderr") == 0) {
    g_ptr l = GET_APPLY_LEFT(redex);
    g_ptr r = GET_APPLY_RIGHT(redex);
    ...
    *rootp = redex;
    return (TRUE);
  } else if (strcmp(ip->name, "stdinfo") == 0) {
    g_ptr l = GET_APPLY_LEFT(redex);
    g_ptr r = GET_APPLY_RIGHT(redex);
  ...
  if (cur == NULL) {
    ...
    g_ptr r = GET_APPLY_RIGHT(redex);
    MAKE_REDEX_FAILURE(redex,
                       Fail_pr("Trying to close %s more than once!", ip->name));
    DEC_REF_CNT(l);
    DEC_REF_CNT(r);
    *spp = *spp + 1;
    *depthp -= 1;
...
static bool in_range(char c, int max) {
  if (max < 10) {
    int ci = c - '0';
    return (0 <= ci && ci <= max);
  } else {
...
bool Sscanf(g_ptr *rootp, g_ptr **spp, int *depthp) {
  ...
  PROCESS_PRINTF_PAT(fmt, sscanf_cnt_args_lbl2, /* lbl */
                     {},                        /* start_of_pat */
                     {},                        /* pat_0 */
                     {},                        /* pat_1_to_9 */
                     {},                        /* pat_minus */
  ...
  PROCESS_PRINTF_PAT(
      ...
      {
        /* pat_B */
        MAKE_REDEX_FAILURE(redex, Fail_pr("-E- sscanf does not support %%B"));
        goto end_sfn;
      },
      {
        /* pat_S */
        MAKE_REDEX_FAILURE(redex, Fail_pr("-E- sscanf does not support %%S"));
        goto end_sfn;
      },
      {}, /* pat_error */
      {
  ...
  PROCESS_PRINTF_PAT(
      ...
      },
      {
        ...
        if (Arbi_IsNegative(v)) {
          ...
          if (!prs(pfn, i, redex, Arbi_ToString(v, 2), ljust, zfill, size))
            goto end_pfn;
      ...
      },
      {
        ...
        if (!prs(pfn, i, redex, Arbi_ToString(v, 8), ljust, zfill, size))
          goto end_pfn;
        if (neg) {
          string s = tmp;
          if (*s != '0') {
      ...
      },
      {
        ...
        if (!prs(pfn, i, redex, Arbi_ToString(v, 16), ljust, zfill, size))
          goto end_pfn;
        if (neg) {
          string s = tmp;
          if (*s != '0') {
      ...
      },
      {
        ...
        if (!prs(pfn, i, redex, Arbi_ToString(v, 10), ljust, zfill, size))
          goto end_pfn;
        if (neg) {
          string s = tmp;
          if (*s != '0') {
      ...
      {
        /* pat_s */
        if (!prs(pfn, i, redex, GET_STRING(GET_APPLY_RIGHT(*(sp + i))), ljust,
                 zfill, size))
          goto end_pfn;
        i++;
      },
...
bool Save_graph(string type_sig, string file_name, g_ptr node) {
  ...
  // Tar up the files
  Sprintf(buf, "tar -c -z -C %s -f %s signature bexprs bools strings graph",
          dir, file_name);
  if (system(buf) != 0) {
...
bool Load_graph(string type_sig, string file_name, g_ptr redex) {
  ...
  if (fscanf(fp, "%d\n", &g_node_cnt) != 1) {
    fclose(fp);
    MAKE_REDEX_FAILURE(redex,
                       Fail_pr("Syntax error at line 1 in %s", sig_file_name));
    Unserialize_End();
    return (FALSE);
  }
  string tmp = strtemp("");
  ...
  if (strncmp(tmp, type_sig, strlen(type_sig)) != 0) {
    MAKE_REDEX_FAILURE(redex,
                       Fail_pr("Type signature mismatch in %s", file_name));
    Unserialize_End();
    return (FALSE);
  }
  // ------------- Read bools ------------------
  ...
  if (!Load_BDDs(bool_file_name, &bool_results)) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("Failed to read BDDs in %s", file_name));
  ...
  if (!Load_bexprs(bexpr_file_name, &bexpr_results)) {
    MAKE_REDEX_FAILURE(redex,
                       Fail_pr("Failed to read bexprs in %s", file_name));
    free_buf(&bool_results);
  ...
  while (!feof(fp)) {
    ...
    for (int i = 0; i < sz; i++) {
      if ((c = fgetc(fp)) == EOF) {
        MAKE_REDEX_FAILURE(redex, Fail_pr("Syntax error in %s on line %d",
                                          str_file_name, i + 1));
        fclose(fp);
        free_buf(&bool_results);
        free_buf(&bexpr_results);
...
static void c_mktemp(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void c_mktempd(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  string base = GET_STRING(r);
  char buf[1024];
  ...
  if (res == NULL) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("Failed to create temporary directory"));
    return;
  }
...
static void file_exists(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void stream2filename(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
void Io_Install_Functions() {
  ...
  Add_ExtAPI_Function("mktemp", "1", FALSE,
                      GLmake_arrow(GLmake_string(), GLmake_fp()), c_mktemp);
  ...
  Add_ExtAPI_Function("feof", "1", FALSE,
                      GLmake_arrow(GLmake_fp(), GLmake_bool()), Feof);

  Add_ExtAPI_Function("fgets", "1", FALSE,
                      GLmake_arrow(GLmake_fp(), GLmake_string()), Fgets);

  Add_ExtAPI_Function("fget", "1", FALSE,
                      GLmake_arrow(GLmake_fp(), GLmake_string()), Fget);
}
  ...
  if (IS_NIL(l)) {
    ...
    while (!IS_NIL(l)) {
      ...
      if (sep == ',')
        charappend(' ');
      sep = ',';
      if (!prs(pfn, i, redex, GET_STRING(GET_CONS_HD(l)), ljust, zfill, size))
        return (FALSE);
      l = GET_CONS_TL(l);
    }
    charappend(']');
...
static bool can_be_saved(g_ptr np) {
restart:
  if (find_hash(&g_save_tbl, (pointer)np) != NULL)
    return (TRUE);
  insert_hash(&g_save_tbl, (pointer)np, (pointer)np);
  switch (GET_TYPE(np)) {
  case APPLY_ND: {
    if (!can_be_saved(GET_APPLY_LEFT(np)))
      return (FALSE);
  ...
  case CONS_ND: {
    if (IS_NIL(np))
      return (TRUE);
    if (!can_be_saved(GET_CONS_HD(np)))
      return (FALSE);
    np = GET_CONS_TL(np);
    goto restart;
  }
    ...
    case PRIM_FN:
      ...
      case P_FILEFP: {
        ...
        if ((strcmp(ip->name, "stdout") == 0) ||
          ...
          Fail_pr("Cannot save stream %s", ip->name);
          return FALSE;
        }
      }
      default:
