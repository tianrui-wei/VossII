...
static inline bool is_match(const mat_ptr iso, const mat_ptr p,
                            const mat_ptr g);
// Utils for iso. algo.
static void trim(mat_ptr iso, mat_ptr p, mat_ptr g, unint r, unint c,
                 updates_ptr *ps);
...
// Strict isomatching.
static void strict_search(mat_ptr iso, mat_ptr p, mat_ptr g, mat_ptr c,
                          unint row, bool *used);
...
static string sprint_row(mat_ptr m, unint r) {
...
static string sprint_mat(mat_ptr m) {
  tstr_ptr ts = new_temp_str_mgr();
  string buf = gen_strtemp(ts, "[");
...
static bool mk_iso_list(sig_ptr *sigs, g_ptr p) {
  debug_print("%p, %p\n", (void *)sigs, (void *)p);
  ...
  if (is_P_HIER(cont, &children)) {
    ...
    *sigs = sig;
    ...
    FOR_CONS(children, tmp, child) {
      if (!is_PINST(child, &name, &attrs, &leaf, &fa_inps, &fa_outs, &inter,
                    &cont)) {
...
static inline unint sig_length(sig_ptr p) {
  unint len = 0;
  for (; p != NULL; p = p->next) {
    len++;
  }
...
static bool is_isomorphism(const mat_ptr iso, const mat_ptr p,
                           const mat_ptr g) {
  // P : AxA, G : BxB, ISO : AxB
...
static void strict_isomatch(mat_ptr iso, mat_ptr p, mat_ptr g, unint row) {
  bool *used;
  mat_ptr copy;
...
loop : {
  ...
  // /
  unint next = start;
  while (next < C && (s->used[next] || !M->mat[s->row][next])) {
    next++;
  }
  if (next < C) {
...
static void strict_isomatch_fn(g_ptr redex) {
  debug_print("%p\n", (void *)redex);
...
static void internal_search_create_fn(g_ptr redex) {
...
static void internal_search_step_fn(g_ptr redex) {
  debug_print("%p\n", (void *)redex);
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr g_search;
...
void Iso_Init() {
  // Search. state.
  search_mem_oidx = Add_ExtAPI_Object("internal_search_state",
                                      mark_search_mem_fn, sweep_search_mem_fn,
...
void Iso_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      ...
      // pex->pex->void
      ,
      GLmake_arrow(pexlif_tp, GLmake_arrow(pexlif_tp, GLmake_void())),
      strict_isomatch_fn);
  Add_ExtAPI_Function(
      "internal_search_create", "111",
      TRUE
      // pex->pex->search
      ,
      GLmake_arrow(
          pexlif_tp,
          GLmake_arrow(pexlif_tp, GLmake_arrow(GLmake_bool(), search_mem_tp))),
      internal_search_create_fn);
  Add_ExtAPI_Function("internal_search_step", "1",
                      TRUE
                      // search->step
                      ,
                      GLmake_arrow(search_mem_tp, step_tp),
