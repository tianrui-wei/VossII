...
g_ptr Gen_map2(string parent_op, g_ptr (*fun)(g_ptr, g_ptr), g_ptr l, g_ptr r,
               bool read_only);
...
#define SET_B_LEFT(np, p)                                                      \
  ((np)->L = (ui)((((ui)((np)->L)) & 0x3) | (((ui)p) & ~0x3)))

#define GET_LEFT(np) (((ui)((np)->L)) & ~0x7)
#define SET_LEFT(np, p)                                                        \
  ((np)->L = (ui)((((ui)((np)->L)) & 0x7) | (((ui)p) & ~0x7)))
#define GET_RIGHT(np) (((ui)((np)->R)) & ~0x3)
#define SET_RIGHT(np, p)                                                       \
  ((np)->R = (ui)((((ui)((np)->R)) & 0x3) | (((ui)p) & ~0x3)))
...
#define SET_TYPE(np, tag)                                                      \
  ((np)->L = (ui)((((ui)((np)->L)) & ~0x3) | (((ui)(tag)) & 0x3)))
#define MAX_REF_CNT 3
#define GET_REFCNT(np) (((ui)((np)->R)) & 0x3)
#define SET_REFCNT(np, m)                                                      \
  ((np)->R = (ui)((((ui)((np)->R)) & ~0x3) | (((ui)m) & 0x3)))
/* REF_CNT goes 00 -> 01 -> 11 (and gets stuck at 11) */
#define INC_REFCNT(np)                                                         \
  ((np)->R = (ui)(((((ui)((np)->R)) << 1) & 0x2) | 0x1 | ((ui)((np)->R))))
...
#define SET_MARK(np, m)                                                        \
  ((np)->R = (ui)((((ui)((np)->R)) & ~0x2) | ((((ui)m) & 0x1) << 1)))
...
#define IS_NIL(p)                                                              \
  (IS_CONS(p) && (M_GET_CONS_HD(p) == NULL) && (M_GET_CONS_TL(p) == NULL))

#define FOR_CONS(list, li, data)                                               \
  for (li = list; !IS_NIL(li) && (data = M_GET_CONS_HD(li), TRUE);             \
       li = M_GET_CONS_TL(li))
...
#define SET_LEAF_TYPE(np, t)                                                   \
  ((np)->L = (ui)((((ui)((np)->L)) & ~0x38) | ((((ui)t) & 0x7) << 3)))
...
#define SET_VERSION(np, f)                                                     \
  ((np)->L = (ui)((((ui)((np)->L)) & 0xffff003f) | (((ui)f) << 6)))
...
#define SET_EXT_OBJ_CLASS(np, c)                                               \
  ((np)->L = (ui)((((ui)((np)->L)) & ~0xffc0) | (((ui)c) << 6)))

#define IS_PRIM_FN(np) (GET_LEAF_TYPE(np) == PRIM_FN)
#define GET_PRIM_FN(np) ((GET_LEFT(np) >> 6) & 0x3ff)
#define SET_PRIM_FN(np, f)                                                     \
  ((np)->L = (ui)((((ui)((np)->L)) & 0xffff003f) | (((ui)f) << 6)))

#define IS_P_Y(np) (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_Y)
#define IS_P_CACHE(np)                                                         \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_CACHE)
#define IS_P_STRICT_ARGS(np)                                                   \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_STRICT_ARGS)
#define IS_P_ERROR(np)                                                         \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_ERROR)
#define IS_P_PCATCH(np)                                                        \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_PCATCH)
#define IS_REF_VAR(np)                                                         \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_REF_VAR)
#define IS_FAILURE(np)                                                         \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_FAIL)

#define IS_VOID(np) (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_VOID)

#define IS_DEBUG(np)                                                           \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_DEBUG)

#define IS_MK_REF_VAR(np)                                                      \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_MK_REF_VAR)

#define IS_UNQUOTE(np)                                                         \
  (IS_LEAF(np) && IS_PRIM_FN(np) && GET_PRIM_FN(np) == P_UNQUOTE)
...
#define SET_EXTAPI_FN(np, f)                                                   \
  ((np)->L = (ui)((((ui)((np)->L)) & 0xffff00000000ffff) | (((ui)f) << 16)))

#define GET_LINE_NBR(np) ((int)(GET_LEFT(np) >> 48))
#define SET_LINE_NBR(np, ln)                                                   \
  ((np)->L = (ui)((((ui)((np)->L)) & 0xffffffffffff) | (((ui)ln) << 48)))
...
typedef struct ext_obj_rec {
  ...
  int (*sha256_fn)(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                   pointer obj);
} ext_obj_rec;
