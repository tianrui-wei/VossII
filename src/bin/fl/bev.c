...
static bev_ptr get_bev_rec(g_ptr l) {
  bev_ptr res;
...
static void mark_bev_fn(pointer p) {
...
static int get_bev_length(g_ptr l) {
  int len = 0;
  while (!IS_NIL(l)) {
    len++;
    l = GET_CONS_TL(l);
...
static void save_bev_fn(FILE *fp, pointer p) {
...
static pointer load_bev_fn(FILE *fp) {
  int len;
...
static bool is_symbolic(bexpr f) { return (f != BE_One() && f != BE_Zero()); }

static string bev_list2str(g_ptr l, int depth) {
...
static string bev2str_fn(pointer p) {
  bev_ptr bp = (bev_ptr)p;
  g_ptr l = bp->u.l;
  return (bev_list2str(l, -1));
}

static formula bev_eq_fn(pointer p1, pointer p2, bool identical) {
  (void)identical;
...
static int bev_sha256_fn(int *g_cntp, hash_record *g_tblp, SHA256_ptr sha,
                         pointer a) {
...
static pointer bev_gmap_fn(gmap_info_ptr ip, pointer a) {
  bev_ptr bp = (bev_ptr)a;
  g_ptr l = bp->u.l;
  if (ip->read_only) {
...
static int zx2(g_ptr *lp1, g_ptr *lp2) {
  g_ptr l1 = *lp1;
  g_ptr l2 = *lp2;
  int len1 = get_bev_length(l1);
  int len2 = get_bev_length(l2);
...
static pointer bev_gmap2_fn(gmap_info_ptr ip, pointer a1, pointer a2) {
...
static g_ptr add_rec(bool neg, g_ptr l1, g_ptr l2, bexpr *coutp) {
  if (IS_NIL(l1)) {
    ASSERT(IS_NIL(l2));
...
static g_ptr gen_add(bool neg, g_ptr l1, g_ptr l2) {
  bexpr cout;
...
static bexpr is_zero_list(g_ptr l) {
  bexpr zero = BE_One();
  while (!IS_NIL(l)) {
...
static g_ptr negate_bev_list(g_ptr l) {
  g_ptr zero = Make_CONS_ND(Zero, Make_NIL());
  sx2(&l, &zero);
  return (trim_bev(gen_add(TRUE, zero, l)));
}

static g_ptr decrement_bev_list(g_ptr l) {
  g_ptr one = Make_CONS_ND(Zero, Make_CONS_ND(One, Make_NIL()));
  sx2(&l, &one);
  return (trim_bev(gen_add(TRUE, l, one)));
}

static g_ptr ite_bev_list(bexpr cond, g_ptr l1, g_ptr l2) {
  sx2(&l1, &l2);
  g_ptr res = Make_NIL();
  g_ptr tail = res;
  g_ptr n1, n2;
  for (n1 = l1, n2 = l2; !IS_NIL(n1);
       n1 = GET_CONS_TL(n1), n2 = GET_CONS_TL(n2)) {
...
static bexpr less_rec(g_ptr l1, g_ptr l2) {
  if (IS_NIL(l1)) {
    ASSERT(IS_NIL(l2));
    return (BE_Zero());
  }
  ...
  bexpr b = GET_BEXPR(GET_CONS_HD(l2));
  return (
      BE_Or(BE_And(BE_Not(a), b),
            BE_And(BE_Xnor(a, b), less_rec(GET_CONS_TL(l1), GET_CONS_TL(l2)))));
}

static bexpr less_than_bev_list(g_ptr l1, g_ptr l2) {
  sx2(&l1, &l2);
...
static bexpr bev_eq_list(g_ptr l1, g_ptr l2) {
  int len1 = get_bev_length(l1);
  int len2 = get_bev_length(l2);
...
static bool is_scalar_list(g_ptr list) {
  bool scalar = TRUE;
...
static int constant_power_of_two(g_ptr bev) {
  if (!is_scalar_list(bev)) {
    return (-1);
  }
  bool found = FALSE;
...
static void fixed_ext_bevdiv(g_ptr av, g_ptr bev, g_ptr *Qp, g_ptr *Rp) {
  ...
  while (!IS_NIL(av)) {
    ...
    for (int i = sz - 1; i >= 0; i--) {
      ...
      geq = BE_Or(BE_And(ri, BE_Not(bi)), BE_And(BE_Xnor(ri, bi), geq));
    }
...
static void bev_construct(g_ptr redex) {
...
static void bev_destruct(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static void bev2str(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_size(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bev_ptr bp = (bev_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void bev_var(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
static arbi_T bev_list2int(g_ptr l, arbi_T *ppow) {
  if (IS_NIL(l)) {
    *ppow = Arbi_FromInt(1);
...
static void bev2int(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bev_ptr bp = (bev_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void int2bev(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1 = GET_APPLY_RIGHT(redex);
...
static void bev_add(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_sub(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_mul(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
  ...
  g_ptr abs_res = Make_CONS_ND(Zero, mult_bev_list(abs_av, abs_bev));
...
static void signed_bevdiv(g_ptr dv, g_ptr nv, g_ptr *Q, g_ptr *R) {
...
static void bev_ashr(g_ptr redex) {
  g_ptr arg1;
  g_ptr arg2;
  EXTRACT_2_ARGS(redex, arg1, arg2);
  int cnt = GET_INT(arg2);
  if (cnt < 0) {
    MAKE_REDEX_FAILURE(redex, Fail_pr("bev_ashr by negative amount (%d)", cnt));
    return;
  }
  if (cnt == 0) {
...
static void bev_div(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_mod(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_NOT(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bev_ptr bp = (bev_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void bev_ZX(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  bev_ptr bp = (bev_ptr)GET_EXT_OBJ(GET_APPLY_RIGHT(redex));
  g_ptr list = bp->u.l;
...
static void bev_AND(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_OR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_XOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_XNOR(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_less(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_leq(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_greater(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_geq(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_eq(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
static void bev_neq(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
  g_ptr arg1;
  g_ptr arg2;
...
void Bev_Init() {
  ...
  PUSH_GLOBAL_GC(One);
  bev_free_list = NULL;
  bev_oidx = Add_ExtAPI_Object("bev", mark_bev_fn, sweep_bev_fn, save_bev_fn,
                               load_bev_fn, bev2str_fn, bev_eq_fn, bev_gmap_fn,
                               bev_gmap2_fn, bev_sha256_fn);
...
void Bev_Install_Functions() {
  ...
  Add_ExtAPI_Function("bev_size", "1", FALSE,
                      GLmake_arrow(bev_handle_tp, GLmake_int()), bev_size);

  Add_ExtAPI_Function("int2bev", "1", FALSE,
                      GLmake_arrow(GLmake_int(), bev_handle_tp), int2bev);

  Add_ExtAPI_Function("bev2int", "1", FALSE,
                      GLmake_arrow(bev_handle_tp, GLmake_int()), bev2int);

  Add_ExtAPI_Function("bev_variable", "1", FALSE,
                      GLmake_arrow(GLmake_string(), bev_handle_tp), bev_var);

  Add_ExtAPI_Function(
      "bev2str", "11", FALSE,
      GLmake_arrow(GLmake_int(), GLmake_arrow(bev_handle_tp, GLmake_string())),
      bev2str);

  Add_ExtAPI_Function(
      "bev_add", "11", FALSE,
      GLmake_arrow(bev_handle_tp, GLmake_arrow(bev_handle_tp, bev_handle_tp)),
      bev_add);
  ...
  Add_ExtAPI_Function(
      "bev_ashr", "11", FALSE,
      GLmake_arrow(bev_handle_tp, GLmake_arrow(GLmake_int(), bev_handle_tp)),
      bev_ashr);
  ...
  Add_ExtAPI_Function("bev_NOT", "1", FALSE,
                      GLmake_arrow(bev_handle_tp, bev_handle_tp), bev_NOT);

  Add_ExtAPI_Function("bev_ZX", "1", FALSE,
                      GLmake_arrow(bev_handle_tp, bev_handle_tp), bev_ZX);
  ...
  Add_ExtAPI_Function(
      "bev_less", "11", FALSE,
      GLmake_arrow(bev_handle_tp, GLmake_arrow(bev_handle_tp, GLmake_bexpr())),
      bev_less);
