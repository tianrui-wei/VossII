...
static unint primes[] = {
    241,       503,        1019,       2039,     4091,     8179,      16369,
    32749,     65521,      131063,     262139,   524269,   1048571,   2097143,
    4194287,   8388593,    16777199,   33554393, 67108859, 134217689, 268435399,
    536870879, 1073741789, 2147483629, 0};
...
g_ptr Find_in_g_cache(int cache_tbl, g_ptr argl) {
  g_cache_ptr gp;
  ui hash;
  g_ptr *recp;
...
void Insert_in_g_cache(int cache_tbl, g_ptr argl, g_ptr res) {
  g_cache_ptr gp;
  ui hash, i;
  g_ptr *recp;
...
static void clean_clets(g_ptr redex) {
...
void Cache_ops_Install_Functions() {
  ...
  Add_ExtAPI_Function("clean_clets", "1", FALSE,
                      GLmake_arrow(GLmake_void(), GLmake_void()), clean_clets);
}
...
static unint g_rec_hash_rec(pointer p, unint n, unint sz) {
  if (sz == 0) {
    return 13;
  }
  g_ptr nd = (g_ptr)p;
repeat_g_rec_hash:
  ...
  case CONS_ND:
    return ((931 * g_rec_hash_rec(GET_CONS_HD(nd), n, sz - 1) +
             137 * g_rec_hash_rec(GET_CONS_TL(nd), n, sz - 1)) %
            n);
  case APPLY_ND:
    return ((931 * (PTR2UINT(GET_APPLY_LEFT(nd))) +
             (PTR2UINT(GET_APPLY_RIGHT(nd)))) %
            n);
  case LEAF:
...
unint G_rec_hash(pointer p, unint n) {
...
restart_g_rec_equ:
  ...
  switch (type) {
  case CONS_ND:
    if (!G_rec_equ(GET_CONS_HD(n1), GET_CONS_HD(n2))) {
      return (FALSE);
    }
    n1 = GET_CONS_TL(n1);
    n2 = GET_CONS_TL(n2);
  ...
  case LEAF:
    ...
    case PRIM_FN:
      if (GET_PRIM_FN(n1) != GET_PRIM_FN(n2)) {
        return FALSE;
      }
      ...
      case P_EPRINTF:
      case P_SSCANF:
        return (GET_PRINTF_STRING(n1) == GET_PRINTF_STRING(n2));
      case P_EXTAPI_FN:
        return (GET_EXTAPI_FN(n1) == GET_EXTAPI_FN(n2));
      case P_FILEFP:
        return (GET_FILE_IO_PTR(n1) == GET_FILE_IO_PTR(n2));
      case P_FAIL:
        return (M_GET_FAIL_STRING(n1) == M_GET_FAIL_STRING(n2));
      case P_DEBUG:
        return (M_GET_DEBUG_STRING(n1) == M_GET_DEBUG_STRING(n2));
      default:
        return (TRUE);
      }
    case EXT_OBJ: {
      ...
      formula eq = op->eq_fn(GET_EXT_OBJ(n1), GET_EXT_OBJ(n2), TRUE);
      if (eq == B_One()) {
        return TRUE;
      } else {
        return FALSE;
...
static bool cache_eq(g_ptr n1, g_ptr n2) {
  int type, ltype;
  arbi_T ai1, ai2;
...
static unsigned int cache_hash(g_ptr nd, unint n) {
  if (nd == NULL)
    return (0);
