...
bool VDB_has_name_collision(vector_db_ptr vdbp, string vec, bool basename_only);
...
#define DBG_PRINT_RNG(rp)                                                      \
  {                                                                            \
    ...
    for (range_ptr rs = rp; rs != NULL; fst = FALSE, rs = rs->next) {          \
      if (rs->upper == rs->lower) {                                            \
        fprintf(stderr, "%s%d", (fst ? "" : ", "), rs->upper);                 \
      } else {                                                                 \
        fprintf(stderr, "%s%d:%d, ", (fst ? "" : ", "), rs->upper, rs->lower); \
      }                                                                        \
    }                                                                          \
  }
...
#define DBG_PRINT_VEC(vp)                                                      \
  for (vec_ptr vs = vp; vs != NULL; vs = vs->next) {                           \
    if (vs->type == TXT) {                                                     \
      fprintf(stderr, "%s", vs->u.name);                                       \
    } else {                                                                   \
      DBG_PRINT_RNG(vs->u.ranges);                                             \
    }                                                                          \
  }
...
#define DBG_PRINT_STR_LIST(slp)                                                \
  {                                                                            \
    bool fst = TRUE;                                                           \
    fprintf(stderr, "[");                                                      \
    for (sname_list_ptr sls = slp; sls != NULL;                                \
         fst = FALSE, sls = sls->next) {                                       \
      if (!fst)                                                                \
        fprintf(stderr, ", ");                                                 \
