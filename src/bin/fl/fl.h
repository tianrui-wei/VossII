...
#define DEC_REF_CNT(nd)                                                        \
  DPR((nd));                                                                   \
  PRIM_DEC_REF_CNT((nd));
#define DEC_REF_CNT2(nd)                                                       \
  DPR((nd));                                                                   \
  PRIM_DEC_REF_CNT2((nd));
#else
...
#define GET_OLD_L(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)1))))
...
#define GET_NEW_H(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)2))))
#define SET_NEW_H(np, f)                                                       \
  {                                                                            \
    formula t = (f);                                                           \
    store_buf(&ValBuf, ((np)->start + (unint)2), (pointer)&t);                 \
  }
#define GET_NEW_L(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)3))))
#define SET_NEW_L(np, f)                                                       \
  {                                                                            \
    formula t = (f);                                                           \
    store_buf(&ValBuf, ((np)->start + (unint)3), (pointer)&t);                 \
  }
#define GET_ASS_H(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)4))))
...
#define GET_ASS_L(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)5))))
...
#define GET_CHK_H(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)6))))
...
#define GET_CHK_L(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)7))))
...
#define GET_INV_H(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)8))))
...
#define GET_INV_L(np)                                                          \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)9))))
...
#define GET_WEAK_H(np)                                                         \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)10))))
#define SET_WEAK_H(np, f)                                                      \
  {                                                                            \
    formula t = (f);                                                           \
    store_buf(&ValBuf, ((np)->start + (unint)10), (pointer)&t);                \
  }
#define GET_WEAK_L(np)                                                         \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)11))))
...
#define GET_DEL_H(np, i)                                                       \
  (*((formula *)M_LOCATE_BUF(&ValBuf, ((np)->start + (unint)12 + i))))
#define SET_DEL_H(np, i, f)                                                    \
  {                                                                            \
    formula t = (f);                                                           \
    store_buf(&ValBuf, ((np)->start + (unint)12 + i), (pointer)&t);            \
  }

#define GET_DEL_L(np, i)                                                       \
  (*((formula *)M_LOCATE_BUF(&ValBuf,                                          \
                             ((np)->start + NBR_H_DELS(np) + (unint)13 + i))))

#define SET_DEL_L(np, i, f)                                                    \
  {                                                                            \
    formula t = (f);                                                           \
    store_buf(&ValBuf, ((np)->start + NBR_H_DELS(np) + (unint)13 + i),         \
              (pointer)&t);                                                    \
  }
...
#define DIE(...)                                                               \
  while (1) {                                                                  \
    ...
    if (RCadd_debug_info) {                                                    \
      FP(err_fp, "\n%s", Get_stack_trace(RCmax_stack_trace_entries));          \
      fprintf(stderr, "\n%s", Get_stack_trace(RCmax_stack_trace_entries));     \
    }                                                                          \
    exit(-1);                                                                  \
  };
