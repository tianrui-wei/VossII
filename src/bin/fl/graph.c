...
static g_ptr pat_match_rec(g_ptr arg, g_ptr E, int cnt, string *name, int *c);
...
void f_SET_APPLY_LEFT(g_ptr np, g_ptr v) { SET_APPLY_LEFT(np, v); }

void f_SET_APPLY_RIGHT(g_ptr np, g_ptr v) { SET_APPLY_RIGHT(np, v); }
...
void AddComment(string s) {
  comment_list_ptr clp = (comment_list_ptr)new_rec(&comment_list_rec_mgr);
  clp->comment = wastrsave(&strings, s);
  clp->line = line_nbr;
...
void Install_PrinterFn(result_ptr res) {
  ...
  if (setjmp(*start_envp) == 0) {
    ...
    if (is_fail(res->expr)) {
      ...
      insert_hash(&pretty_printer_tbl, (pointer)pr_type, (pointer)(res->expr));
    }
  } else {
...
bool is_fail(g_ptr node) {
  return (GET_TYPE(node) == LEAF && GET_LEAF_TYPE(node) == PRIM_FN &&
...
void Free_result_ptr(result_ptr rp) { free_rec(&result_rec_mgr, (pointer)rp); }

result_ptr Compile(symbol_tbl_ptr stbl, g_ptr onode, typeExp_ptr type,
                   bool delayed) {
  res_rec rt;
...
g_ptr Make_TYPE(string constr_name, int arg_cnt) {
...
static void mark_pp_fns(pointer key, pointer data) {
  g_ptr node;
  (void)key;
  node = (g_ptr)data;
  Mark(node);
...
#endif
{
  ...
  FOR_REC(&g_rec_mgr, g_ptr, nd) { SET_REFCNT(nd, 0); }
  Reset_Ref_cnts();
  ...
  Mark_tcl_callbacks();
  FOR_BUF(&global_gc_buf, g_ptr, ndp) { Mark(*ndp); }
  Mark_symbols();
...
void Debug_change_string(char *new_name, g_ptr n) { SET_STRING(n, new_name); }
...
void Print_leaf(g_ptr node, odests fp) {
  ...
  break;
  case PRIM_FN:
    FP(fp, "%s", Get_pfn_name(node, TRUE));
    break;
    break;
  case VAR:
...
g_ptr Gen_map2(string parent_op, g_ptr (*fun2)(g_ptr, g_ptr), g_ptr l, g_ptr r,
               bool read_only) {
  gmap_info_rec ir;
...
restart_walk2:
  ...
  case CONS_ND:
    ...
    return TRUE;
  case LEAF:
    if (GET_LEAF_TYPE(l) != GET_LEAF_TYPE(r))
      goto fail_walk2;
    if (IS_REF_VAR(l)) {
      if (IS_REF_VAR(r)) {
...
static string leaftype2str(int leaftype) {
  switch (leaftype) {
  case INT:
    return s_INT;
  case STRING:
...
static g_ptr gen_map2_rec(gmap_info_ptr ip, g_ptr l, g_ptr r) {
  ...
  if (IS_LEAF(l) && (GET_LEAF_TYPE(l) != GET_LEAF_TYPE(r))) {
    ...
    Sprintf(msg, "LEAFTYPE(l)=%s but LEAFTYPE(r)=%s",
            leaftype2str(GET_LEAF_TYPE(l)), leaftype2str(GET_LEAF_TYPE(r)));
    FAIL_GM2(l, r, msg);
  }
    ...
    if (IS_REF_VAR(l)) {
      if (IS_REF_VAR(r)) {
        ...
        if (lref_var != rref_var) {
          res = Make_0inp_Primitive(P_FAIL);
          SET_FAIL_STRING(res, Fail_pr("Different ref vars in gen_map2"));
          return res;
        }
...
static g_ptr gen_map_rec(gmap_info_ptr ip, g_ptr node) {
  ...
  case LEAF: {
    ...
    insert_hash(&(ip->gen_map_tbl), (pointer)node, (pointer)new_node);
    return (new_node);
  }
  default:
    DIE("Should never happen");
...
static int number_of_digits_needed(int n) {
  if (n == 0)
    return 1;
  int cnt = 0;
  while (n != 0) {
...
void Get_Vars(buffer *bp, g_ptr node) {
  ...
  case LEAF:
    ...
    if (is_T_fun(node) || is_F_fun(node)) {
      return;
    }
...
static int profile_cmp_time_per_invocation(const void *pi, const void *pj) {
...
static int profile_cmp_time(const void *pi, const void *pj) {
  profile_data_ptr p1 = (profile_data_ptr)pi;
  profile_data_ptr p2 = (profile_data_ptr)pj;
...
static int profile_cmp_invocations(const void *pi, const void *pj) {
  profile_data_ptr p1 = (profile_data_ptr)pi;
  profile_data_ptr p2 = (profile_data_ptr)pj;
  return (p2->invocation_cnt - p1->invocation_cnt);
}

static int profile_cmp_fun_name(const void *pi, const void *pj) {
  profile_data_ptr p1 = (profile_data_ptr)pi;
  profile_data_ptr p2 = (profile_data_ptr)pj;
  return (strcmp(p1->fun_name, p2->fun_name));
}

static void print_profile_data(FILE *fp, int sz, buffer *bp) {
  profile_data_ptr pd;
...
void Emit_profile_data() {
  ...
  if (fp == NULL) {
    ...
    fprintf(fp, "%-*s Invocations %15s %15s\n", sz, "Name", "Time",
            "Time/invocation");
    qsort(START_BUF(&out_buf), COUNT_BUF(&out_buf), sizeof(profile_data_rec),
...
static g_ptr type_constr_rec(int cnt, int arg_cnt, g_ptr constr) {
  ...
  Sprintf(buf, "_Q_%d", cnt);
  name = wastrsave(&strings, buf);
  return (Make_2inp_Primitive(
      P_TUPLE, type_constr_rec(cnt + 1, arg_cnt, constr), Make_VAR_leaf(name)));
}
...
static free_list_ptr make_set(string var) {
  free_list_ptr ret;
...
static void do_trace_dbg(int pfn, g_ptr root) {
  force_cnt++;
...
start:
  ...
  case LEAF:
    ...
    case P_SPRIME:
      ...
      redex = *(sp + 3);
      SET_APPLY_LEFT(redex, Make_APPL_ND(arg1, Make_APPL_ND(arg2, arg4)));
      INC_REFCNT(arg1);
      INC_REFCNT(arg2);
      INC_REFCNT(arg4);
    ...
    case P_BSTAR:
      ...
      SET_APPLY_LEFT(redex, arg1);
      INC_REFCNT(arg1);
      SET_APPLY_RIGHT(redex, Make_APPL_ND(arg2, Make_APPL_ND(arg3, arg4)));
      INC_REFCNT(arg2);
      INC_REFCNT(arg3);
      INC_REFCNT(arg4);
      goto finish4;
    ...
    case P_TIME: {
      ...
      SET_CONS_HD(redex, arg1);
      INC_REFCNT(arg1);
      SET_CONS_TL(redex, Make_STRING_leaf(wastrsave(&strings, buf)));
      goto finish1;
    }
    ...
    case P_CONSTR_EQ:
      ...
      if (GET_TYPE(arg1) != LEAF || GET_TYPE(arg2) != LEAF ||
          GET_LEAF_TYPE(arg1) != STRING || GET_LEAF_TYPE(arg2) != STRING) {
    ...
    case P_LOAD:
      ...
      if (is_fail(arg2))
        goto arg2_fail2;
      switch (Read_from_file(GET_STRING(arg1), (GET_BOOL(arg2) == B_One()),
                             FALSE)) {
      case 0:
        MAKE_REDEX_BOOL(redex, B_One());
        goto finish2;
    ...
    case P_GET_VOSSRC:
      ...
      case P_CHR:
        if (GET_INT(arg1) < 0 || GET_INT(arg1) > 127) {
          Fail_pr("Range error in CHR (%d)", GET_INT(arg1));
          goto fail1;
        }
        SET_TYPE(redex, LEAF);
        SET_LEAF_TYPE(redex, STRING);
    ...
    case P_THEREIS:
      ...
      SET_APPLY_LEFT(redex,
                     Make_1inp_Primitive((pfn == P_FORALL) ? P_QUANT_FORALL
                                                           : P_QUANT_THEREIS,
                                         arg3));
      ;
      INC_REFCNT(arg3);
    ...
    case P_QUANT_THEREIS:
      ...
      switch (pfn) {
      case P_QUANT_FORALL:
        res = B_forall(B_Depends(GET_BOOL(arg1)), GET_BOOL(arg2));
        break;
      case P_QUANT_THEREIS:
        res = B_thereis(B_Depends(GET_BOOL(arg1)), GET_BOOL(arg2));
        break;
      }
      SET_TYPE(redex, LEAF);
      SET_LEAF_TYPE(redex, BOOL);
    ...
    case P_RELPROD_FORALL:
      ...
      if (is_fail(arg1))
        goto arg1_fail3;
      Begin_RelProd();
      res = Rel_prod(B_Depends(GET_BOOL(arg1)), GET_BOOL(arg2), GET_BOOL(arg3),
    ...
    case P_HELP:
      ...
      if (s == NULL) {
        Fail_pr("No function named %s defined", GET_STRING(arg1));
        goto fail1;
      } else {
        SET_TYPE(redex, LEAF);
        SET_LEAF_TYPE(redex, STRING);
    ...
    case P_UPDATE_RVAR:
      ...
      redex = *(sp + 1);
      {
        ...
        if (cur_eval_cond != B_One()) {
          ...
          PUSH_GLOBAL_GC(arg3);
          arg2 = Gen_map2(s_update_ref_var, g_b_ite, arg2, arg3, FALSE);
          POP_GLOBAL_GC(1);
        }
...
static unsigned int graph_hash_rec(g_ptr node, unsigned int n) {
  pointer tmp;
  unsigned int res = 1;
...
formula Is_equal(g_ptr l1, g_ptr l2, bool identical) {
  ...
  case CONS_ND:
    ...
    if (res == B_Zero())
      return (res);
    PUSH_BDD_GC(res);
    res = B_And(res, Is_equal(GET_CONS_TL(l1), GET_CONS_TL(l2), identical));
    POP_BDD_GC(1);
    return (res);
  case LEAF:
    if (GET_TYPE(l2) != LEAF)
    ...
    case INT:
      ...
      return (B_Zero());
    case STRING:
      return ((STREQ(GET_STRING(l1), GET_STRING(l2))) ? B_One() : B_Zero());
    case BOOL:
      if (identical)
    ...
    case EXT_OBJ: {
      ...
      return (op->eq_fn(GET_EXT_OBJ(l1), GET_EXT_OBJ(l2), identical));
    }
    case PRIM_FN: {
...
static g_ptr reduce_list(g_ptr redex, int levels) {
  ...
  while (GET_TYPE(cur) == CONS_ND && !IS_NIL(cur)) {
    ...
    if (depth >= (unint)RCcall_limit) {
      Fail_pr("\n");
      Rprintf("Recursion limit reached while forcing list "
              "(RECURSION-CALL-LIMIT=%d)\n",
              RCcall_limit);
    }
...
static bool print_result_local(g_ptr node, odests fp, bool pr_brack,
  ...
  if (conv_fn != NULL) {
    ...
    if (is_fail(print_nd)) {
      FP(fp, "Pretty printer failed: %s", GET_FAIL_STRING(print_nd));
      return (FALSE);
    } else {
      string s;
...
static g_ptr compile_rec(g_ptr node) {
  ...
  case LAMBDA_ND:
    return (
        abstract_rec(GET_LAMBDA_VAR(node), compile_rec(GET_LAMBDA_BODY(node))));
  default:
    return (node);
  }
}
...
static void free_CL_node(cl_ptr nd) { free_rec(&cl_rec_mgr, (pointer)nd); }
...
static g_ptr pat_match_rec(g_ptr arg, g_ptr E, int cnt, string *name,
                           int *tot_cnt) {
  g_ptr res;
...
static g_ptr trarg(g_ptr node, g_ptr E, bool constructor) {
  ...
  case LEAF:
    ...
    case INT:
      ...
      ret = Make_Lambda(new,
                        Make_3inp_Primitive(
                            P_COND,
                            Make_2inp_Primitive(P_EQUAL, Make_VAR_leaf(new),
                                                Make_AINT_leaf(GET_AINT(node))),
                            E, Make_0inp_Primitive(P_PFAIL)));
      return (ret);
    case STRING:
      /* \x. x = val(node) => E | PFAIL */
      new = Find_new_free_var(E);
      ret = Make_Lambda(
          new, Make_3inp_Primitive(
                   P_COND,
                   Make_2inp_Primitive(P_CONSTR_EQ, Make_VAR_leaf(new),
    ...
    case BOOL:
      ...
      ret = Make_Lambda(new,
                        Make_3inp_Primitive(
                            P_COND,
                            Make_2inp_Primitive(P_IDENTICAL, Make_VAR_leaf(new),
  ...
  case CONS_ND:
    ...
    /* \x. (empty x) => E | PFAIL */
    new = Find_new_free_var(E);
    ret = Make_Lambda(new, Make_3inp_Primitive(
                               P_COND,
                               Make_1inp_Primitive(P_EMPTY, Make_VAR_leaf(new)),
                               E, Make_0inp_Primitive(P_PFAIL)));
    return (ret);
  case APPLY_ND:
      ...
      if (GET_TYPE(ll1) == LEAF && GET_LEAF_TYPE(ll1) == PRIM_FN) {
        ...
        case P_CAT:
          ...
          ret = Make_Lambda(
              ...
              Make_3inp_Primitive(
                  P_COND,
                  Make_2inp_Primitive(
                      P_NOT_EQUAL, Make_VAR_leaf(new),
                      Make_STRING_leaf(wastrsave(&strings, ""))),
                  Make_APPL_ND(
                      trarg(a,
                            Make_APPL_ND(trarg(b, E, FALSE),
                                         Make_1inp_Primitive(
                                             P_STRING_HD, Make_VAR_leaf(new))),
                            FALSE),
                      Make_1inp_Primitive(P_STRING_TL, Make_VAR_leaf(new))),
        ...
        case P_CONS:
          ...
          ret = Make_Lambda(
              new,
              Make_3inp_Primitive(
                  P_COND, Make_1inp_Primitive(P_IS_CONS, Make_VAR_leaf(new)),
                  Make_APPL_ND(
                      trarg(a,
                            Make_APPL_ND(trarg(b, E, FALSE),
                                         Make_1inp_Primitive(
                                             P_TAIL, Make_VAR_leaf(new))),
                            FALSE),
                      Make_1inp_Primitive(P_HEAD, Make_VAR_leaf(new))),
                  Make_0inp_Primitive(P_PFAIL)));
          return (ret);
        case P_TUPLE:
          /* ((TUPLE a) b) 				     */
          ...
          ret = Make_Lambda(
              new,
              Make_3inp_Primitive(
                  P_COND, Make_1inp_Primitive(P_IS_TUPLE, Make_VAR_leaf(new)),
                  Make_APPL_ND(
                      trarg(a,
                            Make_APPL_ND(
                                trarg(b, E, FALSE),
                                Make_1inp_Primitive(P_SND, Make_VAR_leaf(new))),
                            FALSE),
                      Make_1inp_Primitive(P_FST, Make_VAR_leaf(new))),
                  Make_0inp_Primitive(P_PFAIL)));
          return (ret);
        default:
          break;
      ...
      if (GET_TYPE(l1) == LEAF && GET_LEAF_TYPE(l1) == PRIM_FN) {
        ...
        case P_SUC:
          ...
          ret = Make_Lambda(
              ...
              Make_3inp_Primitive(
                  P_COND,
                  Make_2inp_Primitive(P_GREATER, Make_VAR_leaf(new),
                                      Make_INT_leaf(0)),
                  Make_APPL_ND(trarg(GET_APPLY_RIGHT(node), E, FALSE),
                               Make_2inp_Primitive(P_MINUS, Make_VAR_leaf(new),
                                                   Make_INT_leaf(1))),
                  Make_0inp_Primitive(P_PFAIL)));
          return (ret);
        default:
...
static string make_Destructor(g_ptr np) {
  ...
  if (Find_Function(symb_tbl, np) == NULL) {
    Rprintf("Illegal pattern in function definition. %s is not a type "
            "constructor\n",
            GET_VAR(np));
  }
  return (ret);
...
static g_ptr g_b_ite(g_ptr l, g_ptr r) {
  ...
  if (!IS_BOOL(l)) {
    if (Is_equal(l, r, TRUE) != B_One()) {
      res = Make_0inp_Primitive(P_FAIL);
      string m =
          wastrsave(&strings, "if-then-else over non-matching structures");
  ...
  formula bres =
      B_Or(B_And(current_bdd_cond, lb), B_And(B_Not(current_bdd_cond), rb));
  if (bres == lb) {
    INC_REFCNT(l);
    return l;
  }
...
static g_ptr mkConstr0(string name) { return (mk_constr_nd(name)); }

static g_ptr mkConstr1(string name, g_ptr e1) {
  return (Make_CONS_ND(mk_constr_nd(name), e1));
}

static g_ptr mkConstr2(string name, g_ptr e1, g_ptr e2) {
...
g_ptr Reflect_expr(g_ptr node) {
  ...
  case APPLY_ND:
    ...
    return (mkConstr2(s_APPLY, Reflect_expr(GET_APPLY_LEFT(node)),
                      Reflect_expr(GET_APPLY_RIGHT(node))));
  case LAMBDA_ND:
    return (mkConstr2(s_LAMBDA, Make_STRING_leaf(GET_LAMBDA_VAR(node)),
                      Reflect_expr(GET_LAMBDA_BODY(node))));
  case CONS_ND:
    return (mkConstr2(s_CONS, Reflect_expr(GET_CONS_HD(node)),
                      Reflect_expr(GET_CONS_TL(node))));
  case LEAF:
    switch (GET_LEAF_TYPE(node)) {
    case INT:
      return (
          mkConstr1(s_LEAF, mkConstr1(s_INT, Make_AINT_leaf(GET_AINT(node)))));
    case STRING:
      return (mkConstr1(
          s_LEAF, mkConstr1(s_STRING, Make_STRING_leaf(GET_STRING(node)))));
    case BOOL:
      return (
          mkConstr1(s_LEAF, mkConstr1(s_BOOL, Make_BOOL_leaf(GET_BOOL(node)))));
    case BEXPR:
      return (mkConstr1(s_LEAF,
                        mkConstr1(s_BEXPR, Make_BEXPR_leaf(GET_BEXPR(node)))));
    case PRIM_FN: {
      int pfn = GET_PRIM_FN(node);
      if (pfn == P_DEBUG) {
        string name = GET_DEBUG_STRING(node);
        return (mkConstr1(s_LEAF, mkConstr2(s_EXT_PRIM_FN, Make_INT_leaf(pfn),
                                            Make_STRING_leaf(name))));
      }
      if (pfn == P_PRINTF || pfn == P_SPRINTF || pfn == P_EPRINTF ||
          pfn == P_FPRINTF) {
      ...
      if (pfn == P_EXTAPI_FN) {
        string name = Get_ExtAPI_Function_Name(GET_EXTAPI_FN(node));
        return (mkConstr1(s_LEAF, mkConstr2(s_EXT_PRIM_FN, Make_INT_leaf(pfn),
                                            Make_STRING_leaf(name))));
      }
      return (mkConstr1(s_LEAF, mkConstr1(s_PRIM_FN, Make_INT_leaf(pfn))));
    }
    case VAR:
      return (mkConstr1(s_VAR, Make_STRING_leaf(GET_VAR(node))));
    case USERDEF: {
      // Not clear what to do here....
      fn_ptr fp = GET_USERDEF(node);
      return (mkConstr1(s_LEAF, mkConstr1(s_USERDEF, Make_INT_leaf(fp->id))));
    }
    default:
      DIE("Illegal leaf node type");
    }
...
formula Get_cur_eval_cond() { return cur_eval_cond; }
...
static bool is_a_list(g_ptr node) {
  g_ptr cur;
  if (node == NULL)
    return (FALSE);
  cur = node;
...
static bool cmp_graph_rec(hash_record *dtbl, g_ptr l1, g_ptr l2) {
  ...
  if (tmp != NULL) {
    ...
    if (cmp_graph_rec(dtbl, tmp, l2) == TRUE) {
      return TRUE;
    }
  ...
  case APPLY_ND:
    ...
    if (cmp_graph_rec(dtbl, GET_APPLY_RIGHT(l1), GET_APPLY_RIGHT(l2)) ==
        FALSE) {
...
static void print_free_set(pointer key, pointer data) {
  g_ptr np;
  free_list_ptr fp;
  np = (g_ptr)key;
...
void PFS() { scan_hash(&free_tbl, print_free_set); }
...
int Print_Nd_type(g_ptr nd) {
  switch (GET_TYPE(nd)) {
  case APPLY_ND:
    FP(err_fp, "APPLY_ND freed: Left=%ld, Right=%ld\n",
       (long int)GET_APPLY_LEFT(nd), (long int)GET_APPLY_RIGHT(nd));
    break;
  case LAMBDA_ND:
    FP(err_fp, "LAMBDA_ND freed: var=%s, body=%ld\n", GET_LAMBDA_VAR(nd),
       (long int)GET_LAMBDA_BODY(nd));
    break;
  case CONS_ND:
...
static void set_up_hash() { create_hash(&visited, 100, ptr_hash, ptr_equ); }
...
static void remove_hash() { dispose_hash(&visited, NULLFCN); }
