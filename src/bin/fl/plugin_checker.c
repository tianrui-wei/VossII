...
static int get_num_args(typeExp_ptr t) {
...
*/
static void check_arg_strictness(fl_plugin_fun f, plugin_errlog log) {
...
static void check_fun_ptr_not_null(fl_plugin_fun f, plugin_errlog log) {
  if (f->fun_ptr == NULL) {
...
static void check_fun_precedence_in_range(fl_plugin_fun f, plugin_errlog log) {
  int out_of_range = 0;
  switch (f->fixity) {
  case PREFIX:
...
static void check_fun_fixity(fl_plugin_fun f, plugin_errlog log) {
  switch (f->fixity) {
  case NO_FIXITY:
...
static void check_fun_has_name(fl_plugin_fun f, plugin_errlog log) {
...
static void check_fun_has_type(fl_plugin_fun f, plugin_errlog log) {
  if (f->type == NULL) {
...
static void check_fun_has_strictness(fl_plugin_fun f, plugin_errlog log) {
...
static void check_fun_not_null(fl_plugin_fun f, plugin_errlog log) {
  if (f == NULL) {
...
static void check_plugin_fun(fl_plugin_fun f, plugin_errlog log) {
  if (f == NULL) {
...
static void check_abi_version(fl_plugin p, plugin_errlog log) {
...
static void check_plugin_name(fl_plugin p, plugin_errlog log) {
...
static void check_exports_anything(fl_plugin p, plugin_errlog log) {
  if (p->num_funs <= 0) {
    log_plugin_error(log, "plugin does not export any functionality.");
  }
}

static void check_exports_nonnegative_types(fl_plugin p, plugin_errlog log) {
  if (p->num_types < 0) {
...
static void missing_callback(char *type, char *callback, plugin_errlog log) {
  static const char *fmt =
      "type '%s' has null function pointer for callback '%s'.";
  log_plugin_error(log, fmt, type, callback);
}

/* Check that all type fields are present. */
static void check_plugin_type(fl_plugin_type t, plugin_errlog log) {
  ...
  if (t->mark == NULL) {
    missing_callback(t->name, "mark", log);
  }
  if (t->sweep == NULL) {
    missing_callback(t->name, "sweep", log);
  }
  if (t->save == NULL) {
    missing_callback(t->name, "save", log);
  }
  if (t->load == NULL) {
    missing_callback(t->name, "load", log);
  }
  if (t->obj2string == NULL) {
    missing_callback(t->name, "obj2string", log);
  }
  if (t->equals == NULL) {
    missing_callback(t->name, "equals", log);
  }
  if (t->gmap == NULL) {
    missing_callback(t->name, "gmap", log);
  }
  if (t->gmap2 == NULL) {
    missing_callback(t->name, "gmap2", log);
  }
}

/* Sanity check the given plugin. */
void check_plugin(fl_plugin p, plugin_errlog log) {
...
void check_plugin_types(int num_types, fl_plugin_types types,
                        plugin_errlog log) {
