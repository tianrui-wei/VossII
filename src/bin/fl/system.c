...
*/
void Init_Paths(string bin_path, string lib_path) {
  if (bin_path == NULL) {
...
static void my_exec(g_ptr redex) {
...
static void do_pid(g_ptr redex) {
  int pid = getpid();
  MAKE_REDEX_INT(redex, pid);
}

static void noX_mode(g_ptr redex) {
  if (gui_mode) {
...
static void get_stack_trace(g_ptr redex) {
  g_ptr res = Get_fl_stack_trace();
  OVERWRITE(redex, res);
}

static void get_cur_eval_cond(g_ptr redex) {
  MAKE_REDEX_BOOL(redex, Get_cur_eval_cond());
}

static void get_USER(g_ptr redex) {
  MAKE_REDEX_STRING(redex, wastrsave(&strings, getenv("USER")));
}

static void normalize_file(g_ptr redex) {
  g_ptr l = GET_APPLY_LEFT(redex);
  g_ptr r = GET_APPLY_RIGHT(redex);
...
void System_Install_Functions() {
  ...
  Add_ExtAPI_Function(
      ...
      GLmake_arrow(GLmake_string(),
                   GLmake_tuple(GLmake_int(), GLmake_list(GLmake_string()))),
      my_exec);
  ...
  Add_ExtAPI_Function("get_stack_trace", "1", FALSE,
                      GLmake_arrow(GLmake_void(), GLmake_list(GLmake_string())),
                      get_stack_trace);
  ...
  Add_ExtAPI_Function("USER", "", FALSE, GLmake_string(), get_USER);
